...
const Jet_NDCompass_Navigation = {
	NONE: 0,
	VOR: 1,
	NAV: 2,
	ILS: 3
...
class Jet_NDCompass extends HTMLElement {
	...
	updateCompass(_deltaTime) {
		...
		const simTrack = SimVar.GetSimVarValue("GPS GROUND MAGNETIC TRACK", "degree");
		...
		if (this.displayMode !== Jet_NDCompass_Display.PLAN) {
			...
			this.setAttribute("rotation", fastToFixed(compass, 2));
		...
		if (this.referenceMode == Jet_NDCompass_Reference.HEADING) {
			...
			this.setAttribute("selected_tracking_bug_rotation", fastToFixed(selectedTrack, 3));
			if (this.trackingLine) {
		...
		this.setAttribute("heading_bug_rotation", fastToFixed(simHeading, 3));
		if (simGroundSpeed <= 10) {
			simTrack = simHeading;
		}

		this.setAttribute("tracking_bug_rotation", fastToFixed(simTrack, 3));
		if (this.ilsGroup) {
			if (this._showILS || this.navigationMode == Jet_NDCompass_Navigation.ILS) {
				let localizer = this.gps.radioNav.getBestILSBeacon();
				if (localizer.id > 0) {
					this.setAttribute("ils_bug_rotation", fastToFixed(localizer.course, 3));
		...
		if (this.mapRanges) {
			for (let i = 0; i < this.mapRanges.length; i++) {
				...
				if (range < 1.0 && this.mapRanges[i].removeInteger) {
					...
					this.mapRanges[i].text.textContent = rangeText.slice(rangeText.indexOf('.'));
				}
				else {
		...
		if (this.courseGroup) {
			if (this.navigationMode == Jet_NDCompass_Navigation.ILS || this.navigationMode == Jet_NDCompass_Navigation.VOR) {
				...
				switch (this.logic_brg1Source) {
					case 1:
						const hasNav = SimVar.GetSimVarValue("NAV HAS NAV:1", "boolean");
						if (hasNav) {
						...
						break;
					case 2:
						const hasNav = SimVar.GetSimVarValue("NAV HAS NAV:2", "boolean");
						if (hasNav) {
