    ...
    static ShowPage2(mcdu) {
        ...
        if (mcdu._zeroFuelWeightZFWCGEntered) {
            if (isFinite(mcdu.zeroFuelWeight)) {
                zfwCell = (mcdu.zeroFuelWeight * mcdu._conversion).toFixed(1);
                zfwColor = "[color]cyan";
            }
        ...
        mcdu.onRightInput[0] = async (value) => {
            if (value === "") {
                ...
                mcdu.sendDataToScratchpad(
                    (isFinite(mcdu.zeroFuelWeight) ? (mcdu.zeroFuelWeight * mcdu._conversion).toFixed(1) : "") +
                    "/" +
        ...
        if (mcdu._blockFuelEntered || mcdu._fuelPlanningPhase === mcdu._fuelPlanningPhases.IN_PROGRESS) {
            if (isFinite(mcdu.blockFuel)) {
                blockFuel = (mcdu.blockFuel * mcdu._conversion).toFixed(1);
        ...
        if (isFinite(mcdu.taxiFuelWeight)) {
            if (mcdu._taxiEntered) {
                taxiFuelCell = (mcdu.taxiFuelWeight * mcdu._conversion).toFixed(1);
            } else {
                taxiFuelCell = "{small}" + (mcdu.taxiFuelWeight * mcdu._conversion).toFixed(1) + "{end}";
            }
        }
        ...
        if (CDUInitPage.fuelPredConditionsMet(mcdu)) {
            ...
            if (isFinite(mcdu.takeOffWeight)) {
                towCell = "{small}" + (mcdu.takeOffWeight * mcdu._conversion).toFixed(1);
            ...
            if (mcdu._fuelPredDone) {
                ...
                if (mcdu._rteFinalEntered) {
                    if (isFinite(mcdu.getRouteFinalFuelWeight())) {
                        finalWeightCell = "{sp}{sp}" + (mcdu.getRouteFinalFuelWeight() * mcdu._conversion).toFixed(1);
                    ...
                    if (isFinite(mcdu.getRouteFinalFuelWeight())) {
                        finalWeightCell = "{sp}{sp}{small}" + (mcdu.getRouteFinalFuelWeight() * mcdu._conversion).toFixed(1) + "{end}";
                        finalTimeCell = FMCMainDisplay.minutesTohhmm(mcdu.getRouteFinalFuelTime());
                        finalColor = "[color]cyan";
                    }
                }
                ...
                if (mcdu._routeAltFuelEntered) {
                    if (isFinite(mcdu.getRouteAltFuelWeight())) {
                        altnWeightCell = "{sp}{sp}" + (mcdu.getRouteAltFuelWeight() * mcdu._conversion).toFixed(1);
                    ...
                    if (isFinite(mcdu.getRouteAltFuelWeight())) {
                        altnWeightCell = "{sp}{sp}{small}" + (mcdu.getRouteAltFuelWeight() * mcdu._conversion).toFixed(1);
                ...
                if (isFinite(mcdu.getTotalTripFuelCons()) && isFinite(mcdu.getTotalTripTime())) {
                    tripWeightCell = "{sp}{sp}{small}" + (mcdu.getTotalTripFuelCons() * mcdu._conversion).toFixed(1);
                ...
                if (mcdu._rteRsvPercentOOR) {
                    ...
                    if (isFinite(mcdu.getRouteReservedWeight()) && isFinite(mcdu.getRouteReservedPercent())) {
                        if (mcdu._rteReservedEntered) {
                            rteRsvWeightCell = "{sp}{sp}" + (mcdu.getRouteReservedWeight() * mcdu._conversion).toFixed(1);
                        } else {
                            rteRsvWeightCell = "{sp}{sp}{small}" + (mcdu.getRouteReservedWeight() * mcdu._conversion).toFixed(1) + "{end}";
                        }
                ...
                mcdu.tryUpdateLW();
                lwCell = (mcdu.landingWeight * mcdu._conversion).toFixed(1);
                ...
                if (mcdu._minDestFobEntered) {
                    minDestFob = "{sp}{sp}" + (mcdu._minDestFob * mcdu._conversion).toFixed(1);
                    minDestFobColor = "[color]cyan";
                } else {
                    mcdu.tryUpdateMinDestFob();
                    minDestFob = "{sp}{sp}{small}" + (mcdu._minDestFob * mcdu._conversion).toFixed(1) + "{end}";
                    minDestFobColor = "[color]cyan";
                }
                ...
                extraWeightCell = "{small}" + (mcdu.tryGetExtraFuel() * mcdu._conversion).toFixed(1);
