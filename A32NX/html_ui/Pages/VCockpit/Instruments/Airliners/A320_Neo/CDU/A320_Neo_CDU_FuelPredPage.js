            ...
            if (mcdu._fuelPredDone) {
                if (mcdu._rteFinalEntered) {
                    if (isFinite(mcdu.getRouteFinalFuelWeight())) {
                        finalFuelCell = "{sp}{sp}" + (mcdu.getRouteFinalFuelWeight() * mcdu._conversion).toFixed(1);
                    ...
                    if (isFinite(mcdu.getRouteFinalFuelWeight())) {
                        finalFuelCell = "{sp}{sp}{small}" + (mcdu.getRouteFinalFuelWeight() * mcdu._conversion).toFixed(1) + "{end}";
                        finalTimeCell = FMCMainDisplay.minutesTohhmm(mcdu.getRouteFinalFuelTime());
                        finalColor = "[color]cyan";
                    }
                }
                ...
                if (mcdu._routeAltFuelEntered) {
                    if (isFinite(mcdu.getRouteAltFuelWeight())) {
                        altFuelCell = "{sp}{sp}" + (mcdu.getRouteAltFuelWeight() * mcdu._conversion).toFixed(1);
                    ...
                    if (isFinite(mcdu.getRouteAltFuelWeight())) {
                        altFuelCell = "{sp}{sp}{small}" + (mcdu.getRouteAltFuelWeight() * mcdu._conversion).toFixed(1);
                ...
                if (mcdu.altDestination) {
                    ...
                    altEFOBCell = (mcdu.getAltEFOB(true) * mcdu._conversion).toFixed(1);
                ...
                destEFOBCell = (mcdu.getDestEFOB(true) * mcdu._conversion).toFixed(1);
                ...
                fobCell = "{small}" + (mcdu.getFOB() * mcdu._conversion).toFixed(1) + "{end}";
                fobOtherCell = "FF";
                fobCellColor = "[color]cyan";

                gwCell = "{small}" + (mcdu.getGW() * mcdu._conversion).toFixed(1);
                ...
                if (isFlying) {
                    ...
                    if (mcdu._rteRsvPercentOOR) {
                        ...
                        rteRsvWeightCell = (mcdu.getRouteReservedWeight() * mcdu._conversion).toFixed(1);
                        rteRsvPercentCell = (mcdu.getRouteReservedPercent() * mcdu._conversion).toFixed(1);
                ...
                if (mcdu._minDestFobEntered) {
                    minDestFobCell = "{sp}{sp}" + (mcdu._minDestFob * mcdu._conversion).toFixed(1);
                    minDestFobCellColor = "[color]cyan";
                } else {
                    mcdu.tryUpdateMinDestFob();
                    minDestFobCell = "{sp}{sp}{small}" + (mcdu._minDestFob * mcdu._conversion).toFixed(1) + "{end}";
                    minDestFobCellColor = "[color]cyan";
                }
                ...
                if (isFlying) {
                    extraFuelCell = "{small}" + ((mcdu.tryGetExtraFuel(true) + mcdu.getRouteReservedWeight()) * mcdu._conversion).toFixed(1);
                } else {
                    extraFuelCell = "{small}" + (mcdu.tryGetExtraFuel(true) * mcdu._conversion).toFixed(1);
                }
