...
(function (Airbus_FMA) {
    ...
    class CurrentPlaneState {
        ...
        static refresh() {
            ...
            this.isILSApproachActive = ((this.flightPhase == FMGC_FLIGHT_PHASES.APPROACH) && (approachType == ApproachType.APPROACH_TYPE_ILS));
            this.isNonILSApproachActive = ((this.flightPhase == FMGC_FLIGHT_PHASES.APPROACH) && (approachType != ApproachType.APPROACH_TYPE_ILS));
    ...
    class Column1 extends Airbus_FMA.ColumnBase {
        ...
        IsActive_MANFLX() {
            if (!Airbus_FMA.CurrentPlaneState.autoPilotThrottleActive && Airbus_FMA.CurrentPlaneState.autoPilotThrottleArmed && (Airbus_FMA.CurrentPlaneState.highestThrottleDetent == ThrottleMode.FLEX_MCT)) {
                if ((Airbus_FMA.CurrentPlaneState.flightPhase <= FMGC_FLIGHT_PHASES.TAKEOFF) && (Airbus_FMA.CurrentPlaneState.flexTemperature !== 0)) {
                    return true;
                }
            }
            return false;
        ...
        IsActive_MANSGA() {
            return Airbus_FMA.CurrentPlaneState.bothEnginesActive
                ...
                && Airbus_FMA.CurrentPlaneState.flightPhase === FMGC_FLIGHT_PHASES.GOAROUND;
        }
        ...
        IsActive_LVRCLB() {
            ...
            const inTakeoff = (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.TAKEOFF);
            const inClimb = (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.CLIMB);
            const inGoAround = (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.GOAROUND) || SimVar.GetSimVarValue("L:A32NX_GOAROUND_PASSED", "bool") === 1;
        ...
        IsActive_LVRMCT() {
            ...
            return false;
            const inTakeoff = (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.TAKEOFF);
            const inGoAround = (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.GOAROUND);
        ...
        IsActive_BRKLO() {
            return Airbus_FMA.CurrentPlaneState.autoBreakActivated
                ...
                && (Airbus_FMA.CurrentPlaneState.flightPhase > FMGC_FLIGHT_PHASES.TAKEOFF
                    || Airbus_FMA.CurrentPlaneState.flightPhase === FMGC_FLIGHT_PHASES.PREFLIGHT);
        }
        IsActive_BRKLOARMED() {
        ...
        IsActive_BRKMAX() {
            return Airbus_FMA.CurrentPlaneState.autoBreakDecel &&
                ...
                && Airbus_FMA.CurrentPlaneState.flightPhase < FMGC_FLIGHT_PHASES.CLIMB;
        }
    ...
    class Column2 extends Airbus_FMA.ColumnBase {
        ...
        getTargetRow1State() {
            ...
            if (altModeState == Airbus_FMA.MODE_STATE.ENGAGED) {
                if (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.CRUISE) {
        ...
        GetModeState_SRS() {
            if (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.TAKEOFF && Airbus_FMA.CurrentPlaneState.thisFlightDirectorActive) {
            ...
            } else if (Airbus_FMA.CurrentPlaneState.flightPhase === FMGC_FLIGHT_PHASES.GOAROUND) {
                if ((Airbus_FMA.CurrentPlaneState.highestThrottleDetent === ThrottleMode.TOGA
            ...
            if (Airbus_FMA.CurrentPlaneState.flightPhase != FMGC_FLIGHT_PHASES.TAKEOFF) {
                SRSEnabled = false;
            }
            if (SRSEnabled) {
                return Airbus_FMA.MODE_STATE.ENGAGED;
        ...
        static GetModeState_GS() {
            ...
            if (Airbus_FMA.CurrentPlaneState.flightPhase != FMGC_FLIGHT_PHASES.GOAROUND) {
        ...
        static GetModeState_CLB() {
            ...
            if ((Airbus_FMA.CurrentPlaneState.autoPilotAltitudeLockActive && Airbus_FMA.CurrentPlaneState.thisFlightDirectorActive || Simplane.getAutoPilotFLCActive() && Airbus_FMA.CurrentPlaneState.thisFlightDirectorActive) && (Airbus_FMA.CurrentPlaneState.altitude < Airbus_FMA.CurrentPlaneState.autoPilotAltitudeLockValue) && Airbus_FMA.CurrentPlaneState.thisFlightDirectorActive) {
                ...
                if (Airbus_FMA.CurrentPlaneState.flightPhase < FMGC_FLIGHT_PHASES.CLIMB) {
                    return Airbus_FMA.MODE_STATE.ARMED;
        ...
        static IsActive_DES() {
            if (Airbus_FMA.CurrentPlaneState.autoPilotAltitudeLockActive && (Airbus_FMA.CurrentPlaneState.altitude > Airbus_FMA.CurrentPlaneState.autoPilotAltitudeLockValue) && Airbus_FMA.CurrentPlaneState.thisFlightDirectorActive) {
                if (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.DESCENT && Simplane.getVerticalSpeed() < -10) {
                    return true;
                }
            }
            if (Simplane.getAutoPilotFLCActive() && Airbus_FMA.CurrentPlaneState.thisFlightDirectorActive) {
    ...
    class Column3 extends Airbus_FMA.ColumnBase {
        ...
        IsActive_RWY() {
            if (Airbus_FMA.CurrentPlaneState.flightPhase <= FMGC_FLIGHT_PHASES.CLIMB && Airbus_FMA.CurrentPlaneState.thisFlightDirectorActive) {
        ...
        IsArmed_HDG() {
            ...
            if (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.GOAROUND && Airbus_FMA.CurrentPlaneState.flapsHandlePercent != 0) {
        ...
        IsActive_HDG() {
            ...
            // Airbus_FMA.CurrentPlaneState.flightPhase != FMGC_FLIGHT_PHASES.GOAROUND || Column2.GetModeState_SRS() == false
            if (Airbus_FMA.CurrentPlaneState.autoPilotHeadingSelectedMode && !Airbus_FMA.CurrentPlaneState.autoPilotTRKFPAModeActive && Airbus_FMA.CurrentPlaneState.thisFlightDirectorActive && Airbus_FMA.CurrentPlaneState.radioAltitude >= 1.5) {
                ...
                if (!Simplane.getAutoPilotAPPRActive() && Airbus_FMA.CurrentPlaneState.flightPhase != FMGC_FLIGHT_PHASES.GOAROUND) {
                    retVal = true;
                }

                if (SimVar.GetSimVarValue("L:A32NX_GOAROUND_HDG_MODE", "bool") === 1 && Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.GOAROUND) {
                    retVal = true;
                }
                return retVal;
            }
        ...
        GetModeState_LOC() {
            if (Airbus_FMA.CurrentPlaneState.thisFlightDirectorActive && Simplane.getAutoPilotApproachType() != 10) {
                if (Simplane.getAutoPilotAPPRActive() && SimVar.GetSimVarValue("L:A32NX_OFF_LOC", "bool") == 1 && Airbus_FMA.CurrentPlaneState.flightPhase != FMGC_FLIGHT_PHASES.GOAROUND) {
                    return Airbus_FMA.MODE_STATE.CAPTURED;
                }
                if (Simplane.getAutoPilotAPPRActive() && Airbus_FMA.CurrentPlaneState.flightPhase != FMGC_FLIGHT_PHASES.GOAROUND) {
                    return Airbus_FMA.MODE_STATE.ENGAGED;
        ...
        GetModeState_NAV() {
            if (
                (Airbus_FMA.CurrentPlaneState.flightPhase !== FMGC_FLIGHT_PHASES.GOAROUND
                || (Airbus_FMA.CurrentPlaneState.flightPhase === FMGC_FLIGHT_PHASES.GOAROUND && Airbus_FMA.CurrentPlaneState.highestThrottleDetent === ThrottleMode.FLEX_MCT))
            ...
            if (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.GOAROUND && SimVar.GetSimVarValue("L:A32NX_GOAROUND_NAV_MODE", "bool") === 1 && SimVar.GetSimVarValue("L:A320_FCU_SHOW_SELECTED_HEADING", "number") === 0) {
                if (Airbus_FMA.CurrentPlaneState.radioAltitude >= 30) {
                    return Airbus_FMA.MODE_STATE.ENGAGED;
                } else {
                    return Airbus_FMA.MODE_STATE.ARMED;
    ...
    class Column4 extends Airbus_FMA.ColumnBase {
        ...
        updateRow3(_deltaTime) {
            ...
            if ((Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.DESCENT) || (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.APPROACH)) {
    ...
    class SpecialMessages {
        ...
        getCurrentRequiredMessage() {
            if (!Column1.IsActive_SpeedCommon() && (Airbus_FMA.CurrentPlaneState.managedAirspeed > 0) && ((Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.TAKEOFF) || (Airbus_FMA.CurrentPlaneState.flightPhase == FMGC_FLIGHT_PHASES.CLIMB))) {
