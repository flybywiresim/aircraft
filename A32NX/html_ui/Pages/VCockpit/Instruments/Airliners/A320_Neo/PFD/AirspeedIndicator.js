class Jet_PFD_AirspeedIndicator extends HTMLElement {
    ...
    update(dTime) {
        ...
        const greenDot = SimVar.GetSimVarValue("L:A32NX_VSPEEDS_GD", "number");
        const vR = SimVar.GetSimVarValue("L:AIRLINER_VR_SPEED", "Knots");
        const planeOnGround = Simplane.getIsGrounded();
        const vs = SimVar.GetSimVarValue("L:A32NX_VSPEEDS_VS", "number");
        const vls = SimVar.GetSimVarValue("L:A32NX_VSPEEDS_VLS", "number");
    ...
    updateMarkerF(_marker, currentAirspeed) {
        ...
        if (flapsHandleIndex == 2 || (flapsHandleIndex == 3 && !SimVar.GetSimVarValue("L:A32NX_VSPEEDS_LANDING_CONF3", "boolean"))) {
            let flapSpeed = 0;
            if (phase == FlightPhase.FLIGHT_PHASE_TAKEOFF || phase == FlightPhase.FLIGHT_PHASE_CLIMB || phase == FlightPhase.FLIGHT_PHASE_GOAROUND) {
                ...
                flapSpeed = SimVar.GetSimVarValue("L:A32NX_VSPEEDS_F", "number");
            }
            if (flapSpeed >= 60) {
    ...
    updateMarkerS(_marker, currentAirspeed) {
        ...
        if (flapsHandleIndex == 1) {
            ...
            if (phase == FlightPhase.FLIGHT_PHASE_TAKEOFF || phase == FlightPhase.FLIGHT_PHASE_CLIMB || phase == FlightPhase.FLIGHT_PHASE_GOAROUND) {
                ...
                slatSpeed = SimVar.GetSimVarValue("L:A32NX_VSPEEDS_S", "number");
            }
            if (slatSpeed >= 60) {
