    ...
    updateDraw(map) {
        ...
        if (isFinite(this.x) && isFinite(this.y)) {
            ...
            if (this.needRepaint || Math.abs(this._lastX - this.x) > 0.1 || Math.abs(this._lastY - this.y) > 0.1) {
                this._lastX = this.x;
                this._lastY = this.y;
                const x = (this.x - map.config.waypointIconSize * 0.5 * (this.minimize ? 0.5 : 1));
                const y = (this.y - map.config.waypointIconSize * 0.5 * (this.minimize ? 0.5 : 1));
                ...
                if (this._label) {
                    ...
                    const textY = y + map.config.waypointLabelDistance;
