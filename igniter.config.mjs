        ...
        new TaskOfTasks(
            ...
            [
                new ExecTask('copy-base-files', [
                    'if [ "${GITHUB_ACTIONS}" == "true" ]; then npm run build-a380x:link-base-files; else npm run build-a380x:copy-base-files; fi',
                    'npm run unchunkLargeFiles',
                    'if [ "${GITHUB_ACTIONS}" == "true" ]; then npm run build-a380x:link-large-files; else npm run build-a380x:copy-large-files; fi',
                    'if [ "${GITHUB_ACTIONS}" == "true" ]; then :; else npm run chunkLargeFiles; fi',
