...
export class PseudoOans {
  private static readonly oansHealthyVar = RegisteredSimVar.createBoolean('L:A380X_OANS_HEALTHY');
  ...
  private readonly oansFailed = MappedSubject.create(
  ...
  constructor(private readonly bus: EventBus) {
    this.oansFailed.sub((failed) => {
      PseudoOans.oansHealthyVar.set(!failed);
    }, true);
