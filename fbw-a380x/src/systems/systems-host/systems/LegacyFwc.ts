...
export class LegacyFwc {
    ...
    _updateFlightPhase(_deltaTime: number) {
        ...
        const activePhases = phases.map((x, i) => [x ? 1 : 0, i + 1]).filter((y) => y[0] === 1).map((z) => z[1]);
    ...
    _updateLandingMemo(_deltaTime: number) {
        ...
        const invalidRadioMemo = this.memoLdgMemo_conf02.write(radioHeight1Invalid && radioHeight2Invalid && gearDownlocked && this.flightPhase === 6, _deltaTime);
