...
export class NavigationDatabase {
    ...
    async searchFix(ident: string): Promise<Waypoint[]> {
        const waypoints = await this.backendDatabase.getWaypoints([ident]);
        const vors = await this.backendDatabase.getNavaids([ident]);
        const ndbs = await this.backendDatabase.getNDBs([ident]);

        return [...waypoints, ...vors, ...ndbs];
    }
