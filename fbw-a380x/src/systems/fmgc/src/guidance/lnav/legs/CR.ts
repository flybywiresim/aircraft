...
export class CRLeg extends Leg {
    ...
    recomputeWithParameters(
        ...
        const d1 = distanceTo(this.getPathStartPoint(), intersections[0]);
        const d2 = distanceTo(this.getPathStartPoint(), intersections[1]);
        if (d1 < d2) {
            this.intercept = intersections[0];
        } else {
            this.intercept = intersections[1];
        }
