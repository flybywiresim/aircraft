...
abstract class HXLeg extends XFLeg {
    ...
    protected getDistanceToGoThisOrbit(ppos: LatLongData): NauticalMiles {
        ...
        case HxLegGuidanceState.Arc1:
            return arcDistanceToGo(ppos, this.fix.location, arcCentreFix1, sweepAngle) + this.computeLegDistance() * 2 + this.radius * Math.PI;
    ...
    updatePrediction() {
        ...
        const windAngleToInbound = Math.abs(MathUtils.diffAngle(windDirection, this.inboundCourse));
