...
const strokeWidth = 3;
export const Button: FC<ButtonProps> = ({ x = 0, y = 0, width = 0, height = 41, children, onClick, disabled, fill, highlighted, gradient, forceHover }) => {
    ...
    return (
        <Layer x={x} y={y} {...hoverProps} onClick={onClick}>
            ...
            {Children.map(children, (child) => {
                ...
                return <text x={width / 2} y={height / 2} fill={disabled ? '#ababab' : 'white'} fontSize={16} textAnchor="middle" dominantBaseline="central">{child}</text>;
            })}
        </Layer>
    );
};
