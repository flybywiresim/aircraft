...
export const Dropdown: FC<DropdownProps> = ({ x, y = 0, width = 192, height = 60, dropDownWidth = 192, fill, selectable, title, children, active, disabled, scrollable, clipItems = true, maxHeight = Infinity }) => {
    ...
    return (
        <Layer {...hoverProps} x={x} y={y}>
            ...
            >
                {selectable
                    ? (
                        <text x={(width - height) / 2} y={height / 2} fill={fill} fontSize={18} dominantBaseline="middle" textAnchor="middle">{title}</text>
                    )
                    : (
                        <>
                            <rect
                ...
                {(selectable) && (
                    <Button x={width - height} width={height} height={height} onClick={() => setOpen(!open)} fill="#666" disabled={disabled} highlighted={open} forceHover={hovered} gradient>
                        <path fill="#ffffff" d={`m ${height / 2 - 14} ${height / 2 - 3} h 20 l -6 7 l -6 -7 z`} strokeLinejoin="round" />
                    </Button>
                )}
            </Button>
                            ...
                            Children.map(children, (child, index) => {
                                if (isValidElement(child)) {
                                    return React.cloneElement(child, {
                                        ...
                                        width: child.props.width ? child.props.width : (dropDownWidth ?? width) - (scrollable ? scrollBarWidth + 3 : 0),
...
export const ScrollBar: FC<ScrollBarProps> = ({ x, y, width = 48, maxHeight, totalChildHeight, scrollPosition, setScrollPosition }) => {
...
export const DropdownItem: FC<DropdownItemProps> = ({ y = 0, onSelect, width = 0, height = 0, centered, children }) => {
    ...
    return (
        <Layer y={y} {...hoverProps} onClick={onSelect}>
            <rect x={3} y={1} width={width - 6} height={height - 2} fill="transparent" stroke={hovered ? 'hsl(203, 100%, 49%)' : 'none'} strokeWidth={2} />
            <text x={centered ? width / 2 : 10} y={height / 2} fill="white" fontSize={18} dominantBaseline="central" textAnchor={centered ? 'middle' : 'start'}>{children}</text>
        </Layer>
    );
};
...
export const DropdownLink: FC<DropdownItemProps & { link: string }> = (props) => {
    ...
    return (<DropdownItem {...props} centered={false} onSelect={() => history.push(path + props.link)}>{props.children}</DropdownItem>);
};
