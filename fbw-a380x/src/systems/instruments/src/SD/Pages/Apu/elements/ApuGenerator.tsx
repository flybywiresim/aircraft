...
export const ApuGenerator: FC<ApuGeneratorProps> = ({ x, y, position }) => {
    ...
    const apuGenFreqNormal = frequency > APU_GEN_FREQUENCY_NORMAL_RANGE_LOWER && frequency < APU_GEN_FREQUENCY_NORMAL_RANGE_UPPER;
    const apuGenVoltageNormal = potential > APU_GEN_VOLTAGE_NORMAL_RANGE_LOWER && potential < APU_GEN_VOLTAGE_NORMAL_RANGE_UPPER;
    ...
    return (
        <g id={`apu-gen-${position}-indication`} transform={`translate(${x} ${y})`}>
            <text x={position === 1 ? 8 : 7} y={10} className={`F22 ${genPushbuttonAuto || !masterSwPbOn ? 'White' : 'Amber'}`}>
                GEN
            </text>
            <text x={position === 1 ? 65 : 64} y={10} className={`F22 ${genPushbuttonAuto || !masterSwPbOn ? 'White' : 'Amber'}`}>
                {position === 1 ? 'A' : 'B'}
            </text>
            ...
            <g className={masterSwPbOn && genPushbuttonAuto ? '' : 'Hide'}>
                ...
                <text
                    x={73}
                    y={42}
                    className={`F29 EndAlign ${apuGenLoadNormal ? 'Green' : 'Amber'} LS1`}
                >
                    {Math.round(load)}
                </text>
                {/* Voltage */}
                <text
                    x={73}
                    y={67}
                    className={`F29 EndAlign ${apuGenFreqNormal ? 'Green' : 'Amber'} LS1`}
                >
                ...
                <text
                    x={73}
                    y={93}
                    className={`F29 EndAlign ${apuGenVoltageNormal ? 'Green' : 'Amber'} LS1`}
                >
            ...
            <text
                x={72}
                y={62}
                className={`F29 EndAlign White LS1 ${genPushbuttonAuto || !masterSwPbOn ? 'Hide' : ''}`}
            >
                OFF
            </text>

            {/* GEN line contactor indication */}
            <Triangle x={position === 1 ? 43 : 39} y={-29} colour={lineContactorClosed ? 'Green' : 'White'} fill={0} orientation={0} scale={1} />
        </g>
    );
};
