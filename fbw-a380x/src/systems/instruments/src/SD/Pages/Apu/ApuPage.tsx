...
export const ApuPage = () => {
    ...
    return (
        <>
            ...
            {apuAvail
                && (
                    <text x={329} y={56} className="F34 Green LS2">AVAIL</text>
                )}
    ...
    return (
        <>
            ...
            <Layer x={x} y={y}>
                ...
                <text x={25} y={27} className="F22 White LS1">BLEED</text>
...
const NGauge = ({ x, y }: ComponentPositionProps) => {
    ...
    return (
        <>
            <Layer x={x} y={y}>
                <Layer x={0} y={-1}>
                    <GaugeComponent x={0} y={0} radius={GAUGE_RADIUS} startAngle={GAUGE_START} endAngle={GAUGE_END} visible className="SW3 White NoFill">
                        <GaugeComponent x={0} y={0} radius={GAUGE_RADIUS - 1} startAngle={GAUGE_END - 21} endAngle={GAUGE_END} visible className="SW6 Red NoFill">
                            {/* 0 */}
                ...
                <text x={104} y={18} className="F26 LS1 White MiddleAlign">N1</text>
                <text x={101} y={57} className="F22 Cyan MiddleAlign">%</text>
                <text x={104} y={97} className="F26 LS1 White MiddleAlign">N2</text>
...
const EgtGauge = ({ x, y }: ComponentPositionProps) => {
    ...
    return (
        <>
            <Layer x={x} y={y}>
                <Layer x={0} y={0}>
                    <GaugeComponent x={0} y={0} radius={GAUGE_RADIUS} startAngle={GAUGE_START - 10} endAngle={GAUGE_END} visible className="Line White NoFill ThickLine">
                        {/* 000 */}
                        <GaugeMarkerComponent
                            x={0}
                            y={0}
                ...
                <text x={77} y={29} className="F26 White LS1">EGT</text>
                <text x={100} y={60} className="F22 Cyan MiddleAlign">&deg;C</text>
                <text
                    x={83}
                    y={67}
                    className={`F35 LS1 EndAlign ${egtNumericalStyle}`}
                >
        ...
        <>
            ...
            <Layer x={x} y={y}>
                {lowFuelPressure.value
                    && <text className="Amber F27 LS2" x={0} y={0}>FUEL PRESS LO</text>}

                {apuFlapOpenPercentage >= 10 && (
                    <text
                        className={`Green F27 LS2 ${isIntakeIndicationFlashing ? 'FillPulse' : ''}`}
                        x={0}
                        y={70}
                    >
