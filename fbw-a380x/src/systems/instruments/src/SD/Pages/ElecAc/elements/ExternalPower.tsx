...
export const ExternalPower: FC<ExternalPowerProps> = ({ x, y, bus }) => {
    ...
    const extPwrFreqNormal = frequency > EXT_PWR_FREQUENCY_NORMAL_RANGE_LOWER && frequency < EXT_PWR_FREQUENCY_NORMAL_RANGE_UPPER;
    const extPwrVoltageNormal = potential > EXT_PWR_VOLTAGE_NORMAL_RANGE_LOWER && potential < EXT_PWR_VOLTAGE_NORMAL_RANGE_UPPER;
            ...
            <g id={`external-power-${bus}-indication`} transform={`translate(${x} ${y})`} className={onGround || extPwrConnected ? '' : 'Hide'}>
                ...
                <g className={extPwrConnected ? '' : 'Hide'}>
                    ...
                    <text
                        x={69}
                        y={38}
                        className={`F29 EndAlign ${extPwrFreqNormal ? 'Green' : 'Amber'} LS1`}
                    >
                    ...
                    <text
                        x={69}
                        y={65}
                        className={`F29 EndAlign ${extPwrVoltageNormal ? 'Green' : 'Amber'} LS1`}
                    >
                ...
                <Triangle x={bus <= 2 ? 40 : 36} y={-29} colour={lineContactorClosed ? 'Green' : 'White'} fill={0} orientation={0} scale={1} />
