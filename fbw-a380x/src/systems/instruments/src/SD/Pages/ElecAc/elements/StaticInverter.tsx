...
export const StaticInverter: FC<StaticInverterProps> = ({ x, y }) => {
    ...
    const statInvFreqNormal = frequency > STAT_INV_FREQUENCY_NORMAL_RANGE_LOWER && frequency < STAT_INV_FREQUENCY_NORMAL_RANGE_UPPER;
    const statInvVoltageNormal = potential > STAT_INV_VOLTAGE_NORMAL_RANGE_LOWER && potential < STAT_INV_VOLTAGE_NORMAL_RANGE_UPPER;
    ...
    if (statInvFailed) {
        triangleColor = 'Amber';
    } else if (staticInvertedDcCtorClosed && (!statInvFreqNormal || !statInvVoltageNormal || !staticInvertedToAcEmerCtorClosed)) {
        triangleColor = 'Amber';
        ...
        <g id="static-inverter-indication" transform={`translate(${x} ${y})`}>
            ...
            <g className={showStaticInvIndication ? '' : 'Hide'}>
                ...
                <text
                    x={110}
                    y={11}
                    className={`F29 EndAlign ${statInvFreqNormal ? 'Green' : 'Amber'} LS1`}
                >
                ...
                <text
                    x={110}
                    y={39}
                    className={`F29 EndAlign ${statInvVoltageNormal ? 'Green' : 'Amber'} LS1`}
                >
