...
export const TransformerRectifier: FC<TransformerRectifierProps> = ({ x, y, bus }) => {
    ...
    return (
        <g id={`tr-${trName}-indication`} transform={`translate(${x} ${y})`}>
            <text x={doubleLine ? 2 : 21} y={doubleLine ? -35 : -11} className={`F26 ${trFault || !acBusPowered ? 'Amber' : 'White'} LS1 MiddleAlign`}>
                {trName}
            </text>

            <text x={doubleLine ? 1 : -11} y={-11} className={`F26 ${trFault || !acBusPowered ? 'Amber' : 'White'} LS1 MiddleAlign`}>
                TR
            </text>
