...
export const EngineGenerator: FC<EngineGeneratorProps> = ({ x, y, bus }) => {
    ...
    const engGenVoltageNormal = potential > ENG_GEN_VOLTAGE_NORMAL_RANGE_LOWER && potential < ENG_GEN_VOLTAGE_NORMAL_RANGE_UPPER;
    ...
    return (
        <g id={`engine-generator-${bus}-indication`} transform={`translate(${x} ${y})`}>
            ...
            <text
                x={67}
                y={62}
                className={`F29 EndAlign White LS1 ${genPushbuttonAuto ? 'Hide' : ''}`}
            >
                OFF
            </text>
