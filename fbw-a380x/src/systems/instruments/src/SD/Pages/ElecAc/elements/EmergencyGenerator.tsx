...
export const EmergencyGenerator: FC<EmergencyGeneratorProps> = ({ x, y }) => {
    ...
    const emerGenVoltageNormal = potential > EMER_GEN_VOLTAGE_NORMAL_RANGE_LOWER && potential < EMER_GEN_VOLTAGE_NORMAL_RANGE_UPPER;
        ...
        <g id='emer-gen-indication' transform={`translate(${x} ${y})`}>
            ...
            <g className={showRatIndication ? '' : 'Hide'}>
                ...
                <text
                    x={-38}
                    y={11}
                    className={`F29 EndAlign ${emerGenLoadNormal ? 'Green' : 'Amber'} LS1`}
                >
                    {Math.round(load)}
                </text>
                {/* Voltage */}
                <text
                    x={-38}
                    y={39}
                    className={`F29 EndAlign ${emerGenVoltageNormal ? 'Green' : 'Amber'} LS1`}
                >
                    {Math.round(potential)}
                </text>
                <text className='Cyan F22' x={-35} y={11}>%</text>
                <text className='Cyan F22' x={-35} y={39}>V</text>
            </g>
