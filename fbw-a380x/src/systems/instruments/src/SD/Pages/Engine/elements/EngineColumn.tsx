...
const EngineColumn: FC<Position & EngineNumber & IgnitionActive & EngineColumnProps> = ({ x, y, engine, ignition, anyEngineRunning }) => {
    ...
    const [fadecPowered] = useSimVar(`L:A32NX_OVHD_FADEC_${engine}`, 'bool', 500);

    const shouldBeDisplayed = ignition || anyEngineRunning || fadecPowered;
    ...
    return (
        <>
            ...
            {/* N2 */}
            <DecimalValues x={x} y={y} value={N2} active={shouldBeDisplayed} />
            ...
            <DecimalValues x={x} y={y + 38} value={N3} active={shouldBeDisplayed} />
            ...
            {/* OIL */}
            <OilQuantityGauge x={x} y={y + 206} engine={engine} active={shouldBeDisplayed} value={oilQuantity} />
            <DecimalValues x={x} y={y + 206} value={oilQuantity} active={shouldBeDisplayed} />
            {!shouldBeDisplayed && (
            ...
            {shouldBeDisplayed && (
                <text
                    x={x + 20}
                    y={y + 248}
                    className={`${engineOilTemperature > 177 ? 'Amber' : 'Green'} EndAlign F29`}
                >
            ...
            {/* Oil Pressure */}
            <OilPressureGauge x={x} y={y + 320} engine={engine} active={shouldBeDisplayed} />
            {/* VIB N1 */}
            <DecimalValues x={x} y={y + 380} value={n1Vibration} active={shouldBeDisplayed} shift={-14} />
            <path className='White SW2' d={`M${engine > 2 ? x - 96 : x + 64},${y + 370} l 26, 0`} />
            {/* VIB N2 */}
            <DecimalValues x={x} y={y + 416} value={n2Vibration} active={shouldBeDisplayed} shift={-14} />
            <path className='White SW2' d={`M${engine > 2 ? x - 96 : x + 64},${y + 406} l 26, 0`} />
            {/* VIB N3 */}
            <DecimalValues x={x} y={y + 450} value={n3Vibration} active={shouldBeDisplayed} shift={-14} />
            <path className='White SW2' d={`M${engine > 2 ? x - 96 : x + 64},${y + 440} l 26, 0`} />

            {/* NAC / Ignition */}
            {(starting || ignition)
                && <StartValve x={x} y={y + 536} engine={engine} />}
            {!starterValveOpen && !ignition
                && <NacelleTemperatureGauge x={x} y={y + 536} engine={engine} active={shouldBeDisplayed} value={240} />}
