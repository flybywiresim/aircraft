...
const StartValve: React.FC<Position & EngineNumber> = ({ x, y, engine }) => {
    ...
    return (
        <g id={`SD-start-valve-${engine}`}>
            ...
            <text x={x - 10} y={y - 60} className={`Green F25 MiddleAlign ${!(showIgniter && startValveOpen) && 'Hide'}`}>A</text>
            <text x={x} y={y + 38} className={`F29 MiddleAlign ${starterInletPressure < 15 || starterInletPressure > 60 ? 'Amber' : 'Green'}`}>{Math.round(starterInletPressure)}</text>
            <path className="SW2 Green" d={`M${x},${y + 11} l 0,10`} />
            {startValveOpen && <path className="SW2 Green" d={`M${x},${y - 37} l 0,-10`} />}
            {engine === 2
            && (
                <>
                    <text x={x + 180} y={y - 50} className="F25 EndAlign White">IGN</text>
                    <text x={x + 180} y={y + 44} className="F25 EndAlign Cyan">PSI</text>
                </>
            )}
        </g>
