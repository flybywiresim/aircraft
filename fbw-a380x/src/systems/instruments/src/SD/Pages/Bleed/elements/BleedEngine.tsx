...
const BleedEngine: React.FC<BleedPageProps> = ({ x, y, engine, sdacDatum }) => {
    ...
    const [packFlowValveOpen] = useSimVar(`L:A32NX_COND_PACK_${packIndex}_FLOW_VALVE_${packFlowValveIndex}_IS_OPEN`, 'bool', 500);
    const [packFlowValveRate] = useSimVar(`L:A32NX_PNEU_PACK_${packIndex}_FLOW_VALVE_${packFlowValveIndex}_FLOW_RATE`, 'number', 100);
    ...
    return (
        <>
            ...
            <Valve
                ...
                sdacDatum={sdacDatum}
            />
            <path className={`SW2 ${shouldBleedValveBeOpen === engineBleedValveOpen && (engineHpValveOpen || isEngineRunning) ? 'Green' : 'Amber'}`} d={`M ${x + 29},${y - 10} l 0,46`} />
