...
const BleedHotAir: FC<BleedHotAirProps> = ({ x, y, hotAir, sdacDatum }) => {
    ...
    return (
        <g id={`HotAir-${hotAir}`}>
            <path className={`${anyPackValveOpen ? 'Green' : 'Amber'} Line`} d={`M${x},${y} l ${hotAir === 2 ? '-' : ''}${xoffset},0 l 0,-30`} />
            <path className={`${(hotAirValveOpen && anyPackValveOpen) ? 'Green' : 'Amber'} Line`} d={`M${hotAir === 1 ? x + xoffset : x - xoffset},${y - 68} l 0,-24`} />
            <Triangle x={hotAir === 1 ? x + xoffset : x - xoffset} y={y - 108} colour={(hotAirValveOpen && anyPackValveOpen) ? 'Green' : 'Amber'} fill={0} orientation={0} scale={1.2} />
            <Valve x={hotAir === 1 ? x + xoffset : x - xoffset} y={y - 49} radius={19} css={`SW2 ${(hotAirValveOpen && anyPackValveOpen) ? 'Green' : 'Amber'}`} position={hotAirValveOpen ? 'V' : 'H'} sdacDatum />
