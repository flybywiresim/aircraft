...
export const Battery: FC<BatteryProps> = ({ x, y, bus }) => {
    ...
    if (bus !== DcElecBus.DcApu) {
        ...
        batVoltageNormal = potential >= BAT_VOLTAGE_NORMAL_RANGE_LOADED && (potential > BAT_VOLTAGE_NORMAL_RANGE_UNLOADED || current <= 0);
    } else {
        ...
        <g id={`bat-${bus}-indication-box`} transform={`translate(${x} ${y})`}>
            ...
            <g className={batPbAuto ? 'Show' : 'Hide'}>
                ...
                <text
                    x={73}
                    y={62}
                    className={`F28 EndAlign ${batVoltageNormal ? 'Green' : 'Amber'} LS1`}
                >
                ...
                <text
                    x={73}
                    y={97}
                    className={`F28 EndAlign ${batCurrentNormal ? 'Green' : 'Amber'} LS1`}
                >
                    {Math.round(current)}
                </text>
            </g>
            <text className={`White F25 ${!batPbAuto ? 'Show' : 'Hide'}`} x={44} y={75}>OFF</text>
        </g>
    );
};
