    ...
    return (
        <g id={`tr-${bus}-indication`} transform={`translate(${x} ${y})`}>
            ...
            <text
                x={65}
                y={62}
                className={`F28 EndAlign ${trVoltageNormal ? 'Green' : 'Amber'} LS1`}
            >
            ...
            <text
                x={65}
                y={97}
                className={`F28 EndAlign ${trCurrentNormal ? 'Green' : 'Amber'} LS1`}
            >
                {Math.round(current)}
            </text>
            <g>
                <Triangle x={56} y={112} colour={acBusPowered ? 'White' : 'Amber'} fill={0} orientation={0} scale={1} />
                <text className={`F25 ${acBusPowered ? 'White' : 'Amber'} MiddleAlign LS1 WS-8`} x={58} y={144}>{acBusTitle}</text>
            </g>
        </g>
    );
};
