...
export const RudderTrim: FC<RudderTrimProps> = ({ x, y }) => {
    ...
    return (
        <g id='rudder-trim' transform={`translate(${x} ${y})`}>
            <text x={70} y={-8} className='F22 White LS1'>RUD TRIM</text>
            ...
            <g visibility={deflectionInfoValid ? 'visible' : 'hidden'}>
                ...
                <path d='m0,0 l6,8 l-6,8 l-6,-8 z' className={`${powerAvailableClass} Fill`} transform={`translate(${deflectionXValue} 0)`} />
                ...
                <text x={111} y={17} className={`${powerAvailableClass} F26 EndAlign`}>{pitchIntegral}</text>
                <text x={110} y={17} className={`${powerAvailableClass} F26`}>.</text>
                <text x={128} y={17} className={`${powerAvailableClass} F26`}>{pitchFractional}</text>
                <text x={159} y={19} className='Cyan F26'>°</text>
            </g>

            <text
                x={94}
                y={17}
                visibility={!deflectionInfoValid ? 'visible' : 'hidden'}
                className='Amber F26'
            >
                XX
            </text>
