...
const CabinTemperatures: React.FC<Position> = ({ x, y }) => {
    ...
    return (
        <>
            <text x={x} y={y + 82} className="F26 Green">{cockpitCabinTemp.toFixed(0)}</text>
            <text x={x + 321} y={y} className="F26 Green">{minUpperDeckTemp.toFixed(0)}</text>
            <text x={x + 433} y={y} className="F26 Green">{maxUpperDeckTemp.toFixed(0)}</text>
            <text x={x + 308} y={y + 97} className="F26 Green">{minMainDeckTemp.toFixed(0)}</text>
            <text x={x + 417} y={y + 97} className="F26 Green">{maxMainDeckTemp.toFixed(0)}</text>
...
const TemperatureController: React.FC<Position> = ({ x, y }) => {
    ...
    return (
        <g id="TemperatureControl" className={noFailure ? 'Hide' : 'Show'}>
            <text x={x} y={y} className="White F23 MiddleAlign">TEMP CTL</text>
            <text x={x - 20} y={y + 25} className={`${taddChannel1Failure ? 'Amber' : 'Green'} F23 MiddleAlign`}>1</text>
            <text x={x + 20} y={y + 25} className={`${taddChannel2Failure ? 'Amber' : 'Green'} F23 MiddleAlign`}>2</text>
    ...
    return (
        <g id={`CabinZoneWarning-${zone}`}>
            <g id={`TrimAirFailure-${zone}`} className={trimAirFailure ? 'Show' : 'Hide'} >
                <text x={x} y={y} className="Amber F22">H</text>
                <Triangle x={x+25} y={y} colour='Amber' fill={0} orientation={180} scale={1} />
            </g>
            <text x={x-12} y={y+25} className={`Amber F22 ${ductOverheat ? 'Show' : 'Hide'}`}>OVHT</text>
        </g>
    )
}
