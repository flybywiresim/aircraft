...
const AvionicsVentilation: React.FC<Position> = ({ x, y }) => {
    ...
    return (
        <g id="AvionicsVentilation">
            <text x={x} y={y} className="F23 Green">VENT</text>
...
const VentilationController: React.FC<VentilationControllerProps> = ({ x, y, vcm }) => {
    ...
    return (
        <g id={`VentilationControl-${vcm}`} className={noFailure ? 'Hide' : 'Show'}>
            <text x={x} y={y - 25} className="White F23 MiddleAlign">{vcm}</text>
            <text x={x} y={y} className="White F23 MiddleAlign">VENT CTL</text>
            <text x={x - 20} y={y + 25} className={`${vcmChannel1Failure ? 'Amber' : 'Green'} F23 MiddleAlign`}>1</text>
            <text x={x + 20} y={y + 25} className={`${vcmChannel2Failure ? 'Amber' : 'Green'} F23 MiddleAlign`}>2</text>
