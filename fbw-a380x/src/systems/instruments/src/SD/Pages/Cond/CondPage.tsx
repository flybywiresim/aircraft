...
export const CondPage = () => {
    ...
    const fwdIsolationValveOpen = vcsDiscreteWordToUse.getBitValueOr(14, false);
    const bulkIsolationValveOpen = vcsDiscreteWordToUse.getBitValueOr(16, false);
    ...
    return (
        <>
            ...
            <text className="F24 Cyan" x={100} y={34}>{unit === '1' ? '°C' : '°F'}</text>
            ...
            <ExtractValve x={248} y={408} value={fwdIsolationValveOpen} min={0} max={3} radius={42} css={fwdIsolationValveOpen ? 'Green Line' : 'Amber Line'} circleCss={`${fwdIsolationValveOpen ? 'Green' : 'Amber'} SW3 BackgroundFill`} />
            {/* Bulk Cargo Isol Valve */}
            <ExtractValve x={548} y={408} value={bulkIsolationValveOpen} min={0} max={3} radius={42} css={bulkIsolationValveOpen ? 'Green Line' : 'Amber Line'} circleCss={`${bulkIsolationValveOpen ? 'Green' : 'Amber'} SW3 BackgroundFill`} />
            {/* Cabin Air Extract Valve */}
            <ExtractValve x={724} y={97} value={cabinExtractValveOpen} min={0} max={1} radius={42} startAngle={172} endAngle={140} />
            {/* RAM Air 1 */}
