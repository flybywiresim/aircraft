...
const PositionIndicator: FC<PositionIndicatiorProps> = ({ system, position, type }) => {
    ...
    const [gearPos]: [number, (v: number) => void] = useSimVar(`L:A32NX_GEAR_${position === GearPosition.Nose ? 'CENTER' : position}_POSITION`, 'number', 500);
...
const GearDoor: FC<GearDoorProps> = ({ position, type }) => {
    // FIXME replace with LGCIU signals once implemented
    const [gearDoorPos]: [number, (v: number) => void] = useSimVar(`L:A32NX_GEAR_DOOR_${position === GearPosition.Nose ? 'CENTER' : position}_POSITION`, 'number', 500);
    ...
    return (
        <g className={`SW2 ${color} ${hide ? 'Hide' : ''}`}>
            ...
            {type === GearType.WLG && (
                <>
                    <path d={`m ${position === GearPosition.Left ? 83 : -16} -7 l ${position === GearPosition.Left ? '-' : ''}94 -5`}  transform={`rotate(${inTransit ? position === GearPosition.Left ? -55 : 55 : 0} ${position === GearPosition.Left ? 84 : -17} -7)`} />
