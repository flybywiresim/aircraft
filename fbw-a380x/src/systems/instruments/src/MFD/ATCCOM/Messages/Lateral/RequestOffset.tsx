...
export const RequestOffset: FC<MessageVisualizationProps> = ({ x = 0, y = 0, index, messageElements, onDelete }) => {
    const updateValue = (value: string, boxIndex: number): boolean => {
        if (value === '') {
            if (boxIndex === 0 && (messageElements[index].message?.TypeId === 'DM16' || messageElements[index].message?.TypeId === 'DM16')) {
        ...
        if (boxIndex === 0) {
            ...
            if (status === AtsuStatusCodes.Ok) {
                ...
                } else if (messageElements[index].message?.TypeId === 'DM16' || messageElements[index].message?.TypeId === 'DM16') {
    ...
    return (
        <MessageElement x={x} y={y} onDelete={onDelete}>
            <Layer>
                <text x={10} fontSize={22} fill="white">REQUEST OFFSET</text>
                <TextBox
                    x={215}
                    y={-28}
                    maxLength={3}
            ...
            <Layer>
                <text x={10} fontSize={22} fill="white">OF ROUTE</text>
                <text x={175} fontSize={22} fill="white">AT</text>
                <TextBox
                    x={215}
                    y={-28}
                    placeholder=" "
            ...
            <Layer>
                <text x={208} fontSize={22} fill="white">(POSITION OR TIME)</text>
            </Layer>
        </MessageElement>
    );
};
