...
export const RequestCruiseClimb: FC<MessageVisualizationProps> = ({ x = 0, y = 0, index, messageElements, onDelete }) => {
    const updateValue = (value: string): boolean => {
        ...
        if (status === AtsuStatusCodes.Ok) {
            messageElements[index].message = CpdlcMessagesDownlink.DM8[1].deepCopy();
    ...
    return (
        <MessageElement x={x} y={y} onDelete={onDelete}>
            <Layer>
                <text x={93} fontSize={22} fill="white">REQUEST</text>
            </Layer>
            <Layer>
