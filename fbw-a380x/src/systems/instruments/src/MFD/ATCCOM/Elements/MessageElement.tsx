...
export const MessageElement: FC<MessageElementProps> = ({ x = 0, y = 0, width = 572, drawSeperator = true, onDelete, children }) => (
    <Layer x={x} y={y}>
        <TrashbinButton x={width - 45} y={10} onClick={onDelete} />
        {Children.map(children, (child, index) => React.cloneElement(child, { x: 10, y: index * 45 + (index !== 0 ? 43 : 38) }))}
