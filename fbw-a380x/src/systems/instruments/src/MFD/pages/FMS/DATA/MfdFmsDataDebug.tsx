...
export class MfdFmsDataDebug extends FmsPage<MfdFmsDataDebugProps> {
    ...
    protected onNewData() {
        ...
        this.tab1lineValues[0].set(`${this.props.fmcService.master?.fmgc.getClimbSpeedLimit().speed.toFixed(2)} U${this.props.fmcService.master?.fmgc.getClimbSpeedLimit().underAltitude.toFixed(0)}` ?? '');
        ...
        this.tab1lineValues[12].set((SimVar.GetSimVarValue('L:A32NX_SPEEDS_MANAGED_PFD', 'number') as number).toFixed(2) ?? '');

        this.tab1lineLabels[13].set('A32NX_SPEEDS_MANAGED_ATHR');
        this.tab1lineValues[13].set((SimVar.GetSimVarValue('L:A32NX_SPEEDS_MANAGED_ATHR', 'number') as number).toFixed(2) ?? '');

        this.tab1lineLabels[14].set('A32NX_PFD_LOWER_SPEED_MARGIN');
        this.tab1lineValues[14].set((SimVar.GetSimVarValue('L:A32NX_PFD_LOWER_SPEED_MARGIN', 'number') as number).toFixed(2) ?? '');

        this.tab1lineLabels[15].set('A32NX_PFD_UPPER_SPEED_MARGIN');
        this.tab1lineValues[15].set((SimVar.GetSimVarValue('L:A32NX_PFD_UPPER_SPEED_MARGIN', 'number') as number).toFixed(2) ?? '');
    }
                        ...
                        <TopTabNavigatorPage>
                            ...
                            {this.tab1lineLabels.map((_, idx) => (
                                <div style="margin-bottom: 10px;">
                                    <span class="mfd-label" style="margin-right: 25px;">{this.tab1lineLabels[idx]}</span>
