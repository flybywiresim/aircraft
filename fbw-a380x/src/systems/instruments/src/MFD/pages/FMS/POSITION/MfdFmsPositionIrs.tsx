...
export class MfdFmsPositionIrs extends FmsPage<MfdFmsPositionIrsProps> {
    ...
    public onAfterRender(node: VNode): void {
        ...
        this.subs.push(this.irsAreAligned.sub((v) => {
            if (v === true) {
        ...
        this.subs.push(this.ir1MaintWord.sub((v) => this.setIrsStatusColumns(1, v, this.irs1Status, this.irs1SecondColumn, this.irs1ThirdColumn), true));
    ...
    private updateIrsData() {
        ...
        if (ir !== IrsDataFor.NONE) {
            ...
            this.irsDataTrueWindDirection.set(Arinc429Word.fromSimVarValue(`L:A32NX_ADIRS_IR_${ir}_WIND_DIRECTION`).value.toFixed(1));
            this.irsDataTrueWindSpeed.set(`/${Arinc429Word.fromSimVarValue(`L:A32NX_ADIRS_IR_${ir}_WIND_SPEED`).value.toFixed(1)}`);
    ...
    private setIrsStatusColumns(ir: number, v: Arinc429Register, first: Subject<IrsStatus>, second: Subject<string>, third: Subject<string>) {
