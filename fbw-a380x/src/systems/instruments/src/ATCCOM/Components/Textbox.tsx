...
export const TextBox: FC<TextBoxProps> = ({
    x, y, width = 50, height = 41, maxLength = 8, disabled, disabledBackgroundColor, defaultValue, onSubmit, prefix, midfix, midfixPosition, suffix,
    fixFontSize = 24, hideFixesIfEmpty, autoFilled, placeholder, placeholderTextColor = 'orange', textColor, textFontSize, textAnchor, resetValueIfDisabled,
}) => {
    ...
    return (
        <Layer x={x} y={y} {...hoverProps} onClick={focus}>
            ...
            {!textAnchor && fixesVisible ? (
                <>
                    <text x={5} y={height / 2 + 1} dominantBaseline="central" fill="#3e54e6" fontSize={fixFontSize} ref={prefixRef}>{prefix}</text>
                    <text x={width - 5} y={height / 2 + 1} dominantBaseline="central" fill="#3e54e6" fontSize={fixFontSize} textAnchor="end" ref={suffixRef}>{suffix}</text>
                </>
            ) : <></>}

            {isCursorVisible && value && focused && <rect x={flashingBoxX + 3.5} y={6} width={textBbox?.width! + 1.5} height={height - 12} fill="cyan" />}
            <text
            ...
            >
                {textAnchor && fixesVisible && !disabled ? <tspan fontSize={fixFontSize} dy={1} fill="#3e54e6" dominantBaseline="central">{prefix}</tspan> : <></>}
                {midfix && midfixPosition && fixesVisible ? (
                    <>
                        {preMidfixText.length !== 0 && (
                            <>
                                {preMidfixText}
                                {!disabled && <tspan fontSize={fixFontSize} dy={1} fill="#3e54e6" dominantBaseline="central">{midfix}</tspan>}
                                {postMidfixText}
                            </>
                        )}
                        {preMidfixText.length === 0 && (
                    ...
                    <>
                        ...
                        <tspan fill={isCursorVisible && focused ? 'white' : ''} dominantBaseline="central">{value[value.length - 1]}</tspan>
                    </>
                )}
                {textAnchor && fixesVisible && !disabled ? <tspan fontSize={fixFontSize} dy={1} fill="#3e54e6" dominantBaseline="central">{suffix}</tspan> : <></>}
            </text>

            <text ref={textRef} fontSize={textFontSize || (autoFilled ? 23 : 29)} fill="transparent">{value[value.length - 1]}</text>
        </Layer>
    );
};
