...
export const Checkbox: FC<CheckboxProps> = ({ x, y = 0, strokeWidth = 2, buttonWidth = 10, verticalSpacing = 40, textSpacing = 40, uniqueSelection, disabled = false, children }) => (
    <Layer x={x} y={y}>
...
export const CheckboxItem: FC<CheckboxItemProps> = ({
    ...
    return (
        ...
        >
            {uniqueSelection ? (
                <>
                    <circle {...hoverRef} cx={x + buttonWidth / 2} cy={y + buttonWidth / 2} r={buttonWidth} stroke={(hovered && !disabled) ? 'cyan' : 'white'} strokeWidth={1} />
                    {selected && <circle {...hoverRef} cx={x + buttonWidth / 2} cy={y + buttonWidth / 2} r={buttonWidth - strokeWidth} fill="cyan" />}
                    <text x={x + textSpacing} y={y + buttonWidth * 1.5 + 1} {...hoverRef} fill={!disabled ? 'white' : '#ababab'} fontSize={21}>{children}</text>
                </>
            ) : (
                <>
                    <rect
                    ...
                    <rect {...hoverRef} x={x} y={y} width={buttonWidth} height={buttonWidth} stroke="white" strokeWidth={1} fill="transparent" />
                    <text
                        ref={textRef}
