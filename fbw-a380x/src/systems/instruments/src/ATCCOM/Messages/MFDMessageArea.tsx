...
export const MFDMessageArea: FC<MFDMessageAreaProps> = ({ path }) => {
    ...
    if (mfdMessageManager.typeIMessage() !== undefined) {
        ...
        if (messageUid !== mfdMessageManager.typeIMessage()?.uid) setMessageUid(mfdMessageManager.typeIMessage()?.uid || -1);
    ...
    return (
        <Layer x={0} y={950}>
            ...
            {message.length >= 1 && <text fontSize={29} fill="black" x={109} y={29} ref={firstLineRef}>{message[0]}</text>}
            {message.length > 1 && <text fontSize={29} fill="black" x={109} y={61} ref={secondLineRef}>{message[1]}</text>}
        </Layer>
    );
};
