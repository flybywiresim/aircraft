...
export const WhenCanWeExpectCruiseClimb: FC<MessageVisualizationProps> = ({ x = 0, y = 0, index, messageElements, onDelete }) => {
    const updateValue = (value: string): boolean => {
        ...
        if (status === AtsuStatusCodes.Ok) {
            messageElements[index].message = CpdlcMessagesDownlink.DM54[1].deepCopy();
    ...
    return (
        <MessageElement x={x} y={y} onDelete={onDelete}>
            ...
            <Layer>
                <text x={10} fontSize={22} fill="white">CRUISE CLB TO</text>
                <TextBox
                    x={200}
                    y={-28}
                    maxLength={5}
