...
export const WhenCanWeExpectClimb: FC<MessageVisualizationProps> = ({ x = 0, y = 0, index, messageElements, onDelete }) => {
    const updateValue = (value: string): boolean => {
        ...
        if (status === AtsuStatusCodes.Ok) {
            messageElements[index].message = CpdlcMessagesDownlink.DM87[1].deepCopy();
    ...
    return (
        <MessageElement x={x} y={y} onDelete={onDelete}>
            ...
            <Layer>
                <text x={10} fontSize={22} fill="white">CLB TO</text>
                <TextBox
                    x={110}
                    y={-28}
                    maxLength={5}
