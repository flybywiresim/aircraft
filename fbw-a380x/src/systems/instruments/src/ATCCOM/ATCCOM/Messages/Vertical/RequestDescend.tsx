...
export const RequestDescend: FC<MessageVisualizationProps> = ({
  x = 0,
  y = 0,
  mode,
  index,
  ...
  const updateValue = (value: string, boxIndex: number): boolean => {
    ...
    if (
      ...
      (messageElements[index].message === undefined || messageElements[index].message?.TypeId !== 'DM12')
    ) {
      ...
      (messageElements[index].message === undefined || messageElements[index].message?.TypeId !== 'DM14')
    ) {
