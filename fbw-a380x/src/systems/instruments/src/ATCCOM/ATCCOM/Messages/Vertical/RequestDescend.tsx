...
export const RequestDescend: FC<MessageVisualizationProps> = ({ x = 0, y = 0, mode, index, messageElements, onDelete }) => {
    const updateValue = (value: string, boxIndex: number): boolean => {
        if (value === '') {
            if (boxIndex === 0) {
                if (messageElements[index].message?.TypeId === 'DM10') {
            ...
            } else if (messageElements[index].message?.TypeId !== 'DM10') {
                const newMessage = CpdlcMessagesDownlink.DM10[1].deepCopy();
        ...
        if (boxIndex === 0) {
            ...
            if (status === AtsuStatusCodes.Ok) {
                if (messageElements[index].message === undefined || messageElements[index].message?.TypeId === 'DM10') {
                    messageElements[index].message = CpdlcMessagesDownlink.DM10[1].deepCopy();
        ...
        if (validWaypoint === AtsuStatusCodes.Ok && (messageElements[index].message === undefined || messageElements[index].message?.TypeId !== 'DM12')) {
            const newMessage = CpdlcMessagesDownlink.DM12[1].deepCopy();
            newMessage.Content[1].Value = messageElements[index].message.Content[0].Value;
            messageElements[index].message = newMessage;
        } else if (validTime === AtsuStatusCodes.Ok && (messageElements[index].message === undefined || messageElements[index].message?.TypeId !== 'DM14')) {
            const newMessage = CpdlcMessagesDownlink.DM14[1].deepCopy();
            newMessage.Content[1].Value = messageElements[index].message.Content[0].Value;
            messageElements[index].message = newMessage;
    ...
    return (
        <MessageElement x={x} y={y} onDelete={onDelete}>
            <Layer>
                <text x={10} fontSize={22} fill="white">REQUEST DES TO</text>
                <TextBox
                    x={220}
                    y={-28}
                    maxLength={5}
            ...
            {mode === FansMode.FansA && (
                <Layer>
                    <text x={176} fontSize={22} fill="white">AT</text>
                    <TextBox
                        x={220}
                        y={-28}
                        placeholder=" "
