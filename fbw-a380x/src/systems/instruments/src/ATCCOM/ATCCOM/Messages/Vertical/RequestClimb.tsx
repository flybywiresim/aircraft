...
export const RequestClimb: FC<MessageVisualizationProps> = ({ x = 0, y = 0, mode, index, messageElements, onDelete }) => {
    const updateValue = (value: string, boxIndex: number): boolean => {
        ...
        if (validWaypoint === AtsuStatusCodes.Ok && (messageElements[index].message === undefined || messageElements[index].message?.TypeId !== 'DM11')) {
            ...
            messageElements[index].message = newMessage;
        } else if (validTime === AtsuStatusCodes.Ok && (messageElements[index].message === undefined || messageElements[index].message?.TypeId !== 'DM13')) {
    ...
    return (
        <MessageElement x={x} y={y} onDelete={onDelete}>
            <Layer>
                <text x={10} fontSize={22} fill="white">REQUEST CLB TO</text>
                <TextBox
                    x={220}
                    y={-28}
                    maxLength={5}
