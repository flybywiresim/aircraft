...
export const RequestWeatherDeviation: FC<MessageVisualizationProps> = ({ x = 0, y = 0, index, messageElements, onDelete }) => {
    ...
    return (
        <MessageElement x={x} y={y} onDelete={onDelete}>
            <Layer>
                <text x={10} fontSize={22} fill="white">REQUEST WX DEVIATION</text>
            </Layer>
            <Layer>
                <text x={10} fontSize={22} fill="white">UP TO</text>
                <TextBox
                    x={100}
                    y={-28}
                    maxLength={3}
                ...
                />
                <text x={340} fontSize={22} fill="white">OF ROUTE</text>
            </Layer>
        </MessageElement>
    );
};
