...
export const RequestLevelBlock: FC<MessageVisualizationProps> = ({ x = 0, y = 0, index, messageElements, onDelete }) => {
    const updateValue = (value: string, boxIndex: number): boolean => {
        ...
        if (value === '') {
            ...
            if (messageElements[index].message !== undefined && messageElements[index].message?.Content[otherIndex].Value !== '') {
        ...
        if (otherIndex < boxIndex) {
            status = InputValidation.validateAltitudeRange(messageElements[index].message?.Content[otherIndex].Value || '', formattedValue);
        } else {
            status = InputValidation.validateAltitudeRange(formattedValue, messageElements[index].message?.Content[otherIndex].Value || '');
        }
        ...
        messageElements[index].readyToSend = messageElements[index].message?.Content[0].Value !== '' && messageElements[index].message?.Content[1].Value !== '';
    ...
    return (
        <MessageElement x={x} y={y} onDelete={onDelete}>
            <Layer>
                <text x={10} fontSize={22} fill="white">REQUEST BLOCK</text>
                <TextBox
                    x={205}
                    y={-28}
                    maxLength={5}
            ...
            <Layer>
                <text x={160} fontSize={22} fill="white">TO</text>
                <TextBox
                    x={205}
                    y={-28}
                    maxLength={5}
