...
export const Page: FC = () => {
    const [elements, setElements] = useState<{ id: string, message: CpdlcMessageElement | undefined, readyToSend: boolean }[]>([]);
    ...
    return (
        <Layer x={0} y={140}>
            {elements.map(
                (element: { id: string, message: CpdlcMessageElement | undefined, readyToSend: boolean }, elementIndex: number) => React.createElement(MessageTable[element.id].visualization, {
                    x: 0,
                    y: elementIndex * 147,
                    mode: FansMode.FansA,
                    index: elementIndex,
            ...
            {activeFansB
                ? <FansB x={573} y={0} elements={elements} blacklist={blacklistEntries} exchangelist={exchangeEntries} disableFreetext={disableFreetext} onSelect={elementSelect} />
                : <FansA x={573} y={0} elements={elements} blacklist={blacklistEntries} exchangelist={exchangeEntries} disableFreetext={disableFreetext} onSelect={elementSelect} />}
