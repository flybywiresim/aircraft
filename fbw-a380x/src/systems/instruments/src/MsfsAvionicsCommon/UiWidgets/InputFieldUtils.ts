 ...
 */
export function shouldRenderMandatoryPlaceholder(state: InputFieldDisplayState): boolean {
  return state.mandatory && !state.inactive;
}
 ...
 */
export function shouldApplyMandatoryClass(state: InputFieldDisplayState): boolean {
  return state.mandatory && !state.disabled && !state.inactive;
}
