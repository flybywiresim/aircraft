...
export class CdsDisplayUnit extends DisplayComponent<DisplayUnitProps> {
    ...
    */
    ...
    public update() {
        const potentiometer = SimVar.GetSimVarValue(`LIGHT POTENTIOMETER:${DisplayUnitToPotentiometer[this.props.displayUnitId]}`, 'percent over 100');
        const poweredByBus1 = SimVar.GetSimVarValue(`L:A32NX_ELEC_${DisplayUnitToDCBus[this.props.displayUnitId][0]}_BUS_IS_POWERED`, 'Bool');
        const poweredByBus2 = SimVar.GetSimVarValue(`L:A32NX_ELEC_${DisplayUnitToDCBus[this.props.displayUnitId][1]}_BUS_IS_POWERED`, 'Bool');
    ...
    updateState() {
        if (this.state !== DisplayUnitState.Off && this.failed) {
            ...
            this.state = DisplayUnitState.On;
            clearTimeout(this.timeOut);
        } else if (this.state === DisplayUnitState.Off && (this.brightness.get() !== 0 && this.powered.get() && !this.failed)) {
            this.state = DisplayUnitState.ThalesBootup;
            this.setTimer(0.25 + (Math.random() * 0.2));
        } else if (this.state === DisplayUnitState.ThalesBootup && (this.brightness.get() !== 0 && this.powered.get() && !this.failed)) {
            this.state = DisplayUnitState.Selftest;
            this.setTimer(parseInt(NXDataStore.get('CONFIG_SELF_TEST_TIME', '15')));
        } else if ((this.state === DisplayUnitState.Selftest || this.state === DisplayUnitState.ThalesBootup) && (this.brightness.get() === 0 || !this.powered.get())) {
            this.state = DisplayUnitState.Off;
            clearTimeout(this.timeOut);
        }
