...
export const LegacyCdsDisplayUnit = forwardRef<SVGSVGElement, PropsWithChildren<DisplayUnitProps>>(({ displayUnitId, failed, children }, ref) => {
    ...
    const [potentiometer] = useSimVar(`LIGHT POTENTIOMETER:${DisplayUnitToPotentiometer[displayUnitId]}`, 'percent over 100', 200);
    const [electricityState0] = useSimVar(`L:A32NX_ELEC_${DisplayUnitToDCBus[displayUnitId][0]}_BUS_IS_POWERED` /* TODO 380 simvar */, 'bool', 200);
    const [electricityState1] = useSimVar(`L:A32NX_ELEC_${DisplayUnitToDCBus[displayUnitId][1]}_BUS_IS_POWERED` /* TODO 380 simvar */, 'bool', 200);
    const [homeCockpit] = useSimVar('L:A32NX_HOME_COCKPIT_ENABLED', 'bool', 200);

    useUpdate(useCallback((deltaTime) => {
    ...
    useEffect(() => {
        if (state !== DisplayUnitState.Off && failed) {
            setState(DisplayUnitState.Off);
        } else if (state === DisplayUnitState.On && (potentiometer === 0 || (electricityState0 === 0 && electricityState1 === 0))) {
            setState(DisplayUnitState.Standby);
            setTimer(10);
        } else if (state === DisplayUnitState.Standby && (potentiometer !== 0 && (electricityState0 !== 0 || electricityState1 !== 0))) {
            setState(DisplayUnitState.On);
            setTimer(null);
        } else if (state === DisplayUnitState.Off && (potentiometer !== 0 && (electricityState0 !== 0 || electricityState1 !== 0) && !failed)) {
        ...
        } else if ((state === DisplayUnitState.Selftest || state === DisplayUnitState.ThalesBootup) && (potentiometer === 0 || (electricityState0 === 0 && electricityState1 === 0))) {
            setState(DisplayUnitState.Off);
            setTimer(null);
        }
