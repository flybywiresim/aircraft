...
export const GaugeComponent: FC<GaugeComponentProps> = memo(({ x, y, radius, startAngle, endAngle, className, children, visible, sweep, largeArc }) => {
...
export const ThrottlePositionDonutComponent: FC<ThrottlePositionDonutComponentType> = memo(({
    ...
    reverse = false, outerMultiplier = 1.12, donutRadius = 4,
}) => {
    const dir = valueRadianAngleConverter({ value, min, max, endAngle, startAngle, perpendicular: reverse });
...
export const GaugeMaxEGTComponent: FC<GaugeMaxComponentType> = memo(({ value, x, y, min, max, radius, startAngle, endAngle, className }) => {
    const dir = valueRadianAngleConverter({ value, min, max, endAngle, startAngle });
    ...
    return (
        <>
            <path d={`M ${xy.x - 6},${xy.y} l 12,0 l 0,4 l -8,0 Z`} className={className} transform={`rotate(${dir.angle} ${xy.x} ${xy.y})`} />
...
export const GaugeThrustComponent: FC<GaugeThrustComponentProps> = memo(({ x, y, radius, min, max, startAngle, endAngle, className, visible, valueIdle, valueMax, reverse = false }) => {
