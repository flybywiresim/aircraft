#ifndef A380PrimComputerFg_h_
#define A380PrimComputerFg_h_
#include "rtwtypes.h"
#include "A380PrimComputerFg_types.h"
#include "A380FgOuterLoops.h"

class A380PrimComputerFg final
{
 public:
  struct rtDW_LeadLagFilter_A380PrimComputerFg_T {
    real_T pY;
    real_T pU;
    boolean_T pY_not_empty;
    boolean_T pU_not_empty;
  };

  struct rtDW_LagFilter_A380PrimComputerFg_T {
    real_T pY;
    real_T pU;
    boolean_T pY_not_empty;
    boolean_T pU_not_empty;
  };

  struct rtDW_LagFilter_A380PrimComputerFg_b_T {
    real32_T pY;
    real32_T pU;
    boolean_T pY_not_empty;
    boolean_T pU_not_empty;
  };

  struct rtDW_MATLABFunction_A380PrimComputerFg_k_T {
    boolean_T previousInput;
    boolean_T previousInput_not_empty;
  };

  struct rtDW_Subsystem_A380PrimComputerFg_T {
    boolean_T Memory_PreviousInput;
  };

  struct rtDW_MATLABFunction_A380PrimComputerFg_l_T {
    real_T remainingTriggerTime;
    boolean_T previousInput;
  };

  struct rtDW_MATLABFunction_A380PrimComputerFg_b_T {
    real_T timeSinceCondition;
    boolean_T output;
  };

  struct rtDW_SRFlipFlopwithSyncInput_A380PrimComputerFg_T {
    boolean_T Memory_PreviousInput;
  };

  struct rtDW_MATLABFunction1_A380PrimComputerFg_T {
    real_T eventTime;
    boolean_T eventTime_not_empty;
  };

  struct BlockIO_A380PrimComputerFg_T {
    prim_outputs BusAssignment_i;
    prim_outputs BusAssignment_ic;
    real_T u;
    real_T u_d;
    real_T u_l;
    real_T u_ly;
    real_T u_lyj;
    real_T u_lyjj;
    real32_T u_c;
    real32_T u_dg;
    real32_T u_lyjjl;
  };

  struct D_Work_A380PrimComputerFg_T {
    base_prim_out_bus Delay_DSTATE[2];
    base_prim_fg_laws_outputs Delay2_DSTATE;
    base_prim_ap_fd_logic_outputs Delay_DSTATE_c;
    real_T DelayInput1_DSTATE;
    real_T DelayInput1_DSTATE_j;
    real_T DelayInput1_DSTATE_d;
    real_T Delay_DSTATE_o;
    real_T Delay_DSTATE_i;
    real_T Delay_DSTATE_a;
    real_T vMemoEo;
    real_T vMemoGa;
    real_T pY;
    real_T pY_a;
    real_T pU;
    real32_T DelayOneStep_DSTATE;
    real32_T DelayInput1_DSTATE_i;
    real32_T DelayInput1_DSTATE_k;
    real32_T DelayInput1_DSTATE_f;
    real32_T pValue;
    real32_T pValue_e;
    real32_T pValue_i;
    real32_T pValue_p;
    real32_T pY_j;
    real32_T pU_i;
    boolean_T Delay_DSTATE_aw;
    boolean_T Delay_DSTATE_b;
    boolean_T DelayOneStep_DSTATE_a;
    boolean_T Delay_DSTATE_bq;
    boolean_T DelayOneStep1_DSTATE;
    boolean_T Delay_DSTATE_m;
    boolean_T Delay_DSTATE_me;
    boolean_T Delay1_DSTATE;
    boolean_T Delay2_DSTATE_n;
    boolean_T Delay3_DSTATE;
    boolean_T Delay_DSTATE_h;
    boolean_T Delay_DSTATE_p;
    boolean_T icLoad;
    boolean_T Memory_PreviousInput;
    boolean_T Memory_PreviousInput_g;
    boolean_T Memory_PreviousInput_n;
    boolean_T Memory_PreviousInput_o;
    boolean_T Memory_PreviousInput_h;
    boolean_T Memory_PreviousInput_ov;
    boolean_T Memory_PreviousInput_e;
    boolean_T Memory_PreviousInput_en;
    boolean_T Memory_PreviousInput_d;
    boolean_T Memory_PreviousInput_a;
    boolean_T Memory_PreviousInput_l;
    boolean_T Memory_PreviousInput_at;
    boolean_T Memory_PreviousInput_p;
    boolean_T Memory_PreviousInput_lm;
    boolean_T Memory_PreviousInput_o4;
    boolean_T Memory_PreviousInput_eu;
    boolean_T Memory_PreviousInput_k;
    boolean_T Memory_PreviousInput_f;
    boolean_T Memory_PreviousInput_ox;
    boolean_T Memory_PreviousInput_ek;
    boolean_T Memory_PreviousInput_l1;
    boolean_T Memory_PreviousInput_e0;
    boolean_T Memory_PreviousInput_hw;
    boolean_T Memory_PreviousInput_oz;
    boolean_T Memory_PreviousInput_og;
    boolean_T Memory_PreviousInput_oy;
    boolean_T Memory_PreviousInput_j;
    boolean_T Memory_PreviousInput_ei;
    boolean_T Memory_PreviousInput_kj;
    boolean_T Memory_PreviousInput_m;
    boolean_T Memory_PreviousInput_kc;
    boolean_T Memory_PreviousInput_i;
    boolean_T Memory_PreviousInput_il;
    boolean_T Memory_PreviousInput_lp;
    boolean_T Memory_PreviousInput_ae;
    boolean_T Memory_PreviousInput_ik;
    boolean_T Memory_PreviousInput_kx;
    boolean_T Memory_PreviousInput_b;
    boolean_T Memory_PreviousInput_pt;
    boolean_T Memory_PreviousInput_od;
    boolean_T Memory_PreviousInput_hx;
    boolean_T Memory_PreviousInput_py;
    boolean_T Memory_PreviousInput_dp;
    boolean_T Memory_PreviousInput_i5;
    boolean_T Memory_PreviousInput_ou;
    boolean_T Memory_PreviousInput_jm;
    boolean_T Memory_PreviousInput_hs;
    boolean_T Memory_PreviousInput_gd;
    boolean_T Memory_PreviousInput_oyc;
    boolean_T Memory_PreviousInput_g3;
    boolean_T icLoad_p;
    boolean_T vMemoEo_not_empty;
    boolean_T vMemoGa_not_empty;
    boolean_T pValue_not_empty;
    boolean_T prevTrkFpaActive;
    boolean_T prevTrkFpaActive_not_empty;
    boolean_T pValue_not_empty_n;
    boolean_T prevMachActive;
    boolean_T prevMachActive_not_empty;
    boolean_T pValue_not_empty_a;
    boolean_T prevTrkFpaActive_i;
    boolean_T prevTrkFpaActive_not_empty_n;
    boolean_T pValue_not_empty_j;
    boolean_T pY_not_empty;
    boolean_T pU_not_empty;
    boolean_T pY_not_empty_c;
    boolean_T pY_not_empty_g;
    boolean_T pU_not_empty_o;
    boolean_T Runtime_MODE;
    rtDW_LagFilter_A380PrimComputerFg_T sf_LagFilter_f;
    rtDW_LagFilter_A380PrimComputerFg_T sf_LagFilter_c;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_aqw;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_jqs;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_lk;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_cia;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_jfh;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_kdl;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_nvo;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_erf;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_j2;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_oks;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_lq;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_gg;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_n11;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_jo;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_ocv;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_kq;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_lwx;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_dm;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_ef;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_fk;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_j0;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_ok;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_lth;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_a0p;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_d4;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_ep5;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_dxb;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_llo;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_dth;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_gw;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_hq5;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_fhn;
    rtDW_MATLABFunction1_A380PrimComputerFg_T sf_MATLABFunction1_d;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_lis;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_bby;
    rtDW_MATLABFunction1_A380PrimComputerFg_T sf_MATLABFunction1_l;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_cbh;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_ckt;
    rtDW_MATLABFunction1_A380PrimComputerFg_T sf_MATLABFunction1_o;
    rtDW_Subsystem_A380PrimComputerFg_T Subsystem1;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_nv;
    rtDW_Subsystem_A380PrimComputerFg_T Subsystem4_c;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_g0v;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_h5;
    rtDW_Subsystem_A380PrimComputerFg_T Subsystem_c;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_kmp;
    rtDW_Subsystem_A380PrimComputerFg_T Subsystem3;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_du;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_g4;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_nc;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_kl;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_hp;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_av;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_oyv;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_fy;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_hw;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_pw;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_h4e;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_oz;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_oy;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_eow;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_f34;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_bii;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_ob;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_l0a;
    rtDW_LagFilter_A380PrimComputerFg_b_T sf_LagFilter_b4;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_l1;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_oh;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_cd0;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_cn;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_isd;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_oa;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_cm;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_kv;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_os;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_je;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_on;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_nab;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_d0;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_bc;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_ep;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_fb;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_ed;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_lx;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_mm;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_n4;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_dxv;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_b1;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_az;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_kdz;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_ak;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_is;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_n3;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_p4;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_jp;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_iy;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_dvt;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_jx;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_nm;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_aq;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_b2;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_oqd;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_g1d;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_hs;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_ab;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_bf;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_a0v;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_g13;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_my;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_ju;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_cd;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_ph;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_hv2;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_oe;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_ll;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_gm;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_na;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_c4;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_ks;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_eu;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_fre;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_lw;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_li;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_b4;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_oq;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_dv;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_a0;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_byg;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_eo;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_mn;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_ai;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_it;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_h0;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_pq0;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_cg;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_fcu;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_ld;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_ox;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_br;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_an;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_mva;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_g2i;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_k1;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_bq;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_o3;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_pl;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_bl;
    rtDW_Subsystem_A380PrimComputerFg_T Subsystem4;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_l0;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_bo;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_iv;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_dgp;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_ar;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_ps;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_kw;
    rtDW_SRFlipFlopwithSyncInput_A380PrimComputerFg_T SRFlipFlopwithSyncInput_d;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_dg;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_a5;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_fh;
    rtDW_SRFlipFlopwithSyncInput_A380PrimComputerFg_T SRFlipFlopwithSyncInput;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_eh;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_cs;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_mp;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_g2;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_gf;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_pv;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_cj;
    rtDW_MATLABFunction_A380PrimComputerFg_b_T sf_MATLABFunction_at;
    rtDW_MATLABFunction_A380PrimComputerFg_l_T sf_MATLABFunction_hf;
    rtDW_Subsystem_A380PrimComputerFg_T Subsystem_n;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_fr;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_bu;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_d;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_o5;
    rtDW_Subsystem_A380PrimComputerFg_T Subsystem;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_fp;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_a;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_pa;
    rtDW_MATLABFunction_A380PrimComputerFg_k_T sf_MATLABFunction_ci;
    rtDW_LagFilter_A380PrimComputerFg_b_T sf_LagFilter_b;
    rtDW_LagFilter_A380PrimComputerFg_T sf_LagFilter;
    rtDW_LeadLagFilter_A380PrimComputerFg_T sf_LeadLagFilter_c;
    rtDW_LeadLagFilter_A380PrimComputerFg_T sf_LeadLagFilter;
  };

  struct ExternalInputs_A380PrimComputerFg_T {
    prim_outputs in;
  };

  struct ExternalOutputs_A380PrimComputerFg_T {
    prim_outputs out;
  };

  struct rtP_Subsystem_A380PrimComputerFg_T {
    boolean_T Logic_table[16];
  } __attribute__((aligned(8)));

  struct rtP_SRFlipFlopwithSyncInput_A380PrimComputerFg_T {
    boolean_T Logic_table[16];
  } __attribute__((aligned(8)));

  struct rtP_AltitudeChoice_A380PrimComputerFg_T {
    real_T BitfromLabel1_bit;
  } __attribute__((aligned(8)));

  struct Parameters_A380PrimComputerFg_T {
    real_T ScheduledGain1_BreakpointsForDimension1[5];
    real_T ScheduledGain2_BreakpointsForDimension1[4];
    real_T ScheduledGain4_BreakpointsForDimension1[2];
    real_T LagFilter_C1;
    real_T LagFilter1_C1;
    real_T LagFilter1_C1_a;
    real_T WashoutFilter_C1;
    real_T HighPassFilter_C1;
    real_T LowPassFilter_C1;
    real_T LagFilter_C1_j;
    real_T LagFilter1_C1_n;
    real_T WashoutFilter_C1_o;
    real_T HighPassFilter_C2;
    real_T LowPassFilter_C2;
    real_T HighPassFilter_C3;
    real_T LowPassFilter_C3;
    real_T HighPassFilter_C4;
    real_T LowPassFilter_C4;
    real_T DiscreteDerivativeVariableTs_Gain;
    real_T DiscreteTimeIntegratorVariableTsLimit_Gain;
    real_T DiscreteDerivativeVariableTs_InitialCondition;
    real_T RateLimiterVariableTs_InitialCondition;
    real_T ScheduledGain1_Table[5];
    real_T ScheduledGain2_Table[4];
    real_T ScheduledGain4_Table[2];
    real_T BitfromLabel1_bit;
    real_T BitfromLabel1_bit_k;
    real_T BitfromLabel1_bit_d;
    real_T BitfromLabel6_bit;
    real_T BitfromLabel2_bit;
    real_T BitfromLabel3_bit;
    real_T BitfromLabel2_bit_d;
    real_T BitfromLabel1_bit_b;
    real_T BitfromLabel2_bit_dp;
    real_T BitfromLabel1_bit_l;
    real_T BitfromLabel6_bit_h;
    real_T BitfromLabel2_bit_k;
    real_T BitfromLabel7_bit;
    real_T BitfromLabel3_bit_d;
    real_T BitfromLabel8_bit;
    real_T BitfromLabel4_bit;
    real_T BitfromLabel5_bit;
    real_T BitfromLabel3_bit_o;
    real_T BitfromLabel1_bit_a;
    real_T BitfromLabel1_bit_c;
    real_T BitfromLabel2_bit_e;
    real_T BitfromLabel1_bit_cm;
    real_T BitfromLabel2_bit_j;
    real_T BitfromLabel3_bit_n;
    real_T BitfromLabel2_bit_b;
    real_T BitfromLabel1_bit_li;
    real_T BitfromLabel1_bit_i;
    real_T BitfromLabel2_bit_d3;
    real_T BitfromLabel3_bit_os;
    real_T BitfromLabel2_bit_jb;
    real_T BitfromLabel1_bit_j;
    real_T BitfromLabel3_bit_l;
    real_T BitfromLabel1_bit_g;
    real_T BitfromLabel1_bit_n;
    real_T BitfromLabel4_bit_i;
    real_T BitfromLabel1_bit_im;
    real_T BitfromLabel5_bit_b;
    real_T BitfromLabel3_bit_i;
    real_T BitfromLabel6_bit_b;
    real_T BitfromLabel2_bit_i;
    real_T BitfromLabel3_bit_j;
    real_T BitfromLabel2_bit_jq;
    real_T BitfromLabel1_bit_f;
    real_T BitfromLabel3_bit_k;
    real_T BitfromLabel3_bit_kl;
    real_T BitfromLabel1_bit_c3;
    real_T BitfromLabel3_bit_ky;
    real_T BitfromLabel3_bit_b;
    real_T BitfromLabel3_bit_g;
    real_T BitfromLabel3_bit_m;
    real_T BitfromLabel3_bit_e;
    real_T BitfromLabel4_bit_h;
    real_T BitfromLabel6_bit_o;
    real_T BitfromLabel3_bit_i1;
    real_T BitfromLabel1_bit_dx;
    real_T BitfromLabel4_bit_m;
    real_T BitfromLabel3_bit_im;
    real_T BitfromLabel2_bit_l;
    real_T BitfromLabel1_bit_e;
    real_T BitfromLabel3_bit_dp;
    real_T BitfromLabel3_bit_nd;
    real_T BitfromLabel2_bit_a;
    real_T BitfromLabel1_bit_h;
    real_T BitfromLabel3_bit_bw;
    real_T BitfromLabel3_bit_j1;
    real_T BitfromLabel3_bit_i4;
    real_T BitfromLabel4_bit_o;
    real_T BitfromLabel3_bit_mo;
    real_T BitfromLabel4_bit_b;
    real_T BitfromLabel3_bit_gq;
    real_T BitfromLabel3_bit_ib;
    real_T BitfromLabel3_bit_kr;
    real_T BitfromLabel3_bit_er;
    real_T BitfromLabel3_bit_a;
    real_T BitfromLabel1_bit_p;
    real_T BitfromLabel2_bit_es;
    real_T BitfromLabel1_bit_hz;
    real_T BitfromLabel2_bit_ji;
    real_T BitfromLabel2_bit_g;
    real_T BitfromLabel1_bit_la;
    real_T BitfromLabel1_bit_io;
    real_T BitfromLabel1_bit_lz;
    real_T BitfromLabel1_bit_m;
    real_T BitfromLabel1_bit_bb;
    real_T BitfromLabel3_bit_h;
    real_T BitfromLabel1_bit_mi;
    real_T CompareToConstant_const;
    real_T CompareToConstant_const_e;
    real_T CompareToConstant_const_f;
    real_T CompareToConstant_const_g;
    real_T CompareToConstant1_const;
    real_T CompareToConstant2_const;
    real_T CompareToConstant_const_d;
    real_T CompareToConstant1_const_i;
    real_T CompareToConstant2_const_h;
    real_T CompareToConstant_const_k;
    real_T CompareToConstant1_const_h;
    real_T CompareToConstant2_const_o;
    real_T CompareToConstant3_const;
    real_T CompareToConstant1_const_k;
    real_T CompareToConstant_const_fo;
    real_T CompareToConstant2_const_n;
    real_T CompareToConstant1_const_l;
    real_T CompareToConstant2_const_d;
    real_T CompareToConstant3_const_d;
    real_T CompareToConstant4_const;
    real_T CompareToConstant5_const;
    real_T CompareToConstant6_const;
    real_T CompareToConstant7_const;
    real_T CompareToConstant8_const;
    real_T CompareToConstant9_const;
    real_T CompareToConstant10_const;
    real_T CompareToConstant2_const_c;
    real_T CompareToConstant1_const_lr;
    real_T CompareToConstant2_const_m;
    real_T CompareToConstant1_const_ia;
    real_T CompareToConstant_const_h;
    real_T CompareToConstant_const_kb;
    real_T CompareToConstant_const_b;
    real_T CompareToConstant2_const_o4;
    real_T CompareToConstant1_const_c;
    real_T CompareToConstant_const_m;
    real_T CompareToConstant3_const_b;
    real_T CompareToConstant5_const_b;
    real_T CompareToConstant1_const_f;
    real_T CompareToConstant2_const_oh;
    real_T CompareToConstant_const_j;
    real_T CompareToConstant_const_ep;
    real_T CompareToConstant1_const_g;
    real_T CompareToConstant_const_n;
    real_T CompareToConstant1_const_n;
    real_T CompareToConstant1_const_m;
    real_T CompareToConstant_const_i;
    real_T CompareToConstant_const_jk;
    real_T CompareToConstant2_const_a;
    real_T CompareToConstant4_const_l;
    real_T CompareToConstant1_const_p;
    real_T CompareToConstant5_const_o;
    real_T CompareToConstant_const_n2;
    real_T CompareToConstant2_const_mi;
    real_T CompareToConstant_const_c;
    real_T CompareToConstant1_const_cg;
    real_T CompareToConstant2_const_ay;
    real_T CompareToConstant2_const_k;
    real_T CompareToConstant2_const_nt;
    real_T CompareToConstant_const_o;
    real_T CompareToConstant1_const_a;
    real_T CompareToConstant1_const_ai;
    real_T CompareToConstant2_const_cc;
    real_T CompareToConstant2_const_p;
    real_T CompareToConstant_const_il;
    real_T CompareToConstant_const_mv;
    real_T CompareToConstant1_const_cm;
    real_T CompareToConstant_const_de;
    real_T CompareToConstant2_const_j;
    real_T CompareToConstant3_const_g;
    real_T CompareToConstant4_const_h;
    real_T CompareToConstant5_const_a;
    real_T CompareToConstant_const_ki;
    real_T CompareToConstant2_const_ma;
    real_T CompareToConstant3_const_m;
    real_T CompareToConstant_const_iw;
    real_T CompareToConstant1_const_gt;
    real_T CompareToConstant10_const_h;
    real_T CompareToConstant11_const;
    real_T CompareToConstant1_const_b;
    real_T CompareToConstant2_const_f;
    real_T CompareToConstant8_const_k;
    real_T CompareToConstant9_const_d;
    real_T CompareToConstant3_const_i;
    real_T CompareToConstant4_const_c;
    real_T CompareToConstant_const_d4;
    real_T CompareToConstant_const_cm;
    real_T CompareToConstant1_const_h4;
    real_T MTrigNode_isRisingEdge;
    real_T MTrigNode_isRisingEdge_l;
    real_T MTrigNode1_isRisingEdge;
    real_T MTrigNode2_isRisingEdge;
    real_T MTrigNode1_isRisingEdge_m;
    real_T MTrigNode_isRisingEdge_h;
    real_T MTrigNode_isRisingEdge_c;
    real_T MTrigNode_isRisingEdge_f;
    real_T MTrigNode_isRisingEdge_p;
    real_T MTrigNode_isRisingEdge_n;
    real_T MTrigNode_isRisingEdge_d;
    real_T MTrigNode2_isRisingEdge_o;
    real_T MTrigNode_isRisingEdge_le;
    real_T MTrigNode1_isRisingEdge_n;
    real_T MTrigNode1_isRisingEdge_mw;
    real_T MTrigNode_isRisingEdge_k;
    real_T MTrigNode_isRisingEdge_kv;
    real_T APEngagedLogic_isSide2;
    real_T APEngagedLogic1_isSide2;
    real_T RateLimiterVariableTs_lo;
    real_T MTrigNode_retriggerable;
    real_T MTrigNode_retriggerable_b;
    real_T MTrigNode1_retriggerable;
    real_T MTrigNode2_retriggerable;
    real_T MTrigNode1_retriggerable_o;
    real_T MTrigNode_retriggerable_l;
    real_T MTrigNode_retriggerable_c;
    real_T MTrigNode_retriggerable_g;
    real_T MTrigNode_retriggerable_k;
    real_T MTrigNode_retriggerable_d;
    real_T MTrigNode_retriggerable_m;
    real_T MTrigNode2_retriggerable_j;
    real_T MTrigNode_retriggerable_md;
    real_T MTrigNode1_retriggerable_d;
    real_T MTrigNode1_retriggerable_o0;
    real_T MTrigNode_retriggerable_cq;
    real_T MTrigNode_retriggerable_ci;
    real_T ConfirmNode1_timeDelay;
    real_T ConfirmNode1_timeDelay_l;
    real_T ConfirmNode2_timeDelay;
    real_T ConfirmNode3_timeDelay;
    real_T ConfirmNode_timeDelay;
    real_T ConfirmNode_timeDelay_d;
    real_T ConfirmNode_timeDelay_g;
    real_T ConfirmNode1_timeDelay_o;
    real_T ConfirmNode_timeDelay_m;
    real_T ConfirmNode_timeDelay_dd;
    real_T ConfirmNode_timeDelay_p;
    real_T ConfirmNode_timeDelay_df;
    real_T ConfirmNode_timeDelay_j;
    real_T ConfirmNode1_timeDelay_ll;
    real_T ConfirmNode_timeDelay_gg;
    real_T ConfirmNode2_timeDelay_i;
    real_T ConfirmNode1_timeDelay_f;
    real_T ConfirmNode1_timeDelay_l3;
    real_T ConfirmNode_timeDelay_h;
    real_T ConfirmNode_timeDelay_k;
    real_T ConfirmNode_timeDelay_a;
    real_T ConfirmNode_timeDelay_k1;
    real_T ConfirmNode_timeDelay_ax;
    real_T ConfirmNode_timeDelay_c;
    real_T ConfirmNode_timeDelay_kx;
    real_T ConfirmNode_timeDelay_i;
    real_T ConfirmNode_timeDelay_dw;
    real_T ConfirmNode_timeDelay_cd;
    real_T ConfirmNode_timeDelay_kp;
    real_T ConfirmNode_timeDelay_b;
    real_T ConfirmNode_timeDelay_f;
    real_T ConfirmNode_timeDelay_o;
    real_T ConfirmNode_timeDelay_e;
    real_T ConfirmNode_timeDelay_ol;
    real_T ConfirmNode_timeDelay_g5;
    real_T ConfirmNode_timeDelay_iw;
    real_T ConfirmNode_timeDelay_mw;
    real_T ConfirmNode_timeDelay_js;
    real_T ConfirmNode_timeDelay_dt;
    real_T ConfirmNode_timeDelay_mc;
    real_T ConfirmNode_timeDelay_dm;
    real_T ConfirmNode_timeDelay_fa;
    real_T ConfirmNode_timeDelay_l;
    real_T ConfirmNode1_timeDelay_m;
    real_T ConfirmNode_timeDelay_e4;
    real_T ConfirmNode_timeDelay_as;
    real_T ConfirmNode1_timeDelay_b;
    real_T ConfirmNode_timeDelay_ki;
    real_T ConfirmNode1_timeDelay_n;
    real_T ConfirmNode2_timeDelay_j;
    real_T ConfirmNode_timeDelay_aq;
    real_T ConfirmNode_timeDelay_pt;
    real_T ConfirmNode_timeDelay_pz;
    real_T ConfirmNode_timeDelay_el;
    real_T ConfirmNode_timeDelay_jv;
    real_T MTrigNode_triggerDuration;
    real_T MTrigNode_triggerDuration_a;
    real_T MTrigNode1_triggerDuration;
    real_T MTrigNode2_triggerDuration;
    real_T MTrigNode1_triggerDuration_f;
    real_T MTrigNode_triggerDuration_j;
    real_T MTrigNode_triggerDuration_n;
    real_T MTrigNode_triggerDuration_jn;
    real_T MTrigNode_triggerDuration_l;
    real_T MTrigNode_triggerDuration_d;
    real_T MTrigNode_triggerDuration_m;
    real_T MTrigNode2_triggerDuration_g;
    real_T MTrigNode_triggerDuration_jd;
    real_T MTrigNode1_triggerDuration_p;
    real_T MTrigNode1_triggerDuration_b;
    real_T MTrigNode_triggerDuration_c;
    real_T MTrigNode_triggerDuration_h;
    real_T RateLimiterVariableTs_up;
    real_T DetectChange_vinit;
    real_T DetectChange_vinit_a;
    real_T DetectChange_vinit_d;
    fmgc_approach_type EnumeratedConstant_Value;
    char_T pad_EnumeratedConstant_Value[4];
    fmgc_approach_type EnumeratedConstant_Value_g;
    char_T pad_EnumeratedConstant_Value_g[4];
    fmgc_des_submode EnumeratedConstant1_Value;
    char_T pad_EnumeratedConstant1_Value[4];
    fmgc_des_submode EnumeratedConstant2_Value;
    char_T pad_EnumeratedConstant2_Value[4];
    fms_flight_phase EnumeratedConstant_Value_m;
    char_T pad_EnumeratedConstant_Value_m[4];
    fms_flight_phase EnumeratedConstant_Value_i;
    char_T pad_EnumeratedConstant_Value_i[4];
    fms_flight_phase EnumeratedConstant1_Value_d;
    char_T pad_EnumeratedConstant1_Value_d[4];
    fms_flight_phase EnumeratedConstant_Value_a;
    char_T pad_EnumeratedConstant_Value_a[4];
    fms_flight_phase EnumeratedConstant1_Value_n;
    char_T pad_EnumeratedConstant1_Value_n[4];
    fms_flight_phase EnumeratedConstant2_Value_e;
    char_T pad_EnumeratedConstant2_Value_e[4];
    fms_flight_phase EnumeratedConstant1_Value_e;
    char_T pad_EnumeratedConstant1_Value_e[4];
    fms_flight_phase EnumeratedConstant2_Value_g;
    char_T pad_EnumeratedConstant2_Value_g[4];
    fms_flight_phase EnumeratedConstant3_Value;
    char_T pad_EnumeratedConstant3_Value[4];
    fms_flight_phase EnumeratedConstant4_Value;
    char_T pad_EnumeratedConstant4_Value[4];
    fms_flight_phase EnumeratedConstant_Value_m1;
    char_T pad_EnumeratedConstant_Value_m1[4];
    fms_flight_phase EnumeratedConstant1_Value_k;
    char_T pad_EnumeratedConstant1_Value_k[4];
    fms_flight_phase EnumeratedConstant1_Value_o;
    char_T pad_EnumeratedConstant1_Value_o[4];
    fms_flight_phase EnumeratedConstant2_Value_l;
    char_T pad_EnumeratedConstant2_Value_l[4];
    fms_flight_phase EnumeratedConstant3_Value_g;
    char_T pad_EnumeratedConstant3_Value_g[4];
    fms_flight_phase EnumeratedConstant5_Value;
    char_T pad_EnumeratedConstant5_Value[4];
    fms_flight_phase EnumeratedConstant4_Value_m;
    char_T pad_EnumeratedConstant4_Value_m[4];
    fms_flight_phase EnumeratedConstant_Value_e;
    char_T pad_EnumeratedConstant_Value_e[4];
    fms_flight_phase EnumeratedConstant1_Value_b;
    char_T pad_EnumeratedConstant1_Value_b[4];
    fms_flight_phase EnumeratedConstant2_Value_ec;
    char_T pad_EnumeratedConstant2_Value_ec[4];
    fms_flight_phase EnumeratedConstant_Value_p;
    char_T pad_EnumeratedConstant_Value_p[4];
    fms_flight_phase EnumeratedConstant1_Value_p;
    char_T pad_EnumeratedConstant1_Value_p[4];
    fms_flight_phase EnumeratedConstant_Value_gt;
    char_T pad_EnumeratedConstant_Value_gt[4];
    fms_flight_phase EnumeratedConstant_Value_l;
    char_T pad_EnumeratedConstant_Value_l[4];
    fms_flight_phase EnumeratedConstant2_Value_c;
    char_T pad_EnumeratedConstant2_Value_c[4];
    fms_flight_phase EnumeratedConstant_Value_k;
    char_T pad_EnumeratedConstant_Value_k[4];
    fms_flight_phase EnumeratedConstant_Value_ag;
    char_T pad_EnumeratedConstant_Value_ag[4];
    fms_flight_phase EnumeratedConstant1_Value_i;
    char_T pad_EnumeratedConstant1_Value_i[4];
    real32_T CompareToConstant3_const_h;
    char_T pad_CompareToConstant3_const_h[4];
    real32_T CompareToConstant5_const_i;
    char_T pad_CompareToConstant5_const_i[4];
    real32_T CompareToConstant4_const_e;
    char_T pad_CompareToConstant4_const_e[4];
    real32_T CompareToConstant6_const_h;
    char_T pad_CompareToConstant6_const_h[4];
    real32_T CompareToConstant3_const_a;
    char_T pad_CompareToConstant3_const_a[4];
    real32_T CompareToConstant5_const_n;
    char_T pad_CompareToConstant5_const_n[4];
    real32_T CompareToConstant4_const_j;
    char_T pad_CompareToConstant4_const_j[4];
    real32_T CompareToConstant6_const_a;
    char_T pad_CompareToConstant6_const_a[4];
    real32_T CompareToConstant1_const_j;
    char_T pad_CompareToConstant1_const_j[4];
    real32_T CompareToConstant1_const_jq;
    char_T pad_CompareToConstant1_const_jq[4];
    real32_T CompareToConstant1_const_hh;
    char_T pad_CompareToConstant1_const_hh[4];
    real32_T CompareToConstant_const_ov;
    char_T pad_CompareToConstant_const_ov[4];
    real32_T CompareToConstant_const_dl;
    char_T pad_CompareToConstant_const_dl[4];
    real32_T CompareToConstant_const_j0;
    char_T pad_CompareToConstant_const_j0[4];
    real32_T CompareToConstant_const_p;
    char_T pad_CompareToConstant_const_p[4];
    real32_T CompareToConstant1_const_j2;
    char_T pad_CompareToConstant1_const_j2[4];
    real32_T CompareToConstant_const_oj;
    char_T pad_CompareToConstant_const_oj[4];
    real32_T CompareToConstant2_const_e;
    char_T pad_CompareToConstant2_const_e[4];
    real32_T CompareToConstant3_const_br;
    char_T pad_CompareToConstant3_const_br[4];
    real32_T CompareToConstant1_const_hc;
    char_T pad_CompareToConstant1_const_hc[4];
    real32_T CompareToConstant2_const_pq;
    char_T pad_CompareToConstant2_const_pq[4];
    real32_T CompareToConstant_const_a;
    char_T pad_CompareToConstant_const_a[4];
    real32_T CompareToConstant_const_mj;
    char_T pad_CompareToConstant_const_mj[4];
    real32_T CompareToConstant1_const_ko;
    char_T pad_CompareToConstant1_const_ko[4];
    real32_T CompareToConstant2_const_pa;
    char_T pad_CompareToConstant2_const_pa[4];
    real32_T CompareToConstant3_const_l;
    char_T pad_CompareToConstant3_const_l[4];
    real32_T CompareToConstant_const_bx;
    char_T pad_CompareToConstant_const_bx[4];
    real32_T CompareToConstant2_const_kg;
    char_T pad_CompareToConstant2_const_kg[4];
    real32_T CompareToConstant1_const_mh;
    char_T pad_CompareToConstant1_const_mh[4];
    real32_T CompareToConstant3_const_e;
    char_T pad_CompareToConstant3_const_e[4];
    real32_T CompareToConstant12_const;
    char_T pad_CompareToConstant12_const[4];
    real32_T CompareToConstant14_const;
    char_T pad_CompareToConstant14_const[4];
    real32_T CompareToConstant13_const;
    char_T pad_CompareToConstant13_const[4];
    real32_T CompareToConstant15_const;
    char_T pad_CompareToConstant15_const[4];
    real32_T CompareToConstant4_const_g;
    char_T pad_CompareToConstant4_const_g[4];
    real32_T CompareToConstant6_const_k;
    char_T pad_CompareToConstant6_const_k[4];
    real32_T CompareToConstant5_const_p;
    char_T pad_CompareToConstant5_const_p[4];
    real32_T CompareToConstant7_const_l;
    char_T pad_CompareToConstant7_const_l[4];
    real32_T CompareToConstant8_const_kz;
    char_T pad_CompareToConstant8_const_kz[4];
    real32_T CompareToConstant10_const_c;
    char_T pad_CompareToConstant10_const_c[4];
    real32_T CompareToConstant9_const_k;
    char_T pad_CompareToConstant9_const_k[4];
    real32_T CompareToConstant11_const_p;
    char_T pad_CompareToConstant11_const_p[4];
    real32_T CompareToConstant1_const_ak;
    char_T pad_CompareToConstant1_const_ak[4];
    real32_T CompareToConstant_const_gf;
    char_T pad_CompareToConstant_const_gf[4];
    real32_T A429ValueOrDefault3_defaultValue;
    char_T pad_A429ValueOrDefault3_defaultValue[4];
    real32_T A429ValueOrDefault_defaultValue;
    char_T pad_A429ValueOrDefault_defaultValue[4];
    real32_T A429ValueOrDefault1_defaultValue;
    char_T pad_A429ValueOrDefault1_defaultValue[4];
    real32_T A429ValueOrDefault2_defaultValue;
    char_T pad_A429ValueOrDefault2_defaultValue[4];
    real32_T A429ValueOrDefault3_defaultValue_d;
    char_T pad_A429ValueOrDefault3_defaultValue_d[4];
    real32_T A429ValueOrDefault_defaultValue_g;
    char_T pad_A429ValueOrDefault_defaultValue_g[4];
    real32_T A429ValueOrDefault1_defaultValue_b;
    char_T pad_A429ValueOrDefault1_defaultValue_b[4];
    real32_T A429ValueOrDefault2_defaultValue_d;
    char_T pad_A429ValueOrDefault2_defaultValue_d[4];
    real32_T A429ValueOrDefault3_defaultValue_f;
    char_T pad_A429ValueOrDefault3_defaultValue_f[4];
    real32_T A429ValueOrDefault_defaultValue_p;
    char_T pad_A429ValueOrDefault_defaultValue_p[4];
    real32_T A429ValueOrDefault1_defaultValue_i;
    char_T pad_A429ValueOrDefault1_defaultValue_i[4];
    real32_T A429ValueOrDefault2_defaultValue_b;
    char_T pad_A429ValueOrDefault2_defaultValue_b[4];
    real32_T A429ValueOrDefault3_defaultValue_a;
    char_T pad_A429ValueOrDefault3_defaultValue_a[4];
    real32_T A429ValueOrDefault_defaultValue_e;
    char_T pad_A429ValueOrDefault_defaultValue_e[4];
    real32_T A429ValueOrDefault1_defaultValue_h;
    char_T pad_A429ValueOrDefault1_defaultValue_h[4];
    real32_T A429ValueOrDefault2_defaultValue_dd;
    char_T pad_A429ValueOrDefault2_defaultValue_dd[4];
    real32_T A429ValueOrDefault_defaultValue_ph;
    char_T pad_A429ValueOrDefault_defaultValue_ph[4];
    real32_T A429ValueOrDefault1_defaultValue_f;
    char_T pad_A429ValueOrDefault1_defaultValue_f[4];
    real32_T A429ValueOrDefault2_defaultValue_g;
    char_T pad_A429ValueOrDefault2_defaultValue_g[4];
    real32_T A429ValueOrDefault_defaultValue_j;
    char_T pad_A429ValueOrDefault_defaultValue_j[4];
    real32_T A429ValueOrDefault1_defaultValue_o;
    char_T pad_A429ValueOrDefault1_defaultValue_o[4];
    real32_T A429ValueOrDefault_defaultValue_n;
    char_T pad_A429ValueOrDefault_defaultValue_n[4];
    real32_T A429ValueOrDefault1_defaultValue_c;
    char_T pad_A429ValueOrDefault1_defaultValue_c[4];
    real32_T A429ValueOrDefault2_defaultValue_n;
    char_T pad_A429ValueOrDefault2_defaultValue_n[4];
    real32_T A429ValueOrDefault_defaultValue_l;
    char_T pad_A429ValueOrDefault_defaultValue_l[4];
    real32_T A429ValueOrDefault1_defaultValue_a;
    char_T pad_A429ValueOrDefault1_defaultValue_a[4];
    real32_T A429ValueOrDefault2_defaultValue_nt;
    char_T pad_A429ValueOrDefault2_defaultValue_nt[4];
    real32_T A429ValueOrDefault3_defaultValue_i;
    char_T pad_A429ValueOrDefault3_defaultValue_i[4];
    real32_T A429ValueOrDefault1_defaultValue_m;
    char_T pad_A429ValueOrDefault1_defaultValue_m[4];
    real32_T A429ValueOrDefault_defaultValue_py;
    char_T pad_A429ValueOrDefault_defaultValue_py[4];
    real32_T A429ValueOrDefault1_defaultValue_d;
    char_T pad_A429ValueOrDefault1_defaultValue_d[4];
    real32_T A429ValueOrDefault_defaultValue_d;
    char_T pad_A429ValueOrDefault_defaultValue_d[4];
    real32_T A429ValueOrDefault_defaultValue_m;
    char_T pad_A429ValueOrDefault_defaultValue_m[4];
    real32_T A429ValueOrDefault1_defaultValue_k;
    char_T pad_A429ValueOrDefault1_defaultValue_k[4];
    real32_T A429ValueOrDefault2_defaultValue_f;
    char_T pad_A429ValueOrDefault2_defaultValue_f[4];
    real32_T A429ValueOrDefault3_defaultValue_e;
    char_T pad_A429ValueOrDefault3_defaultValue_e[4];
    real32_T A429ValueOrDefault_defaultValue_h;
    char_T pad_A429ValueOrDefault_defaultValue_h[4];
    real32_T A429ValueOrDefault1_defaultValue_bm;
    char_T pad_A429ValueOrDefault1_defaultValue_bm[4];
    real32_T A429ValueOrDefault2_defaultValue_m;
    char_T pad_A429ValueOrDefault2_defaultValue_m[4];
    real32_T A429ValueOrDefault3_defaultValue_c;
    char_T pad_A429ValueOrDefault3_defaultValue_c[4];
    real32_T A429ValueOrDefault_defaultValue_i;
    char_T pad_A429ValueOrDefault_defaultValue_i[4];
    real32_T A429ValueOrDefault_defaultValue_hv;
    char_T pad_A429ValueOrDefault_defaultValue_hv[4];
    real32_T A429ValueOrDefault_defaultValue_f;
    char_T pad_A429ValueOrDefault_defaultValue_f[4];
    real32_T A429ValueOrDefault_defaultValue_jq;
    char_T pad_A429ValueOrDefault_defaultValue_jq[4];
    real32_T A429ValueOrDefault2_defaultValue_e;
    char_T pad_A429ValueOrDefault2_defaultValue_e[4];
    real32_T A429ValueOrDefault3_defaultValue_al;
    char_T pad_A429ValueOrDefault3_defaultValue_al[4];
    real32_T A429ValueOrDefault1_defaultValue_hj;
    char_T pad_A429ValueOrDefault1_defaultValue_hj[4];
    real32_T A429ValueOrDefault4_defaultValue;
    char_T pad_A429ValueOrDefault4_defaultValue[4];
    real32_T A429ValueOrDefault_defaultValue_k;
    char_T pad_A429ValueOrDefault_defaultValue_k[4];
    real32_T A429ValueOrDefault1_defaultValue_n;
    char_T pad_A429ValueOrDefault1_defaultValue_n[4];
    real32_T A429ValueOrDefault2_defaultValue_j;
    char_T pad_A429ValueOrDefault2_defaultValue_j[4];
    real32_T A429ValueOrDefault3_defaultValue_l;
    char_T pad_A429ValueOrDefault3_defaultValue_l[4];
    real32_T A429ValueOrDefault4_defaultValue_e;
    char_T pad_A429ValueOrDefault4_defaultValue_e[4];
    real32_T A429ValueOrDefault5_defaultValue;
    char_T pad_A429ValueOrDefault5_defaultValue[4];
    real32_T A429ValueOrDefault6_defaultValue;
    char_T pad_A429ValueOrDefault6_defaultValue[4];
    real32_T A429ValueOrDefault7_defaultValue;
    char_T pad_A429ValueOrDefault7_defaultValue[4];
    real32_T A429ValueOrDefault_defaultValue_a;
    char_T pad_A429ValueOrDefault_defaultValue_a[4];
    real32_T A429ValueOrDefault1_defaultValue_g;
    char_T pad_A429ValueOrDefault1_defaultValue_g[4];
    real32_T A429ValueOrDefault2_defaultValue_mu;
    char_T pad_A429ValueOrDefault2_defaultValue_mu[4];
    real32_T A429ValueOrDefault3_defaultValue_f2;
    char_T pad_A429ValueOrDefault3_defaultValue_f2[4];
    real32_T A429ValueOrDefault_defaultValue_ag;
    char_T pad_A429ValueOrDefault_defaultValue_ag[4];
    real32_T A429ValueOrDefault1_defaultValue_hs;
    char_T pad_A429ValueOrDefault1_defaultValue_hs[4];
    real32_T A429ValueOrDefault2_defaultValue_i;
    char_T pad_A429ValueOrDefault2_defaultValue_i[4];
    real32_T A429ValueOrDefault3_defaultValue_id;
    char_T pad_A429ValueOrDefault3_defaultValue_id[4];
    real32_T A429ValueOrDefault4_defaultValue_o;
    char_T pad_A429ValueOrDefault4_defaultValue_o[4];
    real32_T A429ValueOrDefault5_defaultValue_n;
    char_T pad_A429ValueOrDefault5_defaultValue_n[4];
    real32_T A429ValueOrDefault6_defaultValue_b;
    char_T pad_A429ValueOrDefault6_defaultValue_b[4];
    real32_T A429ValueOrDefault7_defaultValue_l;
    char_T pad_A429ValueOrDefault7_defaultValue_l[4];
    real32_T A429ValueOrDefault8_defaultValue;
    char_T pad_A429ValueOrDefault8_defaultValue[4];
    real32_T A429ValueOrDefault8_defaultValue_o;
    char_T pad_A429ValueOrDefault8_defaultValue_o[4];
    real32_T A429ValueOrDefault_defaultValue_o;
    char_T pad_A429ValueOrDefault_defaultValue_o[4];
    real32_T A429ValueOrDefault11_defaultValue;
    char_T pad_A429ValueOrDefault11_defaultValue[4];
    real32_T A429ValueOrDefault10_defaultValue;
    char_T pad_A429ValueOrDefault10_defaultValue[4];
    real32_T A429ValueOrDefault8_defaultValue_i;
    char_T pad_A429ValueOrDefault8_defaultValue_i[4];
    real32_T A429ValueOrDefault12_defaultValue;
    char_T pad_A429ValueOrDefault12_defaultValue[4];
    real32_T A429ValueOrDefault13_defaultValue;
    char_T pad_A429ValueOrDefault13_defaultValue[4];
    real32_T A429ValueOrDefault14_defaultValue;
    char_T pad_A429ValueOrDefault14_defaultValue[4];
    real32_T A429ValueOrDefault9_defaultValue;
    char_T pad_A429ValueOrDefault9_defaultValue[4];
    real32_T A429ValueOrDefault3_defaultValue_ce;
    char_T pad_A429ValueOrDefault3_defaultValue_ce[4];
    real32_T A429ValueOrDefault1_defaultValue_iz;
    char_T pad_A429ValueOrDefault1_defaultValue_iz[4];
    real32_T A429ValueOrDefault2_defaultValue_gg;
    char_T pad_A429ValueOrDefault2_defaultValue_gg[4];
    real32_T DetectChange_vinit_m;
    char_T pad_DetectChange_vinit_m[4];
    real32_T DetectChange_vinit_p;
    char_T pad_DetectChange_vinit_p[4];
    real32_T DetectDecrease_vinit;
    char_T pad_DetectDecrease_vinit[4];
    tcas_submode EnumeratedConstant3_Value_a;
    char_T pad_EnumeratedConstant3_Value_a[4];
    tcas_submode EnumeratedConstant3_Value_h;
    char_T pad_EnumeratedConstant3_Value_h[4];
    tcas_submode EnumeratedConstant3_Value_i;
    char_T pad_EnumeratedConstant3_Value_i[4];
    tcas_submode EnumeratedConstant_Value_o;
    char_T pad_EnumeratedConstant_Value_o[4];
    tcas_submode EnumeratedConstant1_Value_bl;
    char_T pad_EnumeratedConstant1_Value_bl[4];
    boolean_T SRFlipFlopwithSyncInput_initial_condition;
    char_T pad_SRFlipFlopwithSyncInput_initial_condition[7];
    boolean_T Subsystem_initial_condition;
    char_T pad_Subsystem_initial_condition[7];
    boolean_T Subsystem_initial_condition_e;
    char_T pad_Subsystem_initial_condition_e[7];
    boolean_T SRFlipFlop1_initial_condition;
    char_T pad_SRFlipFlop1_initial_condition[7];
    boolean_T SRFlipFlopwithSyncInput_initial_condition_l;
    char_T pad_SRFlipFlopwithSyncInput_initial_condition_l[7];
    boolean_T SRFlipFlop_initial_condition;
    char_T pad_SRFlipFlop_initial_condition[7];
    boolean_T Subsystem4_initial_condition;
    char_T pad_Subsystem4_initial_condition[7];
    boolean_T Subsystem3_initial_condition;
    char_T pad_Subsystem3_initial_condition[7];
    boolean_T Subsystem1_initial_condition;
    char_T pad_Subsystem1_initial_condition[7];
    boolean_T Subsystem_initial_condition_l;
    char_T pad_Subsystem_initial_condition_l[7];
    boolean_T SRFlipFlop_initial_condition_k;
    char_T pad_SRFlipFlop_initial_condition_k[7];
    boolean_T SRFlipFlop_initial_condition_l;
    char_T pad_SRFlipFlop_initial_condition_l[7];
    boolean_T SRFlipFlop_initial_condition_b;
    char_T pad_SRFlipFlop_initial_condition_b[7];
    boolean_T SRFlipFlop_initial_condition_o;
    char_T pad_SRFlipFlop_initial_condition_o[7];
    boolean_T SRFlipFlop_initial_condition_j;
    char_T pad_SRFlipFlop_initial_condition_j[7];
    boolean_T SRFlipFlop_initial_condition_lg;
    char_T pad_SRFlipFlop_initial_condition_lg[7];
    boolean_T SRFlipFlop_initial_condition_oh;
    char_T pad_SRFlipFlop_initial_condition_oh[7];
    boolean_T SRFlipFlop_initial_condition_c;
    char_T pad_SRFlipFlop_initial_condition_c[7];
    boolean_T SRFlipFlop_initial_condition_h;
    char_T pad_SRFlipFlop_initial_condition_h[7];
    boolean_T SRFlipFlop_initial_condition_bj;
    char_T pad_SRFlipFlop_initial_condition_bj[7];
    boolean_T SRFlipFlop_initial_condition_m;
    char_T pad_SRFlipFlop_initial_condition_m[7];
    boolean_T SRFlipFlop_initial_condition_g;
    char_T pad_SRFlipFlop_initial_condition_g[7];
    boolean_T SRFlipFlop_initial_condition_jb;
    char_T pad_SRFlipFlop_initial_condition_jb[7];
    boolean_T SRFlipFlop_initial_condition_n;
    char_T pad_SRFlipFlop_initial_condition_n[7];
    boolean_T SRFlipFlop_initial_condition_d;
    char_T pad_SRFlipFlop_initial_condition_d[7];
    boolean_T SRFlipFlop_initial_condition_db;
    char_T pad_SRFlipFlop_initial_condition_db[7];
    boolean_T SRFlipFlop_initial_condition_lv;
    char_T pad_SRFlipFlop_initial_condition_lv[7];
    boolean_T SRFlipFlop1_initial_condition_g;
    char_T pad_SRFlipFlop1_initial_condition_g[7];
    boolean_T SRFlipFlop_initial_condition_jn;
    char_T pad_SRFlipFlop_initial_condition_jn[7];
    boolean_T SRFlipFlop_initial_condition_i;
    char_T pad_SRFlipFlop_initial_condition_i[7];
    boolean_T SRFlipFlop2_initial_condition;
    char_T pad_SRFlipFlop2_initial_condition[7];
    boolean_T SRFlipFlop1_initial_condition_n;
    char_T pad_SRFlipFlop1_initial_condition_n[7];
    boolean_T SRFlipFlop1_initial_condition_c;
    char_T pad_SRFlipFlop1_initial_condition_c[7];
    boolean_T SRFlipFlop_initial_condition_p;
    char_T pad_SRFlipFlop_initial_condition_p[7];
    boolean_T SRFlipFlop_initial_condition_a;
    char_T pad_SRFlipFlop_initial_condition_a[7];
    boolean_T SRFlipFlop_initial_condition_bp;
    char_T pad_SRFlipFlop_initial_condition_bp[7];
    boolean_T SRFlipFlop_initial_condition_lk;
    char_T pad_SRFlipFlop_initial_condition_lk[7];
    boolean_T SRFlipFlop_initial_condition_ok;
    char_T pad_SRFlipFlop_initial_condition_ok[7];
    boolean_T SRFlipFlop_initial_condition_mc;
    char_T pad_SRFlipFlop_initial_condition_mc[7];
    boolean_T SRFlipFlop_initial_condition_j1;
    char_T pad_SRFlipFlop_initial_condition_j1[7];
    boolean_T SRFlipFlop_initial_condition_md;
    char_T pad_SRFlipFlop_initial_condition_md[7];
    boolean_T SRFlipFlop_initial_condition_ac;
    char_T pad_SRFlipFlop_initial_condition_ac[7];
    boolean_T SRFlipFlop_initial_condition_hq;
    char_T pad_SRFlipFlop_initial_condition_hq[7];
    boolean_T SRFlipFlop1_initial_condition_f;
    char_T pad_SRFlipFlop1_initial_condition_f[7];
    boolean_T SRFlipFlop_initial_condition_jq;
    char_T pad_SRFlipFlop_initial_condition_jq[7];
    boolean_T SRFlipFlop1_initial_condition_d;
    char_T pad_SRFlipFlop1_initial_condition_d[7];
    boolean_T SRFlipFlop2_initial_condition_e;
    char_T pad_SRFlipFlop2_initial_condition_e[7];
    boolean_T SRFlipFlop1_initial_condition_j;
    char_T pad_SRFlipFlop1_initial_condition_j[7];
    boolean_T SRFlipFlop_initial_condition_pb;
    char_T pad_SRFlipFlop_initial_condition_pb[7];
    boolean_T SRFlipFlop_initial_condition_mv;
    char_T pad_SRFlipFlop_initial_condition_mv[7];
    boolean_T SRFlipFlop_initial_condition_f;
    char_T pad_SRFlipFlop_initial_condition_f[7];
    boolean_T SRFlipFlop_initial_condition_pq;
    char_T pad_SRFlipFlop_initial_condition_pq[7];
    boolean_T SRFlipFlop1_initial_condition_jw;
    char_T pad_SRFlipFlop1_initial_condition_jw[7];
    boolean_T Subsystem4_initial_condition_p;
    char_T pad_Subsystem4_initial_condition_p[7];
    boolean_T SRFlipFlop1_initial_condition_fn;
    char_T pad_SRFlipFlop1_initial_condition_fn[7];
    boolean_T SRFlipFlop1_initial_condition_d2;
    char_T pad_SRFlipFlop1_initial_condition_d2[7];
    boolean_T SRFlipFlop1_initial_condition_co;
    char_T pad_SRFlipFlop1_initial_condition_co[7];
    boolean_T SRFlipFlop_initial_condition_ll;
    char_T pad_SRFlipFlop_initial_condition_ll[7];
    boolean_T SRFlipFlop1_initial_condition_b;
    char_T pad_SRFlipFlop1_initial_condition_b[7];
    boolean_T ConfirmNode1_isRisingEdge;
    char_T pad_ConfirmNode1_isRisingEdge[7];
    boolean_T PulseNode_isRisingEdge;
    char_T pad_PulseNode_isRisingEdge[7];
    boolean_T PulseNode1_isRisingEdge;
    char_T pad_PulseNode1_isRisingEdge[7];
    boolean_T PulseNode_isRisingEdge_b;
    char_T pad_PulseNode_isRisingEdge_b[7];
    boolean_T PulseNode3_isRisingEdge;
    char_T pad_PulseNode3_isRisingEdge[7];
    boolean_T PulseNode2_isRisingEdge;
    char_T pad_PulseNode2_isRisingEdge[7];
    boolean_T PulseNode1_isRisingEdge_p;
    char_T pad_PulseNode1_isRisingEdge_p[7];
    boolean_T PulseNode_isRisingEdge_d;
    char_T pad_PulseNode_isRisingEdge_d[7];
    boolean_T PulseNode3_isRisingEdge_k;
    char_T pad_PulseNode3_isRisingEdge_k[7];
    boolean_T PulseNode2_isRisingEdge_g;
    char_T pad_PulseNode2_isRisingEdge_g[7];
    boolean_T PulseNode1_isRisingEdge_b;
    char_T pad_PulseNode1_isRisingEdge_b[7];
    boolean_T ConfirmNode1_isRisingEdge_p;
    char_T pad_ConfirmNode1_isRisingEdge_p[7];
    boolean_T PulseNode_isRisingEdge_p;
    char_T pad_PulseNode_isRisingEdge_p[7];
    boolean_T ConfirmNode2_isRisingEdge;
    char_T pad_ConfirmNode2_isRisingEdge[7];
    boolean_T PulseNode1_isRisingEdge_pq;
    char_T pad_PulseNode1_isRisingEdge_pq[7];
    boolean_T PulseNode2_isRisingEdge_m;
    char_T pad_PulseNode2_isRisingEdge_m[7];
    boolean_T PulseNode3_isRisingEdge_p;
    char_T pad_PulseNode3_isRisingEdge_p[7];
    boolean_T PulseNode5_isRisingEdge;
    char_T pad_PulseNode5_isRisingEdge[7];
    boolean_T ConfirmNode3_isRisingEdge;
    char_T pad_ConfirmNode3_isRisingEdge[7];
    boolean_T PulseNode3_isRisingEdge_n;
    char_T pad_PulseNode3_isRisingEdge_n[7];
    boolean_T PulseNode1_isRisingEdge_f;
    char_T pad_PulseNode1_isRisingEdge_f[7];
    boolean_T PulseNode_isRisingEdge_a;
    char_T pad_PulseNode_isRisingEdge_a[7];
    boolean_T PulseNode3_isRisingEdge_m;
    char_T pad_PulseNode3_isRisingEdge_m[7];
    boolean_T PulseNode1_isRisingEdge_j;
    char_T pad_PulseNode1_isRisingEdge_j[7];
    boolean_T PulseNode2_isRisingEdge_k;
    char_T pad_PulseNode2_isRisingEdge_k[7];
    boolean_T PulseNode4_isRisingEdge;
    char_T pad_PulseNode4_isRisingEdge[7];
    boolean_T PulseNode_isRisingEdge_j;
    char_T pad_PulseNode_isRisingEdge_j[7];
    boolean_T PulseNode1_isRisingEdge_o;
    char_T pad_PulseNode1_isRisingEdge_o[7];
    boolean_T PulseNode_isRisingEdge_l;
    char_T pad_PulseNode_isRisingEdge_l[7];
    boolean_T PulseNode1_isRisingEdge_l;
    char_T pad_PulseNode1_isRisingEdge_l[7];
    boolean_T PulseNode3_isRisingEdge_l;
    char_T pad_PulseNode3_isRisingEdge_l[7];
    boolean_T PulseNode_isRisingEdge_dy;
    char_T pad_PulseNode_isRisingEdge_dy[7];
    boolean_T ConfirmNode_isRisingEdge;
    char_T pad_ConfirmNode_isRisingEdge[7];
    boolean_T PulseNode_isRisingEdge_n;
    char_T pad_PulseNode_isRisingEdge_n[7];
    boolean_T ConfirmNode_isRisingEdge_j;
    char_T pad_ConfirmNode_isRisingEdge_j[7];
    boolean_T ConfirmNode_isRisingEdge_o;
    char_T pad_ConfirmNode_isRisingEdge_o[7];
    boolean_T ConfirmNode1_isRisingEdge_f;
    char_T pad_ConfirmNode1_isRisingEdge_f[7];
    boolean_T ConfirmNode_isRisingEdge_k;
    char_T pad_ConfirmNode_isRisingEdge_k[7];
    boolean_T ConfirmNode_isRisingEdge_g;
    char_T pad_ConfirmNode_isRisingEdge_g[7];
    boolean_T PulseNode_isRisingEdge_le;
    char_T pad_PulseNode_isRisingEdge_le[7];
    boolean_T ConfirmNode_isRisingEdge_b;
    char_T pad_ConfirmNode_isRisingEdge_b[7];
    boolean_T PulseNode_isRisingEdge_f;
    char_T pad_PulseNode_isRisingEdge_f[7];
    boolean_T ConfirmNode_isRisingEdge_h;
    char_T pad_ConfirmNode_isRisingEdge_h[7];
    boolean_T PulseNode2_isRisingEdge_b;
    char_T pad_PulseNode2_isRisingEdge_b[7];
    boolean_T PulseNode3_isRisingEdge_n4;
    char_T pad_PulseNode3_isRisingEdge_n4[7];
    boolean_T PulseNode1_isRisingEdge_d;
    char_T pad_PulseNode1_isRisingEdge_d[7];
    boolean_T ConfirmNode_isRisingEdge_hr;
    char_T pad_ConfirmNode_isRisingEdge_hr[7];
    boolean_T ConfirmNode1_isRisingEdge_h;
    char_T pad_ConfirmNode1_isRisingEdge_h[7];
    boolean_T ConfirmNode_isRisingEdge_ok;
    char_T pad_ConfirmNode_isRisingEdge_ok[7];
    boolean_T PulseNode_isRisingEdge_bf;
    char_T pad_PulseNode_isRisingEdge_bf[7];
    boolean_T ConfirmNode2_isRisingEdge_k;
    char_T pad_ConfirmNode2_isRisingEdge_k[7];
    boolean_T ConfirmNode1_isRisingEdge_k;
    char_T pad_ConfirmNode1_isRisingEdge_k[7];
    boolean_T ConfirmNode1_isRisingEdge_e;
    char_T pad_ConfirmNode1_isRisingEdge_e[7];
    boolean_T ConfirmNode_isRisingEdge_bv;
    char_T pad_ConfirmNode_isRisingEdge_bv[7];
    boolean_T PulseNode_isRisingEdge_i;
    char_T pad_PulseNode_isRisingEdge_i[7];
    boolean_T PulseNode3_isRisingEdge_b;
    char_T pad_PulseNode3_isRisingEdge_b[7];
    boolean_T PulseNode1_isRisingEdge_fw;
    char_T pad_PulseNode1_isRisingEdge_fw[7];
    boolean_T ConfirmNode_isRisingEdge_n;
    char_T pad_ConfirmNode_isRisingEdge_n[7];
    boolean_T ConfirmNode_isRisingEdge_om;
    char_T pad_ConfirmNode_isRisingEdge_om[7];
    boolean_T PulseNode_isRisingEdge_k;
    char_T pad_PulseNode_isRisingEdge_k[7];
    boolean_T ConfirmNode_isRisingEdge_ks;
    char_T pad_ConfirmNode_isRisingEdge_ks[7];
    boolean_T ConfirmNode_isRisingEdge_bw;
    char_T pad_ConfirmNode_isRisingEdge_bw[7];
    boolean_T PulseNode3_isRisingEdge_e;
    char_T pad_PulseNode3_isRisingEdge_e[7];
    boolean_T PulseNode5_isRisingEdge_e;
    char_T pad_PulseNode5_isRisingEdge_e[7];
    boolean_T PulseNode2_isRisingEdge_gk;
    char_T pad_PulseNode2_isRisingEdge_gk[7];
    boolean_T PulseNode1_isRisingEdge_a;
    char_T pad_PulseNode1_isRisingEdge_a[7];
    boolean_T PulseNode_isRisingEdge_ft;
    char_T pad_PulseNode_isRisingEdge_ft[7];
    boolean_T PulseNode4_isRisingEdge_b;
    char_T pad_PulseNode4_isRisingEdge_b[7];
    boolean_T ConfirmNode_isRisingEdge_d;
    char_T pad_ConfirmNode_isRisingEdge_d[7];
    boolean_T PulseNode_isRisingEdge_nq;
    char_T pad_PulseNode_isRisingEdge_nq[7];
    boolean_T PulseNode7_isRisingEdge;
    char_T pad_PulseNode7_isRisingEdge[7];
    boolean_T PulseNode3_isRisingEdge_a;
    char_T pad_PulseNode3_isRisingEdge_a[7];
    boolean_T PulseNode4_isRisingEdge_g;
    char_T pad_PulseNode4_isRisingEdge_g[7];
    boolean_T PulseNode2_isRisingEdge_i;
    char_T pad_PulseNode2_isRisingEdge_i[7];
    boolean_T PulseNode1_isRisingEdge_k;
    char_T pad_PulseNode1_isRisingEdge_k[7];
    boolean_T PulseNode6_isRisingEdge;
    char_T pad_PulseNode6_isRisingEdge[7];
    boolean_T PulseNode5_isRisingEdge_g;
    char_T pad_PulseNode5_isRisingEdge_g[7];
    boolean_T ConfirmNode_isRisingEdge_nr;
    char_T pad_ConfirmNode_isRisingEdge_nr[7];
    boolean_T PulseNode1_isRisingEdge_pqz;
    char_T pad_PulseNode1_isRisingEdge_pqz[7];
    boolean_T PulseNode2_isRisingEdge_h;
    char_T pad_PulseNode2_isRisingEdge_h[7];
    boolean_T ConfirmNode_isRisingEdge_e;
    char_T pad_ConfirmNode_isRisingEdge_e[7];
    boolean_T PulseNode_isRisingEdge_k3;
    char_T pad_PulseNode_isRisingEdge_k3[7];
    boolean_T PulseNode3_isRisingEdge_i;
    char_T pad_PulseNode3_isRisingEdge_i[7];
    boolean_T ConfirmNode_isRisingEdge_f;
    char_T pad_ConfirmNode_isRisingEdge_f[7];
    boolean_T ConfirmNode_isRisingEdge_jx;
    char_T pad_ConfirmNode_isRisingEdge_jx[7];
    boolean_T ConfirmNode_isRisingEdge_c;
    char_T pad_ConfirmNode_isRisingEdge_c[7];
    boolean_T PulseNode_isRisingEdge_dn;
    char_T pad_PulseNode_isRisingEdge_dn[7];
    boolean_T ConfirmNode_isRisingEdge_gx;
    char_T pad_ConfirmNode_isRisingEdge_gx[7];
    boolean_T ConfirmNode_isRisingEdge_ne;
    char_T pad_ConfirmNode_isRisingEdge_ne[7];
    boolean_T PulseNode_isRisingEdge_nr;
    char_T pad_PulseNode_isRisingEdge_nr[7];
    boolean_T ConfirmNode_isRisingEdge_l;
    char_T pad_ConfirmNode_isRisingEdge_l[7];
    boolean_T PulseNode1_isRisingEdge_c;
    char_T pad_PulseNode1_isRisingEdge_c[7];
    boolean_T PulseNode3_isRisingEdge_f;
    char_T pad_PulseNode3_isRisingEdge_f[7];
    boolean_T PulseNode4_isRisingEdge_k;
    char_T pad_PulseNode4_isRisingEdge_k[7];
    boolean_T PulseNode5_isRisingEdge_k;
    char_T pad_PulseNode5_isRisingEdge_k[7];
    boolean_T PulseNode_isRisingEdge_pu;
    char_T pad_PulseNode_isRisingEdge_pu[7];
    boolean_T PulseNode1_isRisingEdge_cr;
    char_T pad_PulseNode1_isRisingEdge_cr[7];
    boolean_T ConfirmNode_isRisingEdge_bx;
    char_T pad_ConfirmNode_isRisingEdge_bx[7];
    boolean_T ConfirmNode_isRisingEdge_co;
    char_T pad_ConfirmNode_isRisingEdge_co[7];
    boolean_T PulseNode_isRisingEdge_h;
    char_T pad_PulseNode_isRisingEdge_h[7];
    boolean_T PulseNode_isRisingEdge_j2;
    char_T pad_PulseNode_isRisingEdge_j2[7];
    boolean_T PulseNode1_isRisingEdge_ag;
    char_T pad_PulseNode1_isRisingEdge_ag[7];
    boolean_T ConfirmNode_isRisingEdge_a;
    char_T pad_ConfirmNode_isRisingEdge_a[7];
    boolean_T PulseNode1_isRisingEdge_bi;
    char_T pad_PulseNode1_isRisingEdge_bi[7];
    boolean_T PulseNode1_isRisingEdge_kp;
    char_T pad_PulseNode1_isRisingEdge_kp[7];
    boolean_T ConfirmNode_isRisingEdge_lt;
    char_T pad_ConfirmNode_isRisingEdge_lt[7];
    boolean_T PulseNode_isRisingEdge_c;
    char_T pad_PulseNode_isRisingEdge_c[7];
    boolean_T ConfirmNode_isRisingEdge_gw;
    char_T pad_ConfirmNode_isRisingEdge_gw[7];
    boolean_T PulseNode1_isRisingEdge_bm;
    char_T pad_PulseNode1_isRisingEdge_bm[7];
    boolean_T ConfirmNode_isRisingEdge_p;
    char_T pad_ConfirmNode_isRisingEdge_p[7];
    boolean_T PulseNode_isRisingEdge_nb;
    char_T pad_PulseNode_isRisingEdge_nb[7];
    boolean_T ConfirmNode_isRisingEdge_f5;
    char_T pad_ConfirmNode_isRisingEdge_f5[7];
    boolean_T PulseNode1_isRisingEdge_ds;
    char_T pad_PulseNode1_isRisingEdge_ds[7];
    boolean_T PulseNode_isRisingEdge_ll;
    char_T pad_PulseNode_isRisingEdge_ll[7];
    boolean_T ConfirmNode_isRisingEdge_i;
    char_T pad_ConfirmNode_isRisingEdge_i[7];
    boolean_T ConfirmNode_isRisingEdge_pb;
    char_T pad_ConfirmNode_isRisingEdge_pb[7];
    boolean_T ConfirmNode_isRisingEdge_ab;
    char_T pad_ConfirmNode_isRisingEdge_ab[7];
    boolean_T PulseNode1_isRisingEdge_n;
    char_T pad_PulseNode1_isRisingEdge_n[7];
    boolean_T ConfirmNode_isRisingEdge_bz;
    char_T pad_ConfirmNode_isRisingEdge_bz[7];
    boolean_T ConfirmNode1_isRisingEdge_c;
    char_T pad_ConfirmNode1_isRisingEdge_c[7];
    boolean_T PulseNode3_isRisingEdge_h;
    char_T pad_PulseNode3_isRisingEdge_h[7];
    boolean_T PulseNode2_isRisingEdge_o;
    char_T pad_PulseNode2_isRisingEdge_o[7];
    boolean_T PulseNode4_isRisingEdge_i;
    char_T pad_PulseNode4_isRisingEdge_i[7];
    boolean_T PulseNode7_isRisingEdge_g;
    char_T pad_PulseNode7_isRisingEdge_g[7];
    boolean_T ConfirmNode_isRisingEdge_fi;
    char_T pad_ConfirmNode_isRisingEdge_fi[7];
    boolean_T PulseNode6_isRisingEdge_i;
    char_T pad_PulseNode6_isRisingEdge_i[7];
    boolean_T PulseNode5_isRisingEdge_a;
    char_T pad_PulseNode5_isRisingEdge_a[7];
    boolean_T PulseNode1_isRisingEdge_jl;
    char_T pad_PulseNode1_isRisingEdge_jl[7];
    boolean_T PulseNode5_isRisingEdge_aw;
    char_T pad_PulseNode5_isRisingEdge_aw[7];
    boolean_T PulseNode2_isRisingEdge_j;
    char_T pad_PulseNode2_isRisingEdge_j[7];
    boolean_T PulseNode3_isRisingEdge_g;
    char_T pad_PulseNode3_isRisingEdge_g[7];
    boolean_T PulseNode_isRisingEdge_kh;
    char_T pad_PulseNode_isRisingEdge_kh[7];
    boolean_T ConfirmNode_isRisingEdge_jb;
    char_T pad_ConfirmNode_isRisingEdge_jb[7];
    boolean_T PulseNode6_isRisingEdge_m;
    char_T pad_PulseNode6_isRisingEdge_m[7];
    boolean_T PulseNode7_isRisingEdge_i;
    char_T pad_PulseNode7_isRisingEdge_i[7];
    boolean_T PulseNode_isRisingEdge_g;
    char_T pad_PulseNode_isRisingEdge_g[7];
    boolean_T ConfirmNode1_isRisingEdge_ei;
    char_T pad_ConfirmNode1_isRisingEdge_ei[7];
    boolean_T ConfirmNode_isRisingEdge_f1;
    char_T pad_ConfirmNode_isRisingEdge_f1[7];
    boolean_T ConfirmNode1_isRisingEdge_ku;
    char_T pad_ConfirmNode1_isRisingEdge_ku[7];
    boolean_T ConfirmNode2_isRisingEdge_h;
    char_T pad_ConfirmNode2_isRisingEdge_h[7];
    boolean_T ConfirmNode_isRisingEdge_jo;
    char_T pad_ConfirmNode_isRisingEdge_jo[7];
    boolean_T PulseNode3_isRisingEdge_ky;
    char_T pad_PulseNode3_isRisingEdge_ky[7];
    boolean_T ConfirmNode_isRisingEdge_a0;
    char_T pad_ConfirmNode_isRisingEdge_a0[7];
    boolean_T PulseNode3_isRisingEdge_nw;
    char_T pad_PulseNode3_isRisingEdge_nw[7];
    boolean_T PulseNode_isRisingEdge_m;
    char_T pad_PulseNode_isRisingEdge_m[7];
    boolean_T ConfirmNode_isRisingEdge_gd;
    char_T pad_ConfirmNode_isRisingEdge_gd[7];
    boolean_T PulseNode_isRisingEdge_hj;
    char_T pad_PulseNode_isRisingEdge_hj[7];
    boolean_T ConfirmNode_isRisingEdge_br;
    char_T pad_ConfirmNode_isRisingEdge_br[7];
    boolean_T ConfirmNode_isRisingEdge_d3;
    char_T pad_ConfirmNode_isRisingEdge_d3[7];
    int8_T CompareToConstant_const_l;
    char_T pad_CompareToConstant_const_l[7];
    int8_T CompareToConstant_const_mo;
    char_T pad_CompareToConstant_const_mo[7];
    int8_T CompareToConstant_const_mw;
    char_T pad_CompareToConstant_const_mw[7];
    int8_T FCUKnobTurnsUnpack_field;
    char_T pad_FCUKnobTurnsUnpack_field[7];
    int8_T FCUKnobTurnsUnpack_field_h;
    char_T pad_FCUKnobTurnsUnpack_field_h[7];
    int8_T FCUKnobTurnsUnpack_field_n;
    char_T pad_FCUKnobTurnsUnpack_field_n[7];
    int8_T FCUKnobTurnsUnpack_field_e;
    char_T pad_FCUKnobTurnsUnpack_field_e[7];
    uint8_T CompareToConstant4_const_b;
    char_T pad_CompareToConstant4_const_b[7];
    prim_outputs out_Y0;
    base_prim_out_bus Constant_Value;
    base_prim_fg_laws_outputs Delay2_InitialCondition;
    char_T pad_Delay2_InitialCondition[4];
    base_prim_ap_fd_logic_outputs Delay_InitialCondition;
    char_T pad_Delay_InitialCondition[5];
    base_arinc_429 Constant_Value_h;
    real_T Constant2_Value;
    real_T RETARD_Value;
    real_T Gain_Gain;
    real_T Saturation1_UpperSat;
    real_T Saturation1_LowerSat;
    real_T Constant_Value_g;
    real_T Constant1_Value;
    real_T Saturation_UpperSat;
    real_T Saturation_LowerSat;
    real_T Constant1_Value_h;
    real_T Constant_Value_l;
    real_T Y_Y0;
    real_T Gain1_Gain;
    real_T Gain_Gain_f;
    real_T Gain1_Gain_l;
    real_T Gain_Gain_k;
    real_T Y_Y0_a;
    real_T Y_Y0_l;
    real_T Y_Y0_h;
    real_T Constant_Value_hn;
    real_T Gain1_Gain_k;
    real_T Bias_Bias;
    real_T Gain_Gain_c;
    real_T Constant3_Value;
    real_T Constant_Value_j;
    real_T Constant1_Value_o;
    real_T Gain1_Gain_g;
    real_T Gain_Gain_m;
    real_T Y_Y0_i;
    real_T Constant6_Value;
    real_T Switch7_Threshold;
    real_T Switch8_Threshold;
    real_T Y_Y0_j;
    real_T Constant6_Value_o;
    real_T Switch7_Threshold_g;
    real_T Switch8_Threshold_m;
    real_T Constant_Value_i;
    real_T Constant3_Value_d;
    real_T Constant3_Value_m;
    real_T Bias_Bias_m;
    real_T Bias1_Bias;
    real_T Bias_Bias_p;
    real_T Bias1_Bias_e;
    real_T Bias_Bias_b;
    real_T Bias_Bias_mk;
    real_T Bias_Bias_n;
    real_T Constant4_Value;
    real_T Constant3_Value_k;
    real_T Gain3_Gain;
    real_T Saturation_UpperSat_n;
    real_T Saturation_LowerSat_n;
    real_T Gain_Gain_me;
    real_T Gain1_Gain_o;
    real_T Bias_Bias_mq;
    real_T Gain2_Gain;
    real_T Constant_Value_a;
    real_T Constant5_Value;
    real_T Switch9_Threshold;
    real_T Constant4_Value_f;
    real_T Constant3_Value_i;
    real_T Gain3_Gain_k;
    real_T Saturation_UpperSat_m;
    real_T Saturation_LowerSat_m;
    real_T Gain_Gain_l;
    real_T Gain1_Gain_i;
    real_T Bias_Bias_bi;
    real_T Gain2_Gain_i;
    real_T Constant_Value_ag;
    real_T Constant5_Value_h;
    real_T Switch9_Threshold_f;
    real_T Gain1_Gain_p;
    real_T fpmtoms_Gain;
    real_T kntoms_Gain;
    real_T Saturation_UpperSat_c;
    real_T Saturation_LowerSat_d;
    real_T Gain_Gain_o;
    real_T Gain1_Gain_m;
    real_T Constant_Value_g2;
    real_T Gain1_Gain_ib;
    real_T Gain1_Gain_gv;
    real_T Gain1_Gain_iz;
    real_T Gain_Gain_h;
    real_T Gain1_Gain_oj;
    real_T g_Gain;
    real_T Saturation_UpperSat_a;
    real_T Saturation_LowerSat_mu;
    real_T ktstomps_Gain;
    real_T _Gain;
    real_T Saturation1_UpperSat_l;
    real_T Saturation1_LowerSat_o;
    real_T ktstomps_Gain_n;
    real_T mpstokts_Gain;
    real_T Gain4_Gain;
    real_T Delay_InitialCondition_i;
    real32_T Gain3_Gain_ka;
    char_T pad_Gain3_Gain_ka[4];
    real32_T Gain1_Gain_ix;
    char_T pad_Gain1_Gain_ix[4];
    real32_T uDLookupTable_tableData[7];
    char_T pad_uDLookupTable_tableData[4];
    real32_T uDLookupTable_bp01Data[7];
    char_T pad_uDLookupTable_bp01Data[4];
    real32_T Gain_Gain_fm;
    char_T pad_Gain_Gain_fm[4];
    real32_T Constant_Value_f;
    char_T pad_Constant_Value_f[4];
    real32_T Y_Y0_l0;
    char_T pad_Y_Y0_l0[4];
    real32_T Y_Y0_k;
    char_T pad_Y_Y0_k[4];
    real32_T Y_Y0_g;
    char_T pad_Y_Y0_g[4];
    real32_T DelayOneStep_InitialCondition;
    char_T pad_DelayOneStep_InitialCondition[4];
    real32_T Gain1_Gain_b;
    char_T pad_Gain1_Gain_b[4];
    real32_T Gain1_Gain_mf;
    char_T pad_Gain1_Gain_mf[4];
    real32_T Gain2_Gain_o;
    char_T pad_Gain2_Gain_o[4];
    boolean_T Delay_InitialCondition_f;
    char_T pad_Delay_InitialCondition_f[7];
    boolean_T Delay_InitialCondition_o;
    char_T pad_Delay_InitialCondition_o[7];
    boolean_T DelayOneStep_InitialCondition_c;
    char_T pad_DelayOneStep_InitialCondition_c[7];
    boolean_T Delay_InitialCondition_a;
    char_T pad_Delay_InitialCondition_a[7];
    boolean_T DelayOneStep1_InitialCondition;
    char_T pad_DelayOneStep1_InitialCondition[7];
    boolean_T Constant_Value_o;
    char_T pad_Constant_Value_o[7];
    boolean_T Logic_table[16];
    boolean_T Delay_InitialCondition_ir;
    char_T pad_Delay_InitialCondition_ir[7];
    boolean_T Delay_InitialCondition_n;
    char_T pad_Delay_InitialCondition_n[7];
    boolean_T Logic_table_m[16];
    boolean_T Delay1_InitialCondition;
    char_T pad_Delay1_InitialCondition[7];
    boolean_T Delay2_InitialCondition_b;
    char_T pad_Delay2_InitialCondition_b[7];
    boolean_T Delay3_InitialCondition;
    char_T pad_Delay3_InitialCondition[7];
    boolean_T Delay_InitialCondition_j;
    char_T pad_Delay_InitialCondition_j[7];
    boolean_T Logic_table_m0[16];
    boolean_T Constant_Value_op;
    char_T pad_Constant_Value_op[7];
    boolean_T Constant_Value_g0;
    char_T pad_Constant_Value_g0[7];
    boolean_T Logic_table_k[16];
    boolean_T Logic_table_ml[16];
    boolean_T Constant2_Value_c;
    char_T pad_Constant2_Value_c[7];
    boolean_T Logic_table_b[16];
    boolean_T Logic_table_o[16];
    boolean_T Logic_table_n[16];
    boolean_T Logic_table_mh[16];
    boolean_T Logic_table_a[16];
    boolean_T Logic_table_p[16];
    boolean_T Logic_table_bt[16];
    boolean_T Logic_table_m0u[16];
    boolean_T Logic_table_ou[16];
    boolean_T Logic_table_h[16];
    boolean_T Logic_table_as[16];
    boolean_T Logic_table_i[16];
    boolean_T Logic_table_py[16];
    boolean_T Logic_table_c[16];
    boolean_T Logic_table_pp[16];
    boolean_T Logic_table_hk[16];
    boolean_T Logic_table_f[16];
    boolean_T Logic_table_j[16];
    boolean_T Logic_table_pi[16];
    boolean_T Constant_Value_h2;
    char_T pad_Constant_Value_h2[7];
    boolean_T Logic_table_bw[16];
    boolean_T Logic_table_nb[16];
    boolean_T Logic_table_fi[16];
    boolean_T Logic_table_mh5[16];
    boolean_T Logic_table_kk[16];
    boolean_T Logic_table_fe[16];
    boolean_T Logic_table_on[16];
    boolean_T Logic_table_ig[16];
    boolean_T Logic_table_f5[16];
    boolean_T Logic_table_or[16];
    boolean_T Logic_table_d[16];
    boolean_T Logic_table_kr[16];
    boolean_T Logic_table_l[16];
    boolean_T Logic_table_az[16];
    boolean_T Logic_table_cp[16];
    boolean_T Delay_InitialCondition_h;
    char_T pad_Delay_InitialCondition_h[7];
    boolean_T Logic_table_g[16];
    boolean_T Logic_table_iv[16];
    boolean_T Logic_table_bk[16];
    boolean_T Logic_table_b4[16];
    boolean_T Logic_table_lp[16];
    boolean_T Logic_table_ho[16];
    boolean_T Logic_table_ja[16];
    boolean_T Logic_table_e[16];
    boolean_T Logic_table_fj[16];
    boolean_T Logic_table_fg[16];
    boolean_T Logic_table_nr[16];
    boolean_T Constant2_Value_d;
    char_T pad_Constant2_Value_d[7];
    boolean_T Constant2_Value_o;
    char_T pad_Constant2_Value_o[7];
    boolean_T Constant_Value_e;
    char_T pad_Constant_Value_e[7];
    rtP_AltitudeChoice_A380PrimComputerFg_T AltitudeChoice_g;
    rtP_AltitudeChoice_A380PrimComputerFg_T AltitudeChoice_m;
    rtP_Subsystem_A380PrimComputerFg_T Subsystem1;
    rtP_Subsystem_A380PrimComputerFg_T Subsystem4_c;
    rtP_Subsystem_A380PrimComputerFg_T Subsystem_c;
    rtP_Subsystem_A380PrimComputerFg_T Subsystem3;
    rtP_Subsystem_A380PrimComputerFg_T Subsystem4;
    rtP_SRFlipFlopwithSyncInput_A380PrimComputerFg_T SRFlipFlopwithSyncInput_d;
    rtP_SRFlipFlopwithSyncInput_A380PrimComputerFg_T SRFlipFlopwithSyncInput;
    rtP_Subsystem_A380PrimComputerFg_T Subsystem_n;
    rtP_Subsystem_A380PrimComputerFg_T Subsystem;
  } __attribute__((aligned(8)));

  A380PrimComputerFg(A380PrimComputerFg const&) = delete;
  A380PrimComputerFg& operator= (A380PrimComputerFg const&) & = delete;
  A380PrimComputerFg(A380PrimComputerFg &&) = delete;
  A380PrimComputerFg& operator= (A380PrimComputerFg &&) = delete;
  ExternalInputs_A380PrimComputerFg_T A380PrimComputerFg_U;
  ExternalOutputs_A380PrimComputerFg_T A380PrimComputerFg_Y;
  void initialize();
  void step();
  static void terminate();
  A380PrimComputerFg();
  ~A380PrimComputerFg();
 private:
  BlockIO_A380PrimComputerFg_T A380PrimComputerFg_B;
  D_Work_A380PrimComputerFg_T A380PrimComputerFg_DWork;
  static Parameters_A380PrimComputerFg_T A380PrimComputerFg_P;
  static void A380PrimComputerFg_MATLABFunction(const base_arinc_429 *rtu_u, real32_T rtu_default, real32_T *rty_y);
  static void A380PrimComputerFg_LeadLagFilter_Reset(rtDW_LeadLagFilter_A380PrimComputerFg_T *localDW);
  static void A380PrimComputerFg_LeadLagFilter(real_T rtu_U, real_T rtu_C1, real_T rtu_C2, real_T rtu_C3, real_T rtu_C4,
    real_T rtu_dt, real_T *rty_Y, rtDW_LeadLagFilter_A380PrimComputerFg_T *localDW);
  static void A380PrimComputerFg_LagFilter_Reset(rtDW_LagFilter_A380PrimComputerFg_T *localDW);
  static void A380PrimComputerFg_LagFilter(real_T rtu_U, real_T rtu_C1, real_T rtu_dt, real_T *rty_Y,
    rtDW_LagFilter_A380PrimComputerFg_T *localDW);
  static void A380PrimComputerFg_LagFilter_j_Reset(rtDW_LagFilter_A380PrimComputerFg_b_T *localDW);
  static void A380PrimComputerFg_LagFilter_b(real32_T rtu_U, real_T rtu_C1, real_T rtu_dt, real32_T *rty_Y,
    rtDW_LagFilter_A380PrimComputerFg_b_T *localDW);
  static void A380PrimComputerFg_Voter1(real_T rtu_u1, real_T rtu_u2, real_T rtu_u3, real_T *rty_Y);
  static void A380PrimComputerFg_MATLABFunction_h(const base_arinc_429 *rtu_u, boolean_T *rty_y);
  static void A380PrimComputerFg_MATLABFunction_e(const base_arinc_429 *rtu_u, real_T rtu_bit, uint32_T *rty_y);
  static void A380PrimComputerFg_APEngagedLogic(boolean_T rtu_pulsedFcuButton, boolean_T rtu_prevApEngaged, boolean_T
    rtu_groundEngineStartCondition, boolean_T rtu_dualApDiscCondition, boolean_T rtu_groundGa, boolean_T
    rtu_envelopeInhibition, boolean_T rtu_apFdSpecificCondition, const prim_outputs *rtu_apFdSpecificCondition_l,
    boolean_T *rty_engagementCondition, boolean_T *rty_disengagementCondition, boolean_T *rty_ap_inop);
  static void A380PrimComputerFg_MATLABFunction_o_Reset(rtDW_MATLABFunction_A380PrimComputerFg_k_T *localDW);
  static void A380PrimComputerFg_MATLABFunction_c(boolean_T rtu_u, boolean_T rtu_isRisingEdge, boolean_T *rty_y,
    rtDW_MATLABFunction_A380PrimComputerFg_k_T *localDW);
  static void A380PrimComputerFg_Subsystem_Init(boolean_T rtp_initial_condition, rtDW_Subsystem_A380PrimComputerFg_T
    *localDW);
  static void A380PrimComputerFg_Subsystem_Reset(boolean_T rtp_initial_condition, rtDW_Subsystem_A380PrimComputerFg_T
    *localDW);
  static void A380PrimComputerFg_Subsystem(boolean_T rtu_S, boolean_T rtu_R, boolean_T rtu_externalActive, boolean_T
    rtu_syncEnable, boolean_T rty_Q[2], rtDW_Subsystem_A380PrimComputerFg_T *localDW, rtP_Subsystem_A380PrimComputerFg_T
    *localP);
  static void A380PrimComputerFg_MATLABFunction_e_Reset(rtDW_MATLABFunction_A380PrimComputerFg_l_T *localDW);
  static void A380PrimComputerFg_MATLABFunction_hf(boolean_T rtu_u, real_T rtu_Ts, boolean_T *rty_y, real_T
    rtp_isRisingEdge, real_T rtp_retriggerable, real_T rtp_triggerDuration, rtDW_MATLABFunction_A380PrimComputerFg_l_T
    *localDW);
  static void A380PrimComputerFg_MATLABFunction_em_Reset(rtDW_MATLABFunction_A380PrimComputerFg_b_T *localDW);
  static void A380PrimComputerFg_MATLABFunction_a(boolean_T rtu_u, real_T rtu_Ts, boolean_T rtu_isRisingEdge, real_T
    rtu_timeDelay, boolean_T *rty_y, rtDW_MATLABFunction_A380PrimComputerFg_b_T *localDW);
  static void A380PrimComputerFg_SRFlipFlopwithSyncInput_Init(boolean_T rtp_initial_condition,
    rtDW_SRFlipFlopwithSyncInput_A380PrimComputerFg_T *localDW);
  static void A380PrimComputerFg_SRFlipFlopwithSyncInput_Reset(boolean_T rtp_initial_condition,
    rtDW_SRFlipFlopwithSyncInput_A380PrimComputerFg_T *localDW);
  static void A380PrimComputerFg_SRFlipFlopwithSyncInput(boolean_T rtu_S, boolean_T rtu_R, boolean_T rtu_externalActive,
    boolean_T rtu_syncEnable, boolean_T rty_Q[2], rtDW_SRFlipFlopwithSyncInput_A380PrimComputerFg_T *localDW,
    rtP_SRFlipFlopwithSyncInput_A380PrimComputerFg_T *localP);
  static void A380PrimComputerFg_MATLABFunction_j(const base_arinc_429 *rtu_u, boolean_T *rty_y);
  static void A380PrimComputerFg_FCUKnobTurnsUnpack(real32_T rtu_y, int8_T *rty_value, int8_T rtp_field);
  static void A380PrimComputerFg_MATLABFunction1_Reset(rtDW_MATLABFunction1_A380PrimComputerFg_T *localDW);
  static void A380PrimComputerFg_MATLABFunction1(const prim_outputs *rtu_in, boolean_T rtu_set_dashes, boolean_T
    rtu_set_selection, boolean_T *rty_dashes, rtDW_MATLABFunction1_A380PrimComputerFg_T *localDW);
  static void A380PrimComputerFg_MATLABFunction_a5(const base_arinc_429 *rtu_u, real32_T *rty_y);
  static void A380PrimComputerFg_AltitudeChoice(const base_arinc_429 *rtu_altitude_standard_ft, const base_arinc_429
    *rtu_altitude_corrected_1, const base_arinc_429 *rtu_altitude_corrected_2, boolean_T rtu_useApFdSource1, const
    prim_outputs *rtu_in_Inport_5, base_arinc_429 *rty_alt_ind, rtP_AltitudeChoice_A380PrimComputerFg_T *localP);
  static void A380PrimComputerFg_betaestimation1(real32_T rtu_Vcas_kn, real32_T rtu_n_y_g, real_T rtu_zeta_deg, real_T
    rtu_gross_weight_kg, real32_T *rty_beta);
  A380FgOuterLoops LawMDLOBJ1;
  A380FgOuterLoops Law1MDLOBJ2;
};

#endif

