...
void FlightDataRecorder::update(Prim prims[3],
                                Sec secs[3],
                                Fac facs[2],
  ...
  // PRIMs
  for (int i = 0; i < 3; ++i) {
    auto bus_outputs = prims[i].getBusOutputs();
    fileStream->write((char*)(&bus_outputs), sizeof(bus_outputs));
    auto discrete_outputs = prims[i].getDiscreteOutputs();
    fileStream->write((char*)(&discrete_outputs), sizeof(discrete_outputs));
    auto analog_outputs = prims[i].getAnalogOutputs();
