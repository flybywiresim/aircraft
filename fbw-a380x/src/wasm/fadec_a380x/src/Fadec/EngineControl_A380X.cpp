...
void EngineControl_A380X::update() {
  ...
  for (int engine = 1; engine <= 4; engine++) {
    FLOAT64 engineState, timer, simN1, simN2, simCorrectedN1, correctedFuelFlow;
    // deltaN2
    ...
    switch (engine) {
      case 1:
        ...
        engineStateMachine(1, simData.miscSimDataPtr->data().engineIgniter1, simData.miscSimDataPtr->data().engineStarter1, simN2,
                           this->idleN3, pressureAltitude, ambientTemperature);
        break;
      case 2:
...
void EngineControl_A380X::engineStateMachine(int engine,
  ...
  switch (engine) {
    case 1:
      engineState = simData.engineStateDataPtr->data().engine1State;
      egtFbw = simData.engineEgtDataPtr->data().engine1Egt;
      break;
    case 2:
      engineState = simData.engineStateDataPtr->data().engine2State;
      egtFbw = simData.engineEgtDataPtr->data().engine2Egt;
      break;
    case 3:
      engineState = simData.engineStateDataPtr->data().engine3State;
      egtFbw = simData.engineEgtDataPtr->data().engine3Egt;
      break;
    case 4:
      engineState = simData.engineStateDataPtr->data().engine4State;
      egtFbw = simData.engineEgtDataPtr->data().engine4Egt;
      break;
  }
  ...
  if (engineState == 4 || engineState == 14) {
    if (engineIgniter == 2 && engineStarter == 1) {
      engineState = 3;
      resetTimer = 1;
    } else if (engineStarter == 0 && simN2 < 0.05 && egtFbw <= ambientTemperature) {
      engineState = 0;
      resetTimer = 1;
  ...
  switch (engine) {
    case 1:
      simData.engineStateDataPtr->data().engine1State = engineState;
      if (resetTimer == 1) {
        simData.engineTimerDataPtr->data().engine1Timer = 0;
      }
      break;
    case 2:
      simData.engineStateDataPtr->data().engine2State = engineState;
      if (resetTimer == 1) {
        simData.engineTimerDataPtr->data().engine2Timer = 0;
      }
      break;
    case 3:
      simData.engineStateDataPtr->data().engine3State = engineState;
      if (resetTimer == 1) {
        simData.engineTimerDataPtr->data().engine3Timer = 0;
      }
      break;
    case 4:
      simData.engineStateDataPtr->data().engine4State = engineState;
      if (resetTimer == 1) {
        simData.engineTimerDataPtr->data().engine4Timer = 0;
      }
      break;
...
void EngineControl_A380X::engineStartProcedure(int engine,
  ...
  // clang-format off
  // TODO: Maybe put these into SimData

   FLOAT64* fuelUsedEngine[4] = {
    &simData.fuelUsedEngineDataPtr->data().fuelUsedEngine1,
    &simData.fuelUsedEngineDataPtr->data().fuelUsedEngine2,
    &simData.fuelUsedEngineDataPtr->data().fuelUsedEngine3,
    &simData.fuelUsedEngineDataPtr->data().fuelUsedEngine4
  };

  FLOAT64* engineTimer[4] = {
    &simData.engineTimerDataPtr->data().engine1Timer,
    &simData.engineTimerDataPtr->data().engine2Timer,
    &simData.engineTimerDataPtr->data().engine3Timer,
    &simData.engineTimerDataPtr->data().engine4Timer
  };

  FLOAT64* engineCorrectedN2[4] = {
    &simData.simEngineCorrectedN2DataPtr->data().engine1CorrectedN2,
    &simData.simEngineCorrectedN2DataPtr->data().engine2CorrectedN2,
    &simData.simEngineCorrectedN2DataPtr->data().engine3CorrectedN2,
    &simData.simEngineCorrectedN2DataPtr->data().engine4CorrectedN2
  };

  FLOAT64* engineN2[4] = {
    &simData.simEngineN2DataPtr->data().engine1N2,
    &simData.simEngineN2DataPtr->data().engine2N2,
    &simData.simEngineN2DataPtr->data().engine3N2,
    &simData.simEngineN2DataPtr->data().engine4N2
  };

  FLOAT64* engineEgt[4] = {
    &simData.engineEgtDataPtr->data().engine1Egt,
    &simData.engineEgtDataPtr->data().engine2Egt,
    &simData.engineEgtDataPtr->data().engine3Egt,
    &simData.engineEgtDataPtr->data().engine4Egt
  };

  FLOAT64* engineN3[4] = {
    &simData.engineN3DataPtr->data().engine1N3,
    &simData.engineN3DataPtr->data().engine2N3,
    &simData.engineN3DataPtr->data().engine3N3,
    &simData.engineN3DataPtr->data().engine4N3
  };

  FLOAT64* engineN1[4] = {
    &simData.engineN1DataPtr->data().engine1N1,
    &simData.engineN1DataPtr->data().engine2N1,
    &simData.engineN1DataPtr->data().engine3N1,
    &simData.engineN1DataPtr->data().engine4N1
  };

  FLOAT64* engineFf[4] = {
    &simData.engineFuelFlowDataPtr->data().engine1Ff,
    &simData.engineFuelFlowDataPtr->data().engine2Ff,
    &simData.engineFuelFlowDataPtr->data().engine3Ff,
    &simData.engineFuelFlowDataPtr->data().engine4Ff
  };

  FLOAT64* engineStates[4] = {
    &simData.engineStateDataPtr->data().engine1State,
    &simData.engineStateDataPtr->data().engine2State,
    &simData.engineStateDataPtr->data().engine3State,
    &simData.engineStateDataPtr->data().engine4State
  };

  FLOAT64* engineOilTemperature[4] = {
    &simData.simEngineOilTempDataPtr->data().engine1OilTemperature,
    &simData.simEngineOilTempDataPtr->data().engine2OilTemperature,
    &simData.simEngineOilTempDataPtr->data().engine3OilTemperature,
    &simData.simEngineOilTempDataPtr->data().engine4OilTemperature
  };
  // clang-format on

  if (timer < 1.7) {
    if (msfsHandlerPtr->getSimOnGround()) {
      *fuelUsedEngine[engine - 1] = 0;
      simData.fuelUsedEngineDataPtr->writeDataToSim();
    }
    *engineTimer[engine - 1] = timer + deltaTime;
    simData.engineTimerDataPtr->writeDataToSim();

    *engineCorrectedN2[engine - 1] = 0;
    simData.simEngineCorrectedN2DataPtr->writeDataToSim();
  } else {
    preN3Fbw = *engineN2[engine - 1];
    preEgtFbw = *engineEgt[engine - 1];
    ...
    *engineN3[engine - 1] = newN3Fbw;
    *engineN2[engine - 1] = newN3Fbw + 0.7;
    *engineN1[engine - 1] = Polynomial_A380X::startN1(newN3Fbw, idleN3, idleN1);
    *engineFf[engine - 1] = Polynomial_A380X::startFF(newN3Fbw, idleN3, idleFF);
    ...
    if (engineState == 3) {
      if ((std::abs)(startEgtFbw - preEgtFbw) <= 1.5) {
        *engineStates[engine - 1] = 1;
        *engineEgt[engine - 1] = startEgtFbw;
      } else if (startEgtFbw > preEgtFbw) {
        *engineEgt[engine - 1] = preEgtFbw + (0.75 * deltaTime * (idleN3 - newN3Fbw));
      } else {
        *engineEgt[engine - 1] = shutdownEgtFbw;
      }
    } else {
      *engineEgt[engine - 1] = startEgtFbw;
    }
    ...
    *engineOilTemperature[engine - 1] = oilTemperature;
    simData.simEngineOilTempDataPtr->writeDataToSim();
  }
}
...
void EngineControl_A380X::engineShutdownProcedure(int engine, FLOAT64 ambientTemperature, FLOAT64 simN1, FLOAT64 deltaTime, FLOAT64 timer) {
  ...
  // This is much short and less code duplication than a switch statement
  FLOAT64* simDataEngineTimer[4] = {&simData.engineTimerDataPtr->data().engine1Timer, &simData.engineTimerDataPtr->data().engine2Timer,
                                    &simData.engineTimerDataPtr->data().engine3Timer, &simData.engineTimerDataPtr->data().engine4Timer};
  FLOAT64* simDataEngineN1[4] = {&simData.engineN1DataPtr->data().engine1N1, &simData.engineN1DataPtr->data().engine2N1,
                                 &simData.engineN1DataPtr->data().engine3N1, &simData.engineN1DataPtr->data().engine4N1};
  FLOAT64* simDataEngineN2[4] = {&simData.engineN2DataPtr->data().engine1N2, &simData.engineN2DataPtr->data().engine2N2,
                                 &simData.engineN2DataPtr->data().engine3N2, &simData.engineN2DataPtr->data().engine4N2};
  FLOAT64* simDataEngineN3[4] = {&simData.engineN3DataPtr->data().engine1N3, &simData.engineN3DataPtr->data().engine2N3,
                                 &simData.engineN3DataPtr->data().engine3N3, &simData.engineN3DataPtr->data().engine4N3};
  FLOAT64* simDataEngineEgt[4] = {&simData.engineEgtDataPtr->data().engine1Egt, &simData.engineEgtDataPtr->data().engine2Egt,
                                  &simData.engineEgtDataPtr->data().engine3Egt, &simData.engineEgtDataPtr->data().engine4Egt};

  if (timer < 1.8) {
    *simDataEngineTimer[engine - 1] = timer + deltaTime;
    simData.engineTimerDataPtr->writeDataToSim();
  } else {
    preN1Fbw = *simDataEngineN1[engine - 1];
    preN3Fbw = *simDataEngineN3[engine - 1];
    preEgtFbw = *simDataEngineEgt[engine - 1];
    ...
    *simDataEngineN1[engine - 1] = newN1Fbw;
    simData.engineN1DataPtr->writeDataToSim();
    *simDataEngineN2[engine - 1] = newN3Fbw + 0.7;
    simData.engineN2DataPtr->writeDataToSim();
    *simDataEngineN3[engine - 1] = newN3Fbw;
    simData.engineN3DataPtr->writeDataToSim();
    *simDataEngineEgt[engine - 1] = newEgtFbw;
    simData.engineEgtDataPtr->writeDataToSim();
  }
}
...
int EngineControl_A380X::updateFF(int engine,
  ...
  if (correctedFuelFlow < 1) {
    ...
    outFlow = (correctedFuelFlow * LBS_TO_KGS * Fadec::delta2(mach, ambientPressure) * sqrt(Fadec::theta2(mach, ambientTemperature)));
  }

  switch (engine) {
    case 1:
      simData.engineFuelFlowDataPtr->data().engine1Ff = outFlow;
      break;
    case 2:
      simData.engineFuelFlowDataPtr->data().engine2Ff = outFlow;
      break;
    case 3:
      simData.engineFuelFlowDataPtr->data().engine3Ff = outFlow;
      break;
    case 4:
      simData.engineFuelFlowDataPtr->data().engine4Ff = outFlow;
      break;
  }
  simData.engineFuelFlowDataPtr->writeDataToSim();
...
void EngineControl_A380X::updatePrimaryParameters(int engine, FLOAT64 simN1, FLOAT64 simN3) {
  switch (engine) {
    case 1:
      simData.engineN1DataPtr->data().engine1N1 = simN1;
      simData.engineN2DataPtr->data().engine1N2 = simN3 + 0.7;
      simData.engineN3DataPtr->data().engine1N3 = simN3;
      break;
    case 2:
      simData.engineN1DataPtr->data().engine2N1 = simN1;
      simData.engineN2DataPtr->data().engine2N2 = simN3 + 0.7;
      simData.engineN3DataPtr->data().engine2N3 = simN3;
      break;
    case 3:
      simData.engineN1DataPtr->data().engine3N1 = simN1;
      simData.engineN2DataPtr->data().engine3N2 = simN3 + 0.7;
      simData.engineN3DataPtr->data().engine3N3 = simN3;
      break;
    case 4:
      simData.engineN1DataPtr->data().engine4N1 = simN1;
      simData.engineN2DataPtr->data().engine4N2 = simN3 + 0.7;
      simData.engineN3DataPtr->data().engine4N3 = simN3;
      break;
  }
  simData.engineN1DataPtr->writeDataToSim();
  simData.engineN2DataPtr->writeDataToSim();
...
void EngineControl_A380X::updateEGT(int engine,
  ...
  FLOAT64* engineEgt[4]= {&simData.engineEgtDataPtr->data().engine1Egt,
                          &simData.engineEgtDataPtr->data().engine2Egt,
                          &simData.engineEgtDataPtr->data().engine3Egt,
                          &simData.engineEgtDataPtr->data().engine4Egt};
...
void EngineControl_A380X::updateFuel(FLOAT64 deltaTime) {
  ...
  const FLOAT64 leftOuterQty = simData.simFuelTankDataPtr->data().fuelTankQuantity1 * fuelWeightGallon;    // LBS
  const FLOAT64 feedOneQty = simData.simFuelTankDataPtr->data().fuelTankQuantity2 * fuelWeightGallon;      // LBS
  const FLOAT64 leftMidQty = simData.simFuelTankDataPtr->data().fuelTankQuantity3 * fuelWeightGallon;      // LBS
  const FLOAT64 leftInnerQty = simData.simFuelTankDataPtr->data().fuelTankQuantity4 * fuelWeightGallon;    // LBS
  const FLOAT64 feedTwoQty = simData.simFuelTankDataPtr->data().fuelTankQuantity5 * fuelWeightGallon;      // LBS
  const FLOAT64 feedThreeQty = simData.simFuelTankDataPtr->data().fuelTankQuantity6 * fuelWeightGallon;    // LBS
  const FLOAT64 rightInnerQty = simData.simFuelTankDataPtr->data().fuelTankQuantity7 * fuelWeightGallon;   // LBS
  const FLOAT64 rightMidQty = simData.simFuelTankDataPtr->data().fuelTankQuantity8 * fuelWeightGallon;     // LBS
  const FLOAT64 feedFourQty = simData.simFuelTankDataPtr->data().fuelTankQuantity9 * fuelWeightGallon;     // LBS
  const FLOAT64 rightOuterQty = simData.simFuelTankDataPtr->data().fuelTankQuantity10 * fuelWeightGallon;  // LBS
  const FLOAT64 trimQty = simData.simFuelTankDataPtr->data().fuelTankQuantity11 * fuelWeightGallon;        // LBS

  FLOAT64 fuelLeftOuter = 0;
  FLOAT64 fuelFeedOne = 0;
  FLOAT64 fuelLeftMid = 0;
  FLOAT64 fuelLeftInner = 0;
  FLOAT64 fuelFeedTwo = 0;
  FLOAT64 fuelFeedThree = 0;
  FLOAT64 fuelRightInner = 0;
  FLOAT64 fuelRightMid = 0;
  FLOAT64 fuelFeedFour = 0;
  FLOAT64 fuelRightOuter = 0;
  FLOAT64 fuelTrim = 0;
  ...
  if (uiFuelTamper && devState == 0) {
    ...
    fuelLeftOuter = fuelLeftOuterPre / fuelWeightGallon;    // gal
    fuelFeedOne = fuelFeedOnePre / fuelWeightGallon;        // gal
    fuelLeftMid = fuelLeftMidPre / fuelWeightGallon;        // gal
    fuelLeftInner = fuelLeftInnerPre / fuelWeightGallon;    // gal
    fuelFeedTwo = fuelFeedTwoPre / fuelWeightGallon;        // gal
    fuelFeedThree = fuelFeedThreePre / fuelWeightGallon;    // gal
    fuelRightInner = fuelRightInnerPre / fuelWeightGallon;  // gal
    fuelRightMid = fuelRightMidPre / fuelWeightGallon;      // gal
    fuelFeedFour = fuelFeedFourPre / fuelWeightGallon;      // gal
    fuelRightOuter = fuelRightOuterPre / fuelWeightGallon;  // gal
    fuelTrim = fuelTrimPre / fuelWeightGallon;              // gal

    simData.simFuelTankDataPtr->data().fuelTankQuantity1 = fuelLeftOuter;    // gal
    simData.simFuelTankDataPtr->data().fuelTankQuantity2 = fuelFeedOne;      // gal
    simData.simFuelTankDataPtr->data().fuelTankQuantity3 = fuelLeftMid;      // gal
    simData.simFuelTankDataPtr->data().fuelTankQuantity4 = fuelLeftInner;    // gal
    simData.simFuelTankDataPtr->data().fuelTankQuantity5 = fuelFeedTwo;      // gal
    simData.simFuelTankDataPtr->data().fuelTankQuantity6 = fuelFeedThree;    // gal
    simData.simFuelTankDataPtr->data().fuelTankQuantity7 = fuelRightInner;   // gal
    simData.simFuelTankDataPtr->data().fuelTankQuantity8 = fuelRightMid;     // gal
    simData.simFuelTankDataPtr->data().fuelTankQuantity9 = fuelFeedFour;     // gal
    simData.simFuelTankDataPtr->data().fuelTankQuantity10 = fuelRightOuter;  // gal
    simData.simFuelTankDataPtr->data().fuelTankQuantity11 = fuelTrim;        // gal
    ...
    FLOAT64 fuelFlowRateChange = 0;
    FLOAT64 previousFuelFlowRate = 0;
    FLOAT64 fuelBurn1 = 0;
    FLOAT64 fuelBurn2 = 0;
    FLOAT64 fuelBurn3 = 0;
    FLOAT64 fuelBurn4 = 0;
    ...
    simData.fuelPreDataPtr->data().fuelFeedOnePre = fuelFeedOne;
    simData.fuelPreDataPtr->data().fuelFeedTwoPre = fuelFeedTwo;
    simData.fuelPreDataPtr->data().fuelFeedThreePre = fuelFeedThree;
    simData.fuelPreDataPtr->data().fuelFeedFourPre = fuelFeedFour;
  ...
  if (msfsHandlerPtr->getSimulationTime() - lastFuelSaveTime > 5.0 && msfsHandlerPtr->getSimOnGround() &&
...
void EngineControl_A380X::updateThrustLimits(FLOAT64 simulationTime,
  ...
  if (flexTemp > 0) {
    flex_to =
        ThrustLimits_A380X::limitN1(0, min(16600.0, pressureAltitude), ambientTemperature, ambientPressure, flexTemp, packs, nai, wai);
    flex_ga =
        ThrustLimits_A380X::limitN1(1, min(16600.0, pressureAltitude), ambientTemperature, ambientPressure, flexTemp, packs, nai, wai);
  }
