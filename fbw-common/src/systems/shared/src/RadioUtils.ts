...
export class RadioUtils {
  ...
  /**
   * Packs a VHF frequency into the data portion of an arinc word.
   * @param frequency The MSFS Bcd32 frequency.
   * @returns The ARINC BCD encoded data.
   */
  public static packBcd32VhfFrequencyToArinc(bcd32: number): number {
  ...
  /**
   * Unpacks a VHF frequency from the data portion of an arinc word.
   * @param arincValue The BCD encoded ARINC data.
   * @returns the frequency in MSFS BCD32.
   */
  public static unpackVhfFrequencyFromArincToBcd32(arincValue: number): number {
   ...
   */
  public static unpackVhfFrequencyFromArincToBcd16(arincValue: number): number {
    return (arincValue & 0x7ffff) >> 4;
  }
  ...
  public static packVhfComFrequencyToArinc(frequency: number): number {
    return RadioUtils.packBcd32VhfFrequencyToArinc(RadioUtils.packBcd32(frequency));
  }
  ...
  public static packBcd16(frequency: number): number {
    const freq = Math.round((frequency - 1_000_000) / 10_000);
    return (
      (freq % 10 << 0) +
      (Math.trunc(freq / 10) % 10 << 4) +
      (Math.trunc(freq / 100) % 10 << 8) +
      (Math.trunc((freq / 1000) % 10) << 12)
    );
  }
  ...
  public static formatBcd32(bcd32: number, separator = '.'): string {
    ...
    // return `${(bcd32 >>> 24) & 0x7}${(bcd32 >>> 20) & 0xf}${(bcd32 >>> 16) & 0xf}.${(bcd32 >>> 12) & 0xf}${(bcd32 >>> 8) & 0xf}${(bcd32 >>> 4) & 0xf}`;
  }
  ...
  public static formatBcd16(bcd16: number, separator = '.'): string {
    ...
    // return `1${(bcd16 >>> 12) & 0xf}${(bcd16 >>> 8) & 0xf}.${(bcd16 >>> 4) & 0xf}${(bcd16 >>> 0) & 0xf}0`;
  }
  ...
  public static isValidSpacing(bcd32: number, type: RadioChannelType): boolean {
    const channelInfo = RadioUtils.CHANNEL_TYPES[type];
    return channelInfo.channels.includes(bcd32 & 0xfff);
  }

  /**
   * Count trailing zeroes.
   * @param v The 32-bit number to count.
   * @returns The number of trailing zeros.
   */
  private static ctz(v: number): number {
    let tz = 32;
    v &= -v;
    if (v) {
      tz -= 1;
