...
export class FailureGeneratorAltitude extends GenericGenerator {
  ...
  additionalInitActions(genNumber: number): void {
    this.previousAltitudeCondition[genNumber] =
      this.settings[genNumber * this.numberOfSettingsPerGenerator + this.altitudeConditionIndex];
  }

  generatorSpecificActions(genNumber: number): void {
    const altitudeCondition =
      this.settings[genNumber * this.numberOfSettingsPerGenerator + this.altitudeConditionIndex];
  ...
  conditionToTriggerFailure(genNumber: number): boolean {
    ...
    return (
      (this.altitude > failureAltitude && altitudeCondition === Direction.Climb) ||
  ...
  conditionToArm(genNumber: number): boolean {
    ...
    const altitudeCondition =
      this.settings[genNumber * this.numberOfSettingsPerGenerator + this.altitudeConditionIndex];
    return (
      (this.altitude < altitudeMin - this.resetMargin && altitudeCondition === Direction.Climb) ||
