...
export class FailureGeneratorTakeOff extends GenericGenerator {
  ...
  generatorSpecificActions(genNumber: number): void {
    const medHighTakeOffSpeed: number =
      this.settings[genNumber * this.numberOfSettingsPerGenerator + this.maxSpeedIndex];
    if (
      this.failureGeneratorArmed[genNumber] &&
      ((this.altitude >= this.failureTakeOffAltitudeEnd[genNumber] && this.gs >= medHighTakeOffSpeed) || this.gs < 1)
    ) {
  ...
  conditionToTriggerFailure(genNumber: number): boolean {
    return (
      this.rolledDiceTakeOff <
        this.settings[genNumber * this.numberOfSettingsPerGenerator + this.chancePerTakeOffIndex] &&
      ((this.altitude >= this.failureTakeOffAltitudeThreshold[genNumber] &&
        this.failureTakeOffAltitudeThreshold[genNumber] !== -1) ||
        (this.gs >= this.failureTakeOffSpeedThreshold[genNumber] &&
  ...
  additionalArmingActions(genNumber: number): void {
    const minFailureTakeOffSpeed: number =
      this.settings[genNumber * this.numberOfSettingsPerGenerator + this.minSpeedIndex];
    const medHighTakeOffSpeed: number =
      this.settings[genNumber * this.numberOfSettingsPerGenerator + this.maxSpeedIndex];
    const chanceFailureLowTakeOffRegime: number =
      this.settings[genNumber * this.numberOfSettingsPerGenerator + this.chanceLowIndex];
    const chanceFailureMediumTakeOffRegime: number =
      this.settings[genNumber * this.numberOfSettingsPerGenerator + this.chanceMediumIndex];
    const takeOffDeltaAltitudeEnd: number =
      100 * this.settings[genNumber * this.numberOfSettingsPerGenerator + this.altitudeIndex];
    const lowMedTakeOffSpeed: number =
      this.settings[genNumber * this.numberOfSettingsPerGenerator + this.mediumSpeedIndex];
