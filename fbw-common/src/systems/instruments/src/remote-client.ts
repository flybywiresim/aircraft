...
const EXCLUDED_DATA_STORAGE_KEYS = ['A32NX_NAVIGRAPH_REFRESH_TOKEN'];
...
export class RemoteClient {
  ...
  private attemptConnect(): void {
    ...
    if (simBridgeClientState !== SimBridgeClientState.CONNECTED) {
      console.log(
        `[RemoteClient](attemptConnect) Not attempting to connect, as SimBridge client is in state "${simBridgeClientState}"`,
      );
      return;
    }
    ...
    console.log(
      `[RemoteClient](attemptConnect) Attempting to connect (${this.url}). attempt #${this.connectionAttemptCount}`,
    );
    ...
    switch (msg.type) {
      ...
      case 'remoteDownloadFile':
        if (!msg.fileVfsPath.startsWith(this.fileDownloadBasePath) || msg.fileVfsPath.includes('..')) {
          throw new Error(
            `[RemoteClient](onMessage) remoteDownloadFile received with unauthorized path (${msg.fileVfsPath})`,
          );
        }
      ...
      case 'remoteEnumerateInstruments':
        Wait.awaitCondition(() => this.instruments.length !== 0, 100, 10_000)
          .then(() => {
            this.sendMessage({
      ...
      case 'remoteSetSimVarValue':
        SimVar.SetSimVarValue(msg.name, msg.unit, msg.value)
          .then(() => {
            this.sendMessage({
  ...
  private clearSubscriptionsForSubscriptionGroup(subscriptionGroupID: string, fromClientID: string): number {
    ...
    const groupSubscriptions = this.subscriptions.filter(
      (it) => it.subscriptionGroupID === subscriptionGroupID || it.fromClientID === fromClientID,
    );
  ...
  private async fetchInstrumentsMetadata(file: string): Promise<protocolV0.InstrumentMetadata[]> {
    ...
    if (json.protocolVersion !== RemoteClient.PROTOCOL_VERSION) {
      throw new Error(
        `[RemoteClient](fetchInstrumentsMetadata) protocolVersion of instruments metadata file is not ${RemoteClient.PROTOCOL_VERSION}`,
      );
    }
