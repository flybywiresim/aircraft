...
interface BatteryProps extends ComponentProps {
  bus: EventBus;
  selectedBattery: Subscribable<number>;
  annLtTestPosition: Subscribable<number>;
  textX: number;
  textY: number;
}
...
export class BatteryDisplay extends DisplayComponent<BatteryProps> {
  ...
  private readonly isBatDisplayDisabled = MappedSubject.create(
    ...
    this.props.selectedBattery,
    this.props.annLtTestPosition,
  ...
  onAfterRender(node: VNode) {
    ...
    this.subs.push(
      this.props.selectedBattery.sub((switchPosition) => {
        // This will only fire on the A380X, since the switch index doesn't actually change on the A32NX.

        console.log(switchPosition);

        if (
          this.previousBatSelectorPosition === BatSelectorPositions.OFF &&
          !(this.props.annLtTestPosition.get() === 0 && this.dcBus2PoweredConsumer.get())
        ) {
  ...
  render(): VNode {
    return (
      <svg class={{ 'bat-svg': true, off: this.isBatDisplayDisabled }} viewBox="0 0 200 100">
        <text x={this.props.textX} y={this.props.textY}>
