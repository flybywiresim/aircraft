...
export const FailureGeneratorCardTemplateUI: React.FC<FailureGeneratorCardTemplateUIProps> = ({
  genNumber,
  failureGenData,
  failureGenContext,
}) => {
  ...
  const eraseGenerator: (
    ...
    if (genID === generatorNumber - 1) {
      generatorSettings.settings.splice(
        genID * generatorSettings.numberOfSettingsPerGenerator,
        generatorSettings.numberOfSettingsPerGenerator,
      );
    ...
    <div className="border-theme-accent m-1 flex flex-row justify-between rounded-md border-2 border-solid px-2 pt-2 text-center">
      <div className="align-left mr-4 flex max-w-[86%] grow flex-col text-left">
        ...
        <FailureShortList
          ...
          reducedAtaChapterNumbers={failureGenContext.reducedAtaChapterNumbers}
        />
      </div>
...
const FailureShortList: React.FC<FailureShortListProps> = ({
  failureGenContext,
  uniqueID,
  ...
  if (listOfSelectedFailures.length === 0)
    return <div className="bg-theme-accent mb-1 rounded-md p-1">{t('Failures.Generators.NoFailure')}</div>;
  ...
  const subSetOfChapters = chaptersFullySelected.slice(
    0,
    Math.min(maxNumberOfFailureToDisplay, chaptersFullySelected.length),
  );
  const subSetOfSelectedFailures = listOfSelectedFailures.slice(
    0,
    Math.min(maxNumberOfFailureToDisplay - subSetOfChapters.length, listOfSelectedFailures.length),
  );
  const chaptersToDisplay = subSetOfChapters.map((chapter) => (
    <div className="bg-theme-accent mb-1 grow rounded-md p-1">{AtaChaptersTitle[chapter]}</div>
  ));

  const singleFailuresToDisplay = subSetOfSelectedFailures.map((failure) => (
    <div className="bg-theme-accent mb-1 grow rounded-md p-1">{failure.name}</div>
  ));
