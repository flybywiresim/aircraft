...
import {
  FailuresAtOnceIndex,
  MaxFailuresIndex,
} from 'instruments/src/EFB/Failures/FailureGenerators/EFBFailureGeneratorsUI';
...
import { ArmingModeIndex } from '../../../../../../../../fbw-common/src/systems/shared/src/failures/RandomFailureGen';
...
export const RearmSettingsUI: React.FC<RearmSettingsUIProps> = ({
  generatorSettings,
  genID,
  setNewSetting,
  failureGenContext,
          ...
          if (button.setting !== 'Take-Off' || generatorSettings.disableTakeOffRearm === false) {
            return (
              <SelectItem
                ...
                selected={
                  generatorSettings.settings[genID * generatorSettings.numberOfSettingsPerGenerator + 0] ===
                  button.settingVar
                }
              >
                {t(button.name)}
...
export const FailureGeneratorDetailsModalUI: React.FC<{ failureGenContext: FailureGenContext }> = ({
  failureGenContext,
}) => {
  const bus = useEventBus();
  ...
  if (genNumber !== undefined && !Number.isNaN(genNumber)) {
    const numberOfSelectedFailures = findGeneratorFailures(
      allFailures,
      failureGenContext.generatorFailuresGetters,
      failureGenContext.modalContext.genUniqueID,
    ).length;
    const sample =
      failureGenContext.modalContext.failureGenData.settings[
        genNumber * failureGenContext.modalContext.failureGenData.numberOfSettingsPerGenerator + ArmingModeIndex
      ];
    if (sample !== undefined && !Number.isNaN(sample)) {
      const setNewNumberOfFailureSetting = (newSetting: number, generatorSettings: FailureGenData, genID: number) => {
        ...
        settings[genID * generatorSettings.numberOfSettingsPerGenerator + MaxFailuresIndex] = Math.max(
          settings[genID * generatorSettings.numberOfSettingsPerGenerator + MaxFailuresIndex],
          newSetting,
        );
        updateSettings(
          ...
          <ScrollableContainer height={48} nonRigid>
            <div className="divide-theme-accent w-full divide-y-2">
              ...
              <FailureGeneratorSingleSettingShortcut
                ...
                value={
                  failureGenContext.modalContext.failureGenData.settings[
                    genNumber * failureGenContext.modalContext.failureGenData.numberOfSettingsPerGenerator +
                      FailuresAtOnceIndex
                  ]
                }
                mult={1}
              ...
              <FailureGeneratorSingleSetting
                ...
                failureGenContext={failureGenContext}
              />
              {failureGenContext.modalContext.failureGenData.generatorSettingComponents(
                genNumber,
                failureGenContext.modalContext,
                failureGenContext,
              )}
            </div>
          </ScrollableContainer>
          <NextButton failureGenContext={failureGenContext} />
        </div>
      );
    } else {
      closeModal();
...
export function ArmedState(generatorSettings: FailureGenData, genNumber: number) {
  const readyDisplay: boolean =
    genNumber < generatorSettings.armedState?.length ? generatorSettings.armedState[genNumber] : false;
  switch (generatorSettings.settings[generatorSettings.numberOfSettingsPerGenerator * genNumber + ArmingModeIndex]) {
    case 0:
      return (
        <TooltipWrapper
          text={`${t('Failures.Generators.ToolTipOff')} - ${readyDisplay ? t('Failures.Generators.ToolTipReady') : t('Failures.Generators.ToolTipStandby')}`}
        >
          <ToggleOff size={20} />
        </TooltipWrapper>
      );
    case 1:
      return (
        <TooltipWrapper
          text={`${t('Failures.Generators.ToolTipOnce')} - ${readyDisplay ? t('Failures.Generators.ToolTipReady') : t('Failures.Generators.ToolTipStandby')}`}
        >
          <Icon1Circle size={20} />
        </TooltipWrapper>
      );
    ...
    case 3:
      return (
        <TooltipWrapper
          text={`${t('Failures.Generators.ToolTipRepeat')} - ${readyDisplay ? t('Failures.Generators.ToolTipReady') : t('Failures.Generators.ToolTipStandby')}`}
        >
          <Repeat size={20} />
        </TooltipWrapper>
      );
...
export const FailureGeneratorChoiceSetting: React.FC<FailureGeneratorChoiceSettingProps> = ({
  title,
  multiChoice,
  generatorSettings,
  genIndex,
...
export const FailureGeneratorText: React.FC<FailureGeneratorTextProps> = ({ title, unit, text }) => (
  <SettingItem name={`${title}${unit === '' ? '' : ` (${unit})`}`}>
    <div className="w-32 flex-1 break-keep text-center font-mono text-2xl">{text}</div>
  </SettingItem>
);
