...
import {
  FailureGenContext,
  ModalGenType,
  findGeneratorFailures,
  sendFailurePool,
} from 'instruments/src/EFB/Failures/FailureGenerators/EFBRandomFailureGen';
import {
  getGeneratorFailurePool,
  setSelectedFailure,
} from 'instruments/src/EFB/Failures/FailureGenerators/EFBFailureSelectionFunctions';
...
const FailureAtaList: React.FC<FailureAtaListProps> = ({ failureGenContext, chapter, generatorFailureTable }) => {
  ...
  const selectOneFailure = (failure: Failure, failureGenContext: FailureGenContext, active: boolean): void => {
    ...
    sendFailurePool(
      failureGenContext.modalContext.genLetter,
  ...
  const ATAList: JSX.Element[] = allFailures.map<JSX.Element>((failure) => {
    if (failure.ata === chapter) {
      const active =
        generatorFailureTable.find((genFailure) => failure.identifier === genFailure.identifier) !== undefined;

      return (
        <div className="justify flex flex-row pt-2">
          <Toggle
            value={active}
            onToggle={() => {
...
export const GeneratorFailureSelection: React.FC<GeneratorFailureSelectionProps> = ({
  failureGenContext,
}): JSX.Element => {
  const { allFailures } = useFailuresOrchestrator();
  ...
  const generatorFailureTable: Failure[] = findGeneratorFailures(
    allFailures,
    failureGenContext.generatorFailuresGetters,
    failureGenContext.modalContext.genUniqueID,
  );
    ...
    <div className="bg-theme-body border-theme-accent flex w-3/4 flex-col items-stretch justify-between border-2 px-8 py-2">
      <div className="bg-theme-body flex flex-row items-start justify-between ">
        <div className="space-x-3 text-left">
          <h1 className="font-bold text-current">{t('Failures.Generators.FailureSelection')}</h1>
        </div>
        <div />
        <div
      ...
      <ScrollableContainer height={48}>
        <div>
          <div className="ml-10 flex flex-row place-items-center justify-start pt-4">
            <div
            ...
            <div>
              <h1>{t('Failures.Generators.AllSystems')}</h1>
            </div>
          </div>
        </div>
            ...
            return (
              <div>
                <div className="flex flex-row place-items-center justify-start pt-4">
                  ...
                  <div>
                    <h1>{AtaChaptersTitle[chapter]}</h1>
                  </div>
                </div>
                <FailureAtaList
                  failureGenContext={failureGenContext}
                  generatorFailureTable={generatorFailureTable}
                  chapter={chapter}
                />
              </div>
            );
