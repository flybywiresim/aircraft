...
export const FailureGeneratorsUI = () => {
  ...
  useEffect(() => {
    ...
    const context: ModalContext = {
      failureGenData: settings.allGenSettings.get(chosenGen),
      genNumber: genNumberNewGen,
      genUniqueID,
      genLetter,
  ...
  const sample =
    settings.modalContext?.failureGenData?.settings[
      settings.modalContext?.genNumber * settings.modalContext?.failureGenData?.numberOfSettingsPerGenerator +
        ArmingModeIndex
    ];
  ...
  const failureGeneratorAdd = (generatorSettings: FailureGenData) => {
    ...
    for (let i = 0; i < generatorSettings.settings.length / generatorSettings.numberOfSettingsPerGenerator; i++) {
      if (
        generatorSettings.settings[i * generatorSettings.numberOfSettingsPerGenerator + ArmingModeIndex] === -1 &&
        !didFindADisabledGen
      ) {
        for (let j = 0; j < generatorSettings.numberOfSettingsPerGenerator; j++) {
          generatorSettings.settings[i * generatorSettings.numberOfSettingsPerGenerator + j] =
            generatorSettings.additionalSetting[j];
        }
    ...
    if (didFindADisabledGen === false) {
      if (
        generatorSettings.settings.length % generatorSettings.numberOfSettingsPerGenerator !== 0 ||
        generatorSettings.settings.length === 0
      ) {
        // console.info(`Number of parameters inconsistent, reseting instances of gen ${generatorSettings.uniqueGenPrefix}`);
        updateSettings(
          generatorSettings.additionalSetting,
          generatorSettings.setSetting,
          bus,
          generatorSettings.uniqueGenPrefix,
        );
        genNumber = 0;
      } else {
        updateSettings(
          generatorSettings.settings.concat(generatorSettings.additionalSetting),
          generatorSettings.setSetting,
          bus,
          generatorSettings.uniqueGenPrefix,
        );
    ...
    sendFailurePool(
      generatorSettings.uniqueGenPrefix,
      genNumber,
      getGeneratorFailurePool(generatorSettings.uniqueGenPrefix + genNumber.toString(), Array.from(allFailures)),
      bus,
    );
  ...
  useEffect(() => {
    ...
    const sub1 = bus
      .getSubscriber<FailureGenFeedbackEvent>()
      .on('expectedMode')
      .handle(({ generatorType, mode }) => {
        treatExpectedModeEvent(generatorType, mode);
      });
    const sub2 = bus
      .getSubscriber<FailureGenFeedbackEvent>()
      .on('armingDisplayStatus')
      .handle(({ generatorType, status }) => {
      ...
      <div className="flex flex-col">
        <div className="flex flex-1 flex-row justify-between">
          <div className="flex flex-1 flex-row justify-start space-x-4 py-2">
            <h2 className="flex-none">{`${t('Failures.Generators.GeneratorToAdd')}:`}</h2>
            <SelectInput
        ...
        <ScrollableContainer height={48}>
          <div className="grid grid-flow-row grid-cols-3">{generatorsCardList(settings)}</div>
        </ScrollableContainer>
      </div>
    </>
  );
  ...
  for (const [, generatorSetting] of settings.allGenSettings) {
    ...
    for (let i = 0; i < nbGenerator; i++) {
      if (generatorSetting.settings[i * generatorSetting.numberOfSettingsPerGenerator + ArmingModeIndex] !== -1) {
        temp.push(
          <FailureGeneratorCardTemplateUI
            genNumber={i}
            failureGenData={generatorSetting}
            failureGenContext={settings}
          />,
        );
      }
    }
