...
export const ChartViewer = () => {
    ...
    useEffect(() => {
        navigraphCharts.getChartImage({ chart: { image_day_url: chartLinks.light, image_night_url: chartLinks.dark } as Chart, theme: 'light' }).then((blob) => {
            console.log('fetch for some fucking reason');
            setChartLightBlob(blob);
        });
        navigraphCharts.getChartImage({ chart: { image_day_url: chartLinks.light, image_night_url: chartLinks.dark } as Chart, theme: 'dark' }).then((blob) => {
            console.log('fetch for some fucking reason');
            setChartDarkBlob(blob);
        });
    }, [chartLinks]);
                                ...
                                >
                                    {(chartLinks && provider === 'NAVIGRAPH') && (
                                        ...
                                        >
                                            ...
                                            {' '}
                                            {navigraphAuth.user?.preferred_username ?? '<not logged in>'}
                                        </p>
                                    )}

                                    { (aircraftIconVisible && boundingBox) && (
                                        <svg ref={planeRef} viewBox={`0 0 ${boundingBox.planview.pixels.x2 - boundingBox.planview.pixels.x1} ${Math.abs(boundingBox.planview.pixels.y1 - boundingBox.planview.pixels.y2)}`} className="absolute left-0 top-0 z-30">
                                            <g
