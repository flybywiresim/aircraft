...
export const Data = ({ className }: { className: string }) => {
  ...
  const currentAltitudeSyncEnabled =
    useAppSelector((state) => state.todCalculator.currentAltitudeMode) === TOD_INPUT_MODE.AUTO;
  const calculationInputSyncEnabled =
    useAppSelector((state) => state.todCalculator.calculationInputMode) === TOD_INPUT_MODE.AUTO;
    ...
    if (!inputValid(calculationType!, syncedInput)) {
      ...
      dispatch(
        setTodData({
          ...
          calculation: { input: 3, type: TOD_CALCULATION_TYPE.FLIGHT_PATH_ANGLE },
        }),
      );
      return;
    }
  ...
  const calculationTypes = [
    {
      label: `${t('Performance.TopOfDescent.Data.Distance')}`,
      placeholder: `${t('Performance.TopOfDescent.Data.UnitNM')}`,
      type: TOD_CALCULATION_TYPE.DISTANCE,
      syncValue: distance,
      negativeValue: false,
    },
    {
      label: `${t('Performance.TopOfDescent.Data.VerticalSpeed')}`,
      placeholder: `${t('Performance.TopOfDescent.Data.UnitFtMin')}`,
      type: TOD_CALCULATION_TYPE.VERTICAL_SPEED,
      syncValue: verticalSpeed,
      negativeValue: true,
    },
    {
      label: `${t('Performance.TopOfDescent.Data.Angle')}`,
      placeholder: `${t('Performance.TopOfDescent.Data.UnitAngleDegrees')}`,
      type: TOD_CALCULATION_TYPE.FLIGHT_PATH_ANGLE,
      syncValue: pitchAngle,
      negativeValue: true,
    },
  ];
  ...
  return (
    <Card
      title={t('Performance.TopOfDescent.Data.Title')}
      childrenContainerClassName="flex-1 flex flex-col justify-start space-y-4"
      className={className}
    >
      <div>
          ...
          <TooltipWrapper
            text={
              currentAltitudeSyncEnabled
                ? t('Performance.TopOfDescent.TT.DesynchronizeInputWithAircraftsCurrentAltitude')
                : t('Performance.TopOfDescent.TT.SynchronizeInputWithAircraftsCurrentAltitude')
            }
          >
            <button
            ...
            <div key={type}>
              ...
              <div className="flex flex-row">
                <SimpleInput
                  ...
                  onChange={(input) =>
                    dispatch(
                      setTodData({ calculation: { input: parseFloat(input), type: input !== '' ? type : undefined } }),
                    )
                  }
                ...
                {inputValid(type, syncValue) && (
                  <button
                    ...
                    onClick={() =>
                      dispatch(
                        setTodData({
                          calculationInputMode: !calculationInputSyncEnabled
                            ? TOD_INPUT_MODE.AUTO
