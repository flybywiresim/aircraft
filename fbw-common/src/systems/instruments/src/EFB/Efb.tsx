...
export const getAirframeType = () =>
  new URL(document.querySelectorAll('vcockpit-panel > *')[0].getAttribute('url')).searchParams.get('Airframe');
  ...
  useEffect(() => {
    if (powerState === PowerStates.SHUTOFF) {
      ...
      if ((!simbriefData || !isSimbriefDataLoaded()) && autoSimbriefImport === 'ENABLED') {
        fetchSimbriefDataAction(navigraphUsername ?? '', overrideSimBriefUserID ?? '')
          .then((action) => {
            dispatch(action);
          })
          .catch((e) => {
