        ...
        onConfirm={() => {
          checklists.forEach((cl, clIndex) => {
            cl.items.forEach((_, itemIdx) => {
              ...
              dispatch(
                setChecklistItemCompletion({ checklistIndex: clIndex, itemIndex: itemIdx, completionValue: false }),
              );
            });
        ...
        <div className="flex w-1/4 shrink-0 flex-col justify-between">
          <ScrollableContainer innerClassName="space-y-4" height={46}>
            {CHECKLISTS.map((cl, index) => (
              ...
              >
                {!!autoFillChecklists && firstRelevantUnmarkedIdx === index && <Link45deg size={24} />}
                {cl.name}
              </div>
            ))}
          </ScrollableContainer>
          ...
          <button
            type="button"
            className="border-utility-red bg-theme-body text-utility-red hover:bg-utility-red hover:text-theme-body flex h-12 items-center justify-center rounded-md border-2 font-bold transition duration-100"
