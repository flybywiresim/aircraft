...
const ChecklistItemComponent = ({ item, index }: ChecklistItemComponentProps) => {
  ...
  useEffect(() => {
    if (autoItemTouches === 5) {
      toast.info(
        'You cannot interact with this item because you have enabled the autofill checklist option in the Realism settings page.',
      );
  ...
  return (
    <div className={`flex flex-row items-center space-x-4 py-2 ${color}`} onClick={handleChecklistItemClick}>
      {item.item && (
      ...
      <div className="flex w-full flex-row items-end text-current">
        ...
        <div className="whitespace-nowrap text-current">{item.result}</div>
      </div>
    </div>
  );
};
...
const CompletionButton = () => {
  ...
  if (checklists[selectedChecklistIndex].markedCompleted) {
    if (selectedChecklistIndex < checklists.length - 1) {
      return (
        <div
          className="border-theme-highlight bg-theme-body text-theme-highlight hover:bg-theme-highlight hover:text-theme-body flex w-full items-center justify-center rounded-md border-2 py-2 text-center font-bold transition duration-100"
          onClick={() => {
            dispatch(setSelectedChecklistIndex(selectedChecklistIndex + 1));
          }}
        >
    ...
    return (
      <div className="border-theme-highlight bg-theme-body text-theme-highlight flex w-full items-center justify-center rounded-md border-2 py-2 text-center font-bold">
  ...
  if (firstIncompleteIdx !== -1) {
    return (
      <div
        className="border-utility-green bg-theme-body text-utility-green hover:bg-utility-green hover:text-theme-body flex w-full items-center justify-center rounded-md border-2 py-2 text-center font-bold transition duration-100"
        onClick={() => {
          dispatch(
            setChecklistItemCompletion({
              checklistIndex: selectedChecklistIndex,
  ...
  return (
    <div className="border-utility-green bg-theme-body text-utility-green flex w-full items-center justify-center rounded-md border-2 py-2 text-center font-bold">
