...
export const A380Payload: React.FC<A380Props> = ({
  ...
  const [mainMid1CDesired, setMainMid1CDesired] = useSeatFlags(
    `L:${Loadsheet.seatMap[4].simVar}_DESIRED`,
    Loadsheet.seatMap[4].capacity,
    397,
  );
  const [mainMid2ADesired, setMainMid2ADesired] = useSeatFlags(
    `L:${Loadsheet.seatMap[5].simVar}_DESIRED`,
    Loadsheet.seatMap[5].capacity,
    421,
  );
  const [mainMid2BDesired, setMainMid2BDesired] = useSeatFlags(
    `L:${Loadsheet.seatMap[6].simVar}_DESIRED`,
    Loadsheet.seatMap[6].capacity,
    439,
  );
  const [mainMid2CDesired, setMainMid2CDesired] = useSeatFlags(
    `L:${Loadsheet.seatMap[7].simVar}_DESIRED`,
    Loadsheet.seatMap[7].capacity,
    457,
  );
  const [mainAftADesired, setMainAftADesired] = useSeatFlags(
    `L:${Loadsheet.seatMap[8].simVar}_DESIRED`,
    Loadsheet.seatMap[8].capacity,
    479,
  );
  const [mainAftBDesired, setMainAftBDesired] = useSeatFlags(
    `L:${Loadsheet.seatMap[9].simVar}_DESIRED`,
    Loadsheet.seatMap[9].capacity,
    499,
  );
  const [upperFwdDesired, setUpperFwdDesired] = useSeatFlags(
    `L:${Loadsheet.seatMap[10].simVar}_DESIRED`,
    Loadsheet.seatMap[10].capacity,
    521,
  );
  const [upperMidADesired, setUpperMidADesired] = useSeatFlags(
    `L:${Loadsheet.seatMap[11].simVar}_DESIRED`,
    Loadsheet.seatMap[11].capacity,
    541,
  );
  const [upperMidBDesired, setUpperMidBDesired] = useSeatFlags(
    `L:${Loadsheet.seatMap[12].simVar}_DESIRED`,
    Loadsheet.seatMap[12].capacity,
    557,
  );
  const [upperAftDesired, setUpperAftDesired] = useSeatFlags(
    `L:${Loadsheet.seatMap[13].simVar}_DESIRED`,
    Loadsheet.seatMap[13].capacity,
    571,
  );
        ...
        <div className="mb-10">
          <div className="relative flex flex-col">
            ...
            <div className="absolute top-full flex w-full flex-row px-4">
              ...
              <SelectGroup>
                <SelectItem selected={displayPaxMainDeck} onSelect={() => setDisplayPaxMainDeck(true)}>
                  Main
                </SelectItem>
                <SelectItem selected={!displayPaxMainDeck} onSelect={() => setDisplayPaxMainDeck(false)}>
                  Upper
                </SelectItem>
              </SelectGroup>
            </div>
                ...
                <Card className="h-full w-full" childrenContainerClassName="flex flex-col w-full h-full">
                  <div className="flex flex-row items-center justify-between">
                    ...
                    <SelectGroup>
                      ...
                      <TooltipWrapper
                        text={`${!enableReal ? t('Ground.Fuel.TT.AircraftMustBeColdAndDarkToChangeRefuelTimes') : ''}`}
                      >
                        <div>
                          <SelectItem
