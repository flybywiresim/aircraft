...
export class ColoredMetar extends DisplayComponent<ColoredMetarProps> {
  ...
  onAfterRender(node: VNode) {
    ...
    this.subscriptions.push(this.props.metar.sub(() => this.renderMetar()));
  }
    ...
    for (let i = 0; i < metar.raw_parts.length; i++) {
      ...
      switch (colorCode) {
        case ColorCode.Highlight:
          span.classList.add('text-theme-highlight');
          break;
        ...
        case ColorCode.Warning:
          ...
          break;
        case ColorCode.Info:
          span.classList.add('text-utility-grey');
          break;
        case ColorCode.TrendMarker:
          span.classList.add('font-bold', 'text-gray-500');
          break;
        default:
          break;
      }
