...
export class InputValidation {
   ...
   */
  public static classifyScratchpadWaypointType(
    waypoint: string,
    allowTime: boolean,
  ): [InputWaypointType, AtsuStatusCodes] {
  ...
  private static sameAltitudeType(lower: string, higher: string): boolean {
    ...
    if (
      (lower[lower.length - 1] === 'M' && higher[higher.length - 1] === 'M') ||
      (lower[lower.length - 1] !== 'M' && higher[higher.length - 1] !== 'M')
    ) {
      return true;
    }
