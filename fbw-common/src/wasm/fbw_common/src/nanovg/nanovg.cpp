...
MSFS_CALLBACK NVGcontext* nvgCreateInternal(NVGparams* params)
{
	...
	// Create font texture
	ctx->fontImages[0] = ctx->params.renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_ALPHA, fontParams.width, fontParams.height, 0, NULL, "NanoVG font tex");
...
MSFS_CALLBACK void nvgDeleteInternal(NVGcontext* ctx)
{
	...
	for (int p = 0; p < NVG_MAX_PATHS; ++p) {
		if (ctx->pathData[p].commands != NULL) free(ctx->pathData[p].commands);
		if (ctx->pathData[p].cache != NULL) nvg__deletePathCache(ctx->pathData[p].cache);
	}
	if (ctx->fs)
...
void nvgFill(NVGcontext* ctx)
{
	...
	ctx->params.renderFill(ctx->params.userPtr, &fillPaint, state->compositeOperation, &state->scissor, ctx->fringeWidth,
		ctx->pathData[ctx->activePath].cache->bounds, ctx->pathData[ctx->activePath].cache->paths, ctx->pathData[ctx->activePath].cache->npaths);
...
void nvgStroke(NVGcontext* ctx)
{
	...
	ctx->params.renderStroke(ctx->params.userPtr, &strokePaint, state->compositeOperation, &state->scissor, ctx->fringeWidth,
		strokeWidth, ctx->pathData[ctx->activePath].cache->paths, ctx->pathData[ctx->activePath].cache->npaths);
...
static int nvg__allocTextAtlas(NVGcontext* ctx)
{
	...
	if (ctx->fontImages[ctx->pathData[ctx->activePath].fontImageIdx + 1] != 0)
		...
		ctx->fontImages[ctx->pathData[ctx->activePath].fontImageIdx + 1] = ctx->params.renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_ALPHA, iw, ih, 0, NULL, "NanoVG text atlas");
	}
