...
impl Computer {
    ...
    fn new_fo_and_ovhd(context: &mut InitContext) -> Self {
        Self {
            ...
            // Not used for now.
            // Should be BITE card but is it going to be implemented?
            second_card: None,
            is_power_supply_powered: false,
        }
    }

    fn update(&mut self, context: &UpdateContext, bus: &mut Vec<Arinc429Word<u32>>) {
...
impl AudioCard {
    ...
    fn update(
        ...
        if is_powered {
            ...
            let result_acp = AudioCard::decode_arinc_words_from_acp(
                &mut self.bus_acp,
                &mut self.mixed_audio_acp,
                &mut self.transmission_table_acp,
                &mut self.reset_acp,
            );
            let result_acp3 = AudioCard::decode_arinc_words_from_acp(
            ...
            calls_card
                .as_mut()
                .unwrap()
                .set_reset(self.reset_acp || self.reset_acp3);

            if result_acp {
                AudioCard::send_amu_word(
                    &mut self.bus_acp,
                    self.transmission_table_acp,
                    calls_card.as_ref().unwrap().get_selcal(),
                );
            }
            if result_acp3 {
                AudioCard::send_amu_word(
    ...
    fn send_amu_word(bus_acp: &mut Vec<Arinc429Word<u32>>, transmission_table: u32, selcal: u8) {
