...
class PooledSubscriber {
    private static simvarMap = new Map<string, Array<keyof NavProcSimVars>>([
        ['nav1', [
    ...
    private publisher: NavProcSimVarPublisher;
     ...
     */
    constructor(publisher: NavProcSimVarPublisher) {
        this.publisher = publisher;
        this.subCount = new Map<string, number>();
     ...
     */
    public addSub(type: string): void {
        ...
        if (subCount == 0) {
            ...
            if (simvars) {
                for (const simvar of simvars) {
                    this.publisher.subscribe(simvar as keyof NavProcSimVars);
                }
            }
        }
     ...
     */
    public delSub(type: string): void {
        const subCount = this.subCount.get(type);
        ...
        if (subCount == 1) {
            for (const simvar in PooledSubscriber.simvarMap.get(type)) {
                this.publisher.unsubscribe(simvar as keyof NavProcSimVars);
...
export class NavProcessor {
    ...
    private subController: PooledSubscriber;
    ...
    public init(): void {
        ...
        for (let i = 1; i <= this.config.numNav; i++) {
            ...
            this.simVarSubscriber.on(`nav${i}_cdi` as keyof NavProcSimVars).whenChangedBy(1).handle((deviation) => {
                src.deviation = deviation as number;
            });
            this.simVarSubscriber.on(`nav${i}_obs` as keyof NavProcSimVars).whenChangedBy(1).handle((obs) => {
                src.obs = obs as number;
            });
            this.simVarSubscriber.on(`nav${i}_dme` as keyof NavProcSimVars).whenChangedBy(0.1).handle((distance) => {
                src.distance = distance as number;
                // TODO Fold DME logic into the distance handler.
                this.onDme(src.hasDme, src.distance, src.srcId);
            });
            this.simVarSubscriber.on(`nav${i}_radial` as keyof NavProcSimVars).handle((bearing) => {
                src.bearing = bearing as number;
            });
            this.simVarSubscriber.on(`nav${i}_ident` as keyof NavProcSimVars).whenChanged().handle((ident) => {
                src.ident = ident as string;
            });
            this.simVarSubscriber.on(`nav${i}_signal` as keyof NavProcSimVars).withPrecision(0).handle((signal) => {
                src.signal = signal as number;
            });
            this.simVarSubscriber.on(`nav${i}_has_nav` as keyof NavProcSimVars).whenChanged().handle((valid) => {
                src.valid = valid == 0 ? false : true;
            });
            this.simVarSubscriber.on(`nav${i}_has_dme` as keyof NavProcSimVars).whenChanged().handle((dme) => {
                src.hasDme = dme == 0 ? false : true;
                // TODO Fold DME logic into the distance handler.
                this.onDme(src.hasDme, src.distance, src.srcId);
            });
            this.simVarSubscriber.on(`nav${i}_to_from` as keyof NavProcSimVars).whenChanged().handle((value) => {
                src.toFrom = value as VorToFrom;
            });
            this.simVarSubscriber.on(`nav${i}_localizer` as keyof NavProcSimVars).whenChanged().handle((localizer) => {
                src.hasLocalizer = localizer as boolean;
            });
            this.simVarSubscriber.on(`nav${i}_localizer_crs` as keyof NavProcSimVars).whenChanged().handle((locCourse) => {
                src.localizerCourse = locCourse as number;
            });
            this.simVarSubscriber.on(`nav${i}_glideslope` as keyof NavProcSimVars).whenChanged().handle((gs) => {
                src.hasGlideslope = gs as boolean;
            });
            this.simVarSubscriber.on(`nav${i}_gs_error` as keyof NavProcSimVars).whenChanged().handle((gsDev) => {
                src.glideslopeDeviation = gsDev as number;
            });
            this.simVarSubscriber.on(`nav${i}_raw_gs` as keyof NavProcSimVars).whenChanged().handle((rawGs) => {
                src.glideslopeAngle = rawGs as number;
            });
            this.simVarSubscriber.on(`nav${i}_magvar` as keyof NavProcSimVars).whenChanged().handle((magvar) => {
                src.magneticVariation = magvar as number;
            });
