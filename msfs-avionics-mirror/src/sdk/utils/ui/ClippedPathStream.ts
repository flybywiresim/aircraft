...
export class ClippedPathStream extends AbstractTransformingPathStream {
   ...
   */
  constructor(consumer: PathStream, private readonly bounds: Subscribable<Float64Array>) {
    super(consumer);
  ...
  public arc(x: number, y: number, radius: number, startAngle: number, endAngle: number, counterClockwise = false): void {
    ...
    if (!Vec2Math.equals(this.prevPoint, startPoint)) {
    ...
    if (intersectionStartIndex >= 0) {
      for (let i = 0; i < intersectionCount; i++) {
        ...
        if (isOutside) {
          ...
          this.consumer.arc(x, y, radius, lastRadial, intersection.radial, counterClockwise);
        }
    ...
    const endAngleNormalized = startAngleNormalized + angularWidth * directionSign;

    if (!isOutside) {
      this.consumer.arc(x, y, radius, lastRadial, endAngleNormalized, counterClockwise);
      if ((endAngleNormalized - endAngle) % pi2 !== 0) {
