    ...
    static ShowPage(mcdu, waypoints, callback, page = 0) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (w) {
                ...
                const latString = `${Math.abs(w.infos.coordinates.lat).toFixed(0).toString().padStart(2, "0")}${w.infos.coordinates.lat >= 0 ? 'N' : 'S'}`;
                const longString = `${Math.abs(w.infos.coordinates.long).toFixed(0).toString().padStart(3, "0")}${w.infos.coordinates.long >= 0 ? 'E' : 'W'}`;
