                ...
                Coherent.call('GET_NEAREST_AIRSPACES').then((airspaces) => {
                    for (let i = 0; i < airspaces.length; i++) {
                        ...
                        if (airspaceData.type !== 1) {
                            ...
                            if (!this.nearestAirspaces.find((a) => a.name === name)) {
...
class FacilityLoader {
    ...
    getFacilityRaw(icao, timeout = 1500, skipIntersectionData = false) {
        const queueRawLoad = (loadCall, icao, type) => new Promise((resolve) => {
            const request = {
                resolve,
    ...
    async waitRegistration() {
        if (!this._isCompletelyRegistered) {
            const waitForCompleteRegistration = () => new Promise((resolve) => {
                const f = () => {
                    if (this._isCompletelyRegistered) {
                        resolve();
                    } else {
    ...
    async getFacility(icao, loadFacilitiesTransitively = false) {
        return new Promise((resolve) => this.getFacilityCB(icao, (wp) => {
            resolve(wp);
        }, loadFacilitiesTransitively));
    }
    ...
    async getFacilityData(icao) {
        return new Promise((resolve) => this.getFacilityDataCB(icao, (data) => {
            resolve(data);
        }));
    }
    ...
    getAirportDataCB(icao, callback) {
        if (this._isCompletelyRegistered && this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
            icao = icao.trim();
            const airport = this.loadedFacilities.find((f) => f.icaoTrimed === icao && (f.routes === undefined));
            if (airport) {
    ...
    getIntersectionDataCB(icao, callback) {
        ...
        if (this._isCompletelyRegistered && this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
            icao = icao.trim();
            const intersection = this.loadedFacilities.find((f) => (f.icaoTrimed === icao) && (f.routes != undefined));
            if (intersection) {
    ...
    async getIntersectionsData(icaos) {
        ...
        while (i < icaos.length) {
            ...
            if (intersection) {
                ...
                icaos.splice(i, 1);
            } else if (this.loadingFacilities.find((i) => i === icao)) {
                icaos.splice(i, 1);
    ...
    getNdbDataCB(icao, callback) {
        if (this._isCompletelyRegistered && this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
            icao = icao.trim();
            const ndb = this.loadedFacilities.find((f) => f.icaoTrimed === icao && (f.routes === undefined));
            if (ndb) {
                return callback(ndb);
            }
            if (icao[0] !== 'N') {
    ...
    getVorDataCB(icao, callback) {
        if (this._isCompletelyRegistered && this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
            icao = icao.trim();
            const vor = this.loadedFacilities.find((f) => f.icaoTrimed === icao && (f.routes === undefined));
            if (vor) {
                return callback(vor);
            }
            if (icao[0] !== 'V') {
    ...
    async getAirwayData(intersectionInfo, name = '', maxLength = 100) {
        ...
        const route = intersectionInfo.routes.find((r) => r.name === name);
        if (route) {
            const airway = {
                name: route.name,
                type: route.type,
                    ...
                    if (prevIcao && prevIcao.length > 0 && prevIcao[0] != ' ') {
                        ...
                        if (prevWaypoint) {
                            ...
                            if (prevWaypoint.routes) {
                                currentRoute = prevWaypoint.routes.find((r) => r.name === name);
                            }
                        }
                    } else {
                        break;
                    ...
                    if (nextIcao && nextIcao.length > 0 && nextIcao[0] != ' ') {
                        ...
                        if (nextWaypoint) {
                            ...
                            if (nextWaypoint.routes) {
                                currentRoute = nextWaypoint.routes.find((r) => r.name === name);
                            }
                        }
                    } else {
                        break;
    ...
    async GetAirportNamedFrequencies(airportIcao) {
        ...
        return new Promise((resolve) => {
            SimVar.SetSimVarValue('C:fs9gps:WaypointAirportICAO', 'string', airportIcao, `${this.instrument.instrumentIdentifier}-loader`).then(async () => {
                ...
                const getFrequency = async (index) => new Promise((resolve) => {
                ...
                if (this.createWaypointsCallback) {
                    ...
                    SimVar.SetSimVarValue(`C:fs9gps:${this.SET_ITEM_INDEX}`, 'number', this._itemIterator, `${this.instrument.instrumentIdentifier}-loader`).then(async () => {
                        if (this.GET_ITEM_ICAO && this.createWaypointCallback) {
                            ...
                            let waypoint = this.waypoints.find((a) => a.icao === icao);
                            if (!waypoint) {
                            ...
                            let airport = this.waypoints.find((a) => a.ident === ident);
                            if (!airport) {
...
class NDBLoader extends WaypointLoader {
    constructor(_instrument) {
        ...
        this.createWaypointCallback = async (icao) => new Promise((resolve) => {
        ...
        this.createWaypointsCallback = async (icaos) => {
            ...
            for (let i = 0; i < icaos.length; i++) {
                const icao = icaos[i];
                if (icao) {
                    if (!this.waypoints.find((a) => a.icao === icao)) {
...
class AirportLoader extends WaypointLoader {
    constructor(_instrument, loadFacility = true) {
        ...
        this.createCallback = async (ident) => {
            ...
            const getDeparture = async (lineIndex) => new Promise((resolve) => {
