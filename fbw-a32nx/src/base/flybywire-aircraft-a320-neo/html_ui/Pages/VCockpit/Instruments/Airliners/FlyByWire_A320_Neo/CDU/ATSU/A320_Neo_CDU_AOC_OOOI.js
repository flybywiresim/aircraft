    ...
    static ShowPage(mcdu, offset = 0) {
        ...
        if (messages.length > offset) {
            if (messages[offset].OutGate.timestamp !== null) {
                ...
                outFuel = `{small}{green}${Math.round(messages[offset].OutGate.fuel).toString()}\xa0{end}{end}`;
            }
            ...
            if (messages[offset].OffGround.timestamp !== null) {
                ...
                offFuel = `{small}{green}${Math.round(messages[offset].OffGround.fuel).toString()}\xa0{end}{end}`;
            }
            ...
            if (messages[offset].OnGround.timestamp !== null) {
                ...
                onFuel = `{small}{green}${Math.round(messages[offset].OnGround.fuel).toString()}\xa0{end}{end}`;
            }

            if (messages[offset].InGate.timestamp !== null) {
                inTime = `{small}{green}\xa0${messages[offset].InGate.timestamp.mailboxTimestamp()}{end}{end}`;
                inFuel = `{small}{green}${Math.round(messages[offset].InGate.fuel).toString()}\xa0{end}{end}`;
            }
