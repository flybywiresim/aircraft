    ...
    if (splitData.animation === anim.name) {
      ...
      for (const newAnimData of splitData.newAnimations) {
        const newAnim = {
          ...
          channels: newAnimData.indices.map((oldIndex, newIndex) => ({
            ...anim.channels[oldIndex],
            sampler: newIndex,
          })),
...
for (const model of models) {
  for (let i = 0; i < model.gltf.length; i += 1) {
    ...
    if (modifications.length > 0) {
      ...
      let modifiedBin = applyModifications(fs.readFileSync(p(model.bin[i])), p(model.gltf[i]), modifications);
      modifiedBin = applyOutputSamplerModifications(modifiedBin, p(model.gltf[i]), modifications);
    ...
    for (const addition of additions) {
      if (addition.nodes) {
        ...
        fs.appendFileSync(
          p(model.output.bin[i]),
          Buffer.alloc((4 - (fs.statSync(p(model.output.bin[i])).size % 4)) % 4),
        );
