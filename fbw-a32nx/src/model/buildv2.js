...
function loadGltf(gltfPath) {
  ...
  assert.ok(
    haveBuffer ||
      ((!gltf['buffers'] || gltf['buffers'].length === 0) && (!gltf['accessors'] || gltf['accessors'].length === 0)),
  );
  ...
  for (const accessor of gltf['accessors']) {
    ...
    accessor['data'] = Array.from({ length: info.accessorCount }).map((_, index, arr) => {
      ...
      assert.ok(
        !isNaN(byteOffset),
        `${info.bufferViewOffset} + ${info.accessorOffset} + ${index} * ${info.byteStride}`,
      );
  ...
  for (const mesh of allMeshes) {
    for (const prim of mesh.primitives) {
      ...
      const primAccessors = new Map(
        Object.keys(prim.attributes).map((attributeName) => [
          attributeName,
          gltf.accessors[prim.attributes[attributeName]],
        ]),
      );
...
function saveGltf(gltfPath, gltf, debugOuput = false) {
  ...
  for (const skin of allSkins) {
    ...
    if (accessors.indexOf(accessor) < 0) {
      ...
      let bufIndex = buffers.findIndex(
        (b) => b.type === accessor.type && b.componentType === accessor.componentType && b.usage === 'Skin',
      );
      let buf;
      if (bufIndex === -1) {
    ...
    for (const s of samplers) {
      for (const key of ['input', 'output']) {
        ...
        if (accessors.indexOf(accessor) < 0) {
          ...
          let bufIndex = buffers.findIndex(
            (b) => b.type === accessor.type && b.componentType === accessor.componentType && b.usage === 'Animation',
          );
          let buf;
          if (bufIndex === -1) {
            bufIndex = buffers.length;
  ...
  const findVertexBuffer = (attributes, accessors) => {
    ...
    let buf = buffers.find(
      (b) => b.usage === 'Vertex' && b.byteStride === byteStride && areSetsEqual(b.attributes, attributes),
    );
    if (buf) {
      return buf;
    }
  ...
  for (const mesh of allMeshes) {
    for (const prim of mesh.primitives) {
      ...
      const primAccessors = new Map(
        Object.keys(prim.attributes).map((attributeName) => [attributeName, prim.attributes[attributeName]]),
      );
      ...
      assert.equal(
        doAttributesExist,
        primAttrArray.every((a) => accessors.indexOf(a) >= 0),
      );
...
function getAccessorInfo(gltf, accessor, bufferView = null) {
  ...
  assert.ok(
    !isNaN(byteStride),
    `${bufferView['byteStride']} || ${accessorElementSize} ${AccessorType[accessor['type']]}`,
  );
...
function appendNodes(baseGltf, sourceGltf, nodes, options) {
  ...
  if (options.copyAnimations !== 'false' && sourceGltf.animations) {
    const animsToCopy = sourceGltf.animations.filter((a) =>
      a.channels.some((c) => nodes.indexOf(c.target.node.name) >= 0),
    );
...
function main(args) {
  ...
  for (const task of tasks) {
    ...
    const getAdditionalGltf = (path) =>
      additional_gltfs.get(path) || additional_gltfs.set(path, loadGltf(resolveRelativePath(path))).get(path);
