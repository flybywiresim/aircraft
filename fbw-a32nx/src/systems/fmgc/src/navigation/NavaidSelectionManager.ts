...
export class NavaidSelectionManager {
  ...
  private static readonly SPECIFIED_NDB_APPROACH_TYPES = [
  ...
  private typeIsVor(type: VorType): boolean {
    return type === VorType.VOR || type === VorType.VORDME || type === VorType.VORTAC;
  ...
  private isVorDme(facility: RawVor): boolean {
    return facility.type === VorType.VORDME || facility.type === VorType.VORTAC;
  }
  ...
  private isSuitableType(facility: RawVor): boolean {
    ...
    return facility.vorClass !== VorClass.VOT && facility.type !== VorType.TACAN && facility.type !== VorType.ILS;
  }
  ...
  private selectDmePair(): void {
    ...
    if (this.selectedDmePair !== null) {
      ...
      findNewPair =
        findNewPair ||
        !this.candidateList.some((v) => v.icao === this.selectedDmePair[0].icao) ||
        !this.candidateList.some((v) => v.icao === this.selectedDmePair[1].icao);
  ...
  private getSpecifiedNavaid(): RawVor | null {
    if (NavaidSelectionManager.SPECIFIED_VOR_APPROACH_TYPES.includes(this.fpm.getApproachType(FlightPlans.Active))) {
      ...
      const facility: RawVor | null =
        finalLeg?.additionalData.recommendedFacility?.__Type === 'JS_FacilityVOR'
          ? finalLeg.additionalData.recommendedFacility
          : null;
  ...
  private selectDisplayVor(): void {
    ...
    if (activeLegIndex >= 0) {
      for (let i = activeLegIndex; i < activeLegIndex + 5; i++) {
        ...
        // eslint-disable-next-line no-underscore-dangle
        const facility: RawVor | null =
          leg.additionalData.facility?.__Type === 'JS_FacilityVOR' ? leg.additionalData.facility : null;
  ...
  private getReferenceNavaid(): RawVor | null {
    ...
    candidates.sort(
      (a, b) =>
        distanceTo(destRunway.thresholdCoordinates, { lat: a.lat, long: a.lon }) -
        distanceTo(destRunway.thresholdCoordinates, { lat: b.lat, long: b.lon }),
    );
    if (
      candidates.length > 0 &&
      distanceTo(destRunway.thresholdCoordinates, { lat: candidates[0].lat, long: candidates[0].lon }) < 5
    ) {
      return candidates[0];
    }
    return null;
  ...
  private getSpecifiedNdb(): RawNdb | null {
    if (NavaidSelectionManager.SPECIFIED_NDB_APPROACH_TYPES.includes(this.fpm.getApproachType(FlightPlans.Active))) {
      ...
      const finalLeg = waypoints[waypoints.length - 2];
      // eslint-disable-next-line no-underscore-dangle
      const facility: RawNdb | null =
        finalLeg?.additionalData.recommendedFacility?.__Type === 'JS_FacilityNDB'
          ? finalLeg.additionalData.recommendedFacility
          : null;
      if (facility !== null) {
        return facility;
