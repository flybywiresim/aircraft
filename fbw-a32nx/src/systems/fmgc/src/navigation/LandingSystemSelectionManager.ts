...
export class LandingSystemSelectionManager {
    ...
    if (this.autotuneUpdateThrottler.canUpdate(deltaTime, forceUpdate) > -1) {
      ...
      try {
        ...
        if (phase <= FmgcFlightPhase.Takeoff) {
          ...
          if (
            destination &&
            distanceTo(this.ppos, destination.infos.coordinates) <=
              LandingSystemSelectionManager.DESTINATION_TUNING_DISTANCE
          ) {
            await this.selectApproachIls();
  ...
  private async setIlsFromApproach(
    ...
    if (checkRunwayFrequencies) {
      const runway = (airport.infos as AirportInfo).oneWayRunways.find(
        (r) => r.number === approach.runwayNumber && r.designator === approach.runwayDesignator,
      );
   ...
   */
  private async getIlsCourseSlopeFromApproach(
    airport: WayPoint,
    approach: RawApproach,
    ils: RawVor,
  ): Promise<IlsCourseSlopeData | null> {
    const runway = (airport.infos as AirportInfo).oneWayRunways.find(
      (r) => r.number === approach.runwayNumber && r.designator === approach.runwayDesignator,
    );
    if (runway) {
      ...
      for (const frequency of frequencies) {
        if (frequency.freqMHz > 0 && frequency.icao === ils.icao) {
          LandingSystemSelectionManager.courseSlopeCache.backcourse =
            approach.approachType === ApproachType.APPROACH_TYPE_LOCALIZER_BACK_COURSE;
          LandingSystemSelectionManager.courseSlopeCache.course = frequency.localizerCourse;
          LandingSystemSelectionManager.courseSlopeCache.slope = frequency.hasGlideslope
            ? -frequency.glideslopeAngle
            : null;
    ...
    let finalLegCourse = finalLeg.trueDegrees
      ? A32NX_Util.trueToMagnetic(finalLeg.course, -ils.magneticVariation)
      : finalLeg.course;
    ...
    if (finalLeg.type === LegType.TF) {
      ...
      finalLegCourse = A32NX_Util.trueToMagnetic(
        bearingTo(previousWp.infos.coordinates, finalWp.infos.coordinates),
        -ils.magneticVariation,
      );
    }

    const ilsApproachExists =
      (airport.infos as AirportInfo).approaches.find(
        (a) =>
          a.approachType === ApproachType.APPROACH_TYPE_ILS &&
          a.runwayNumber === approach.runwayNumber &&
          a.runwayDesignator === approach.runwayDesignator,
      ) !== undefined;

    LandingSystemSelectionManager.courseSlopeCache.backcourse =
      approach.approachType === ApproachType.APPROACH_TYPE_LOCALIZER_BACK_COURSE;
    LandingSystemSelectionManager.courseSlopeCache.course = LandingSystemSelectionManager.courseSlopeCache.backcourse
      ? finalLegCourse + 180
      : finalLegCourse;
    LandingSystemSelectionManager.courseSlopeCache.slope =
      finalLeg.verticalAngle && ilsApproachExists ? finalLeg.verticalAngle - 360 : null;
  ...
  private async tryGetCourseSlopeForIlsFromAirport(
    ils: RawVor,
    airport: WayPoint,
