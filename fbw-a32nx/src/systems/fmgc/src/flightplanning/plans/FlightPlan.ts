...
export class FlightPlan<P extends FlightPlanPerformanceData = FlightPlanPerformanceData> extends BaseFlightPlan<P> {
  ...
  directToLeg(ppos: Coordinates, magneticTrack: Degrees, targetLegIndex: number, _withAbeam = false) {
    ...
    const turningPoint = FlightPlanLeg.turningPoint(this.enrouteSegment, ppos, magneticTrack);
  ...
  directToWaypoint(ppos: Coordinates, magneticTrack: Degrees, waypoint: Fix, withAbeam = false) {
    // TODO withAbeam
    ...
    if (existingLegIndex !== -1 && existingLegIndex < this.firstMissedApproachLegIndex) {
      this.directToLeg(ppos, magneticTrack, existingLegIndex, withAbeam);
      return;
    }
