...
export function arcDistanceToGo(ppos: Coordinates, itp: Coordinates, centreFix: Coordinates, sweepAngle: Degrees) {
  ...
  const pposAngle =
    sweepAngle < 0
      ? Avionics.Utils.clampAngle(refFrameOffset - pposBearing)
      : Avionics.Utils.clampAngle(pposBearing - refFrameOffset);
...
export function arcGuidance(
  ...
  const desiredTrack =
    sweepAngle > 0 ? Avionics.Utils.clampAngle(bearingPpos + 90) : Avionics.Utils.clampAngle(bearingPpos - 90);
