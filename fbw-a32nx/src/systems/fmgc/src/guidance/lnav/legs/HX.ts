  ...
  protected getDistanceToGoThisOrbit(ppos: LatLongData): NauticalMiles {
    ...
    switch (this.state) {
      ...
      case HxLegGuidanceState.Outbound:
        return (
          courseToFixDistanceToGo(ppos, this.outboundLegCourse, fixB) +
          this.computeLegDistance() +
          this.radius * Math.PI
        );
...
export class HFLeg extends HXLeg {
  ...
  getGuidanceParameters(ppos: LatLongAlt, trueTrack: Degrees, tas: Knots, gs: Knots): GuidanceParameters {
    if (this.entryTransition) {
      this.termConditionMet =
        this.entryTransition.isNull ||
        this.entryTransition.state === EntryState.Capture ||
        this.entryTransition.state === EntryState.Done;
    }
