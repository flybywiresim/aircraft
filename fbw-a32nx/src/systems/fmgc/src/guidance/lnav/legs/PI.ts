  ...
  recomputeWithParameters(isActive: boolean, tas: number, gs: number, _ppos: Coordinates, _trueTrack: number): void {
    ...
    if (Math.abs(Avionics.Utils.diffAngle(cfInverseCrs, brgToCf)) < 90 && distToCf > minStraightDist) {
      ...
      [tp] = smallCircleGreatCircleIntersection(
        ...
        (p) => Math.abs(Avionics.Utils.diffAngle(cfInverseCrs, bearingTo(this.nextLeg.fix.infos.coordinates, p))) < 90,
      );
    ...
    const theta =
      (Math.abs(Avionics.Utils.diffAngle(this.outbound.course, (this.nextLeg.course + 180) % 360)) * Math.PI) / 180;
    ...
    return [
      {
        ...
        startPoint: this.inboundGuidable?.isComputed
