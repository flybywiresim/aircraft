...
export class RFLeg extends XFLeg {
  ...
  get inboundCourse(): Degrees {
    return Avionics.Utils.clampAngle(
      Avionics.Utils.computeGreatCircleHeading(this.center, this.from.infos.coordinates) + (this.clockwise ? 90 : -90),
    );
  }

  get outboundCourse(): Degrees {
    return Avionics.Utils.clampAngle(
      Avionics.Utils.computeGreatCircleHeading(this.center, this.to.infos.coordinates) + (this.clockwise ? 90 : -90),
    );
  }
  ...
  getGuidanceParameters(ppos: LatLongAlt, trueTrack: number, _tas: Knots): GuidanceParameters | null {
    ...
    return arcGuidance(
      ...
      this.center,
      this.clockwise ? this.angle : -this.angle,
    );
  }
