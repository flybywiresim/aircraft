...
export class FixedRadiusTransition extends Transition {
  ...
  recomputeWithParameters(isActive: boolean, tas: Knots, gs: Knots, ppos: Coordinates, trueTrack: DegreesTrue) {
    ...
    // Turn radius
    this.radius =
      (tas ** 2 / (9.81 * Math.tan(finalBankAngle * Avionics.Utils.DEG2RAD)) / 6997.84) * LnavConfig.TURN_RADIUS_FACTOR;
    ...
    const tooBigForNext =
      'from' in this.nextLeg
        ? distanceTo(this.nextLeg.from.infos.coordinates, this.nextLeg.to.infos.coordinates) < this.tad + 0.1
        : false;
