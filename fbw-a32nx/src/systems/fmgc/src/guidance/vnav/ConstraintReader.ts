...
import {
  ...
  getPathAngleConstraintFromWaypoint,
...
export class ConstraintReader {
    ...
    for (let i = 0; i < fpm.getWaypointsCount(FlightPlans.Active); i++) {
      ...
      if (waypoint.additionalData.cruiseStep && !waypoint.additionalData.cruiseStep.isIgnored) {
        if (i >= fpm.getActiveWaypointIndex()) {
          const { waypointIndex, toAltitude, distanceBeforeTermination } = waypoint.additionalData
            .cruiseStep as CruiseStepEntry;

          this.cruiseSteps.push({
            distanceFromStart:
              this.totalFlightPlanDistance - waypoint.additionalData.distanceToEnd - distanceBeforeTermination,
            toAltitude,
            waypointIndex,
            isIgnored: false,
          });
  ...
  public updateDistanceToEnd(ppos: LatLongAlt) {
    ...
    const nextWaypoint: WayPoint | undefined =
      leg instanceof VMLeg
    ...
    if (activeTransIndex === activeLegIndex) {
      ...
      this.distanceToEnd =
        outboundTransition.getDistanceToGo(ppos) - outboundLength + (nextWaypoint?.additionalData?.distanceToEnd ?? 0);
      ...
      this.distanceToEnd =
        transitionDistanceToGo + legDistance + outboundLength + (nextWaypoint?.additionalData?.distanceToEnd ?? 0);
    } else {
      const distanceToGo =
        leg instanceof VMLeg || leg instanceof IFLeg
  ...
  private updateDistancesToEnd(geometry: Geometry) {
    ...
    for (let i = fpm.getWaypointsCount(FlightPlans.Active) - 1; i >= fpm.getActiveWaypointIndex() - 1 && i >= 0; i--) {
      ...
      if (waypoint.endsInDiscontinuity) {
        ...
        this.totalFlightPlanDistance += Avionics.Utils.computeGreatCircleDistance(
          startingPointOfDisco.infos.coordinates,
