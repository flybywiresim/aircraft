  ...
  private interpolateByAltitude(altitude: Feet): WindVector {
    ...
    for (let i = 0; i < this.inputs.descentWinds.length - 1; i++) {
      if (altitude > this.inputs.descentWinds[i].altitude && altitude <= this.inputs.descentWinds[i + 1].altitude) {
        const scaling =
          (altitude - this.inputs.descentWinds[i].altitude) /
          (this.inputs.descentWinds[i + 1].altitude - this.inputs.descentWinds[i].altitude);

        return this.interpolateVectors(
          this.inputs.descentWinds[i].vector,
          this.inputs.descentWinds[i + 1].vector,
          scaling,
        );
      }
    }
