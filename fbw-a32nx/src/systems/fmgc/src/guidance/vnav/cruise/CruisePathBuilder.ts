...
import { GeographicCruiseStep, MaxSpeedConstraint, NavGeometryProfile, VerticalCheckpoint, VerticalCheckpointReason } from '../profile/NavGeometryProfile';
...
export class CruisePathBuilder {
    ...
    computeCruisePath(
        ...
        for (const step of profile.cruiseSteps) {
            ...
            const segmentToStep = this.computeCruiseSegment(config, altitude, step.distanceFromStart - distanceFromStart, remainingFuelOnBoard, speed, headwind);
    ...
    private tryAddStepFromLastCheckpoint(
        sequence: TemporaryCheckpointSequence, step: GeographicCruiseStep, stepClimbStrategy: ClimbStrategy, stepDescentStrategy: DescentStrategy, maxDistance: NauticalMiles,
    ) {
    ...
    private computeCruiseSegment(config: AircraftConfig, altitude: Feet, distance: NauticalMiles, remainingFuelOnBoard: number, speed: Knots, headwind: WindComponent): StepResults {
    ...
    private levelAccelerationStep(config: AircraftConfig, remainingFuelOnBoard: number, speed: Knots, finalSpeed: Knots, headwind: WindComponent): StepResults {
