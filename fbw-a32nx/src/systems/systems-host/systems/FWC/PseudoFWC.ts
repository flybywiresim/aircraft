...
export class PseudoFWC {
  ...
  private readonly lhLgNotLocked = RegisteredSimVar.createBoolean('L:A32NX_LGCIU_1_LEFT_GEAR_NOT_LOCKED');

  private readonly rhLgNotLocked = RegisteredSimVar.createBoolean('L:A32NX_LGCIU_1_RIGHT_GEAR_NOT_LOCKED');

  private readonly noseLgNotLocked = RegisteredSimVar.createBoolean('L:A32NX_LGCIU_1_NOSE_GEAR_NOT_LOCKED');
  ...
  update(_deltaTime: number) {
    ...
    this.engine1And2NotRunningInAirFor5Seconds.write(
      !this.engine1N2Sup.get() && !this.engine2N2Sup.get() && !this.aircraftOnGround.get(),
      deltaTime,
    );

    this.engine1RunningOrNotFlightPhase12910For1Second.write(
      this.engine1N2Sup.get() || !this.flightPhase12910.get(),
      deltaTime,
    );
    this.engine2RunningOrNotFlightPhase12910For1Second.write(
      this.engine2N2Sup.get() || !this.flightPhase12910.get(),
      deltaTime,
    );
    ...
    const lhGearLockUp =
      (!lgciu1Or2DiscreteWord1Invalid && !lgciu1LhGearNotLockUp && !lgciu2LhGearNotLockUp) ||
    ...
    const rhGearLockUp =
      (!lgciu1Or2DiscreteWord1Invalid && !lgciu1RhGearNotLockUp && !lgciu2RhGearNotLockUp) ||
    ...
    const noseGearLockUp =
      (!lgciu1Or2DiscreteWord1Invalid && !lgciu1NoseGearNotLockUp && !lgciu2NoseGearNotLockUp) ||
