...
export class Terrain {
    ...
    public static async mapdataAvailable(): Promise<boolean> {
        ...
        return fetchWithTimeout(`${getSimBridgeUrl()}/api/v1/terrain/available`).then((response) => {
    ...
    public static async setCurrentPosition(latitude: number, longitude: number, heading: number, altitude: number, verticalSpeed: number): Promise<void> {
        if (Terrain.endpointsAvailable) {
            fetchWithTimeout(`${getSimBridgeUrl()}/api/v1/terrain/position`, {
                method: 'PATCH',
    ...
    public static async setDisplaySettings(side: EfisSide, settings: {
        ...
        if (Terrain.endpointsAvailable) {
            fetchWithTimeout(`${getSimBridgeUrl()}/api/v1/terrain/displaysettings?display=${side}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    public static async ndMapAvailable(side: EfisSide, timestamp: number): Promise<boolean> {
        if (Terrain.endpointsAvailable) {
            return fetchWithTimeout(`${getSimBridgeUrl()}/api/v1/terrain/ndMapAvailable?display=${side}&timestamp=${timestamp}`).then((response) => {
                if (response.ok) {
    ...
    public static async ndTransitionMaps(side: EfisSide, timestamp: number): Promise<string[]> {
        if (Terrain.endpointsAvailable) {
            return fetchWithTimeout(`${getSimBridgeUrl()}/api/v1/terrain/ndmaps?display=${side}&timestamp=${timestamp}`, {
                method: 'GET',
    ...
    Promise<{
        ...
        if (Terrain.endpointsAvailable) {
            return fetchWithTimeout(`${getSimBridgeUrl()}/api/v1/terrain/terrainRange?display=${side}&timestamp=${timestamp}`, {
                method: 'GET',
                headers: { Accept: 'application/json' },
    ...
    public static async renderNdMap(side: EfisSide): Promise<number> {
        if (Terrain.endpointsAvailable) {
            return fetchWithTimeout(`${getSimBridgeUrl()}/api/v1/terrain/renderMap?display=${side}`).then((response) => response.text().then((text) => parseInt(text)));
        }
