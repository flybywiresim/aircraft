...
export const FlightPlanProvider: React.FC = ({ children }) => {
  ...
  return <FlightPlanContext.Provider value={{ flightPlanManager }}>{children}</FlightPlanContext.Provider>;
};
...
export const useCurrentFlightPlan = (): ManagedFlightPlan => {
  ...
  const [currentFlightPlan, setCurrentFlightPlan] = useState<ManagedFlightPlan>(() =>
    flightPlanManager.getCurrentFlightPlan(),
  );
