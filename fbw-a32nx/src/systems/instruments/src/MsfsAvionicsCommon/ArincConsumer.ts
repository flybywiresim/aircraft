...
export class BasicArincConsumer<T> extends BasicConsumer<T> {
    ...
    public withArinc429Precision(precision: number): ArincConsumer<T> {
        return new BasicArincConsumer<T>(this.arincSubscribe, { lastValue: 0, hasLastValue: false }, this.getWithArinc429PrecisionHandler(precision));
    }
    ...
    public whenArinc429SsmChanged(): ArincConsumer<T> {
        return new BasicArincConsumer<T>(this.arincSubscribe, { lastValue: 0, hasLastValue: false }, this.getWhenArinc429SsmChangedHandler());
    }
