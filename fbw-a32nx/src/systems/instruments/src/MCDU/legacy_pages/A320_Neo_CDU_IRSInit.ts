...
export class CDUIRSInit {
  static ShowPage(
    ...
    const currentGPSLat = CDUInitPage.ConvertDDToDMS(
      SimVar.GetSimVarValue('GPS POSITION LAT', 'degree latitude'),
      false,
    );
    const currentGPSLon = CDUInitPage.ConvertDDToDMS(
    ...
    if (!areAllAligned) {
      ...
      GPSPosAlign = ['--°--.--', '--°--.--', ' '];
      originAirportString = [
        originAirportLat['deg'] +
          '°{small}' +
          originAirportLat['min'] +
          '.' +
          originAirportLat['sec'] +
          '{end}' +
          originAirportLat['dir'] +
          '[color]cyan',
        originAirportLon['deg'] +
          '°{small}' +
          originAirportLon['min'] +
          '.' +
          originAirportLon['sec'] +
          '{end}' +
          originAirportLon['dir'] +
          '[color]cyan',
        referenceName,
      ];
      if (
        SimVar.GetSimVarValue('L:A32NX_OVHD_ADIRS_IR_1_MODE_SELECTOR_KNOB', 'Enum') ||
        SimVar.GetSimVarValue('L:A32NX_OVHD_ADIRS_IR_2_MODE_SELECTOR_KNOB', 'Enum') ||
        SimVar.GetSimVarValue('L:A32NX_OVHD_ADIRS_IR_3_MODE_SELECTOR_KNOB', 'Enum')
      ) {
        GPSPosAlign = [
          currentGPSLat['deg'] +
            '°{small}' +
            currentGPSLat['min'] +
            '.' +
            Math.ceil(Number(currentGPSLat['sec'] / 100)) +
            '{end}' +
            currentGPSLat['dir'] +
            '[color]green',
          currentGPSLon['deg'] +
            ...
            currentGPSLon['min'] +
            '.' +
            Math.ceil(Number(currentGPSLon['sec'] / 100)) +
            '{end}' +
            currentGPSLon['dir'] +
            '[color]green',
          '',
      ...
      if (!areAllAligned && SimVar.GetSimVarValue('L:A32XN_Neo_ADIRS_ALIGN_TYPE_REF', 'Enum') !== 1) {
        ...
        if ((activeReference['deg'] >= 90 && !lon) || (activeReference['deg'] >= 180 && lon)) {
          ...
          if (activeReference['min'] >= 60) {
            ...
            activeReference['deg'] = !lon
              ? activeReference['deg'] + 1
              : (Number(activeReference['deg']) + 1).toString().padStart(3, '0');
          }
        }
      }
    };
