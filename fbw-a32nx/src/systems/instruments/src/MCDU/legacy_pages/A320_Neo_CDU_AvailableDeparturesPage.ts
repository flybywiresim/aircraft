...
export class CDUAvailableDeparturesPage {
  static ShowPage(
    ...
    const selectedRunwayPage = selectedRunway
      ? Math.floor(
          availableRunways.findIndex((runway) => runway.ident === selectedRunway.ident) / DeparturePagination.DEPT_PAGE,
        )
      : -1;
    ...
    if (selectedRunway) {
      selectedRunwayCell = RunwayUtils.runwayString(selectedRunway.ident);
    ...
    if (!sidSelection) {
      ...
      for (let i = 0; i < DeparturePagination.DEPT_PAGE; i++) {
        ...
        if (runway) {
          ...
          rows[2 * i] = [
            `{${color}}${selected ? '{sp}' : '{'}${RunwayUtils.runwayString(runway.ident).padEnd(3)}${hasIls ? '{small}-ILS{end}' : '{sp}{sp}{sp}{sp}'}${NXUnits.mToUser(runway.length).toFixed(0).padStart(6, '\xa0')}{small}${NXUnits.userDistanceUnit().padEnd(2)}{end}{end}`,
          ];
          const ilsText = hasIls ? `${runway.lsIdent.padStart(6)}/${runway.lsFrequencyChannel.toFixed(2)}` : '';
          rows[2 * i + 1] = [
            `{${color}}{sp}{sp}{sp}${Utils.leadingZeros(Math.round(runway.magneticBearing), 3)}${ilsText}{end}`,
          ];
      ...
      if (selectedSid) {
        ...
        for (let i = 0; i < DeparturePagination.DEPT_PAGE; i++) {
          ...
          if (trans) {
            const selected =
              (trans === Labels.NO_TRANS && selectedTransition === null) ||
              (selectedTransition && selectedTransition.ident === trans.ident);
            const color = selected && !editingTmpy ? 'green' : 'cyan';
