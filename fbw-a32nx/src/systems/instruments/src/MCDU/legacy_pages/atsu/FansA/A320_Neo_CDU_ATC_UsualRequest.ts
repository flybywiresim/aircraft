...
export class CDUAtcUsualRequestFansA {
  ...
  static CreateRequests(mcdu, data) {
    ...
    if (data.heading) {
      retval.push(
        CDUAtcUsualRequestFansA.CreateRequest(mcdu, 'DM70', [data.heading === 0 ? '360' : data.heading.toString()]),
      );
    }
    if (data.climbTo) {
  ...
  static ShowPage(mcdu: LegacyAtsuPageInterface, data = CDUAtcUsualRequestFansA.CreateDataBlock()) {
    mcdu.clearDisplay();
