...
export class CDUAtcVertRequestFansA {
  ...
  static CanSendData(data) {
    return (
      ...
      data.whenHigher ||
      data.whenLower ||
      data.blockAltitudeLow ||
      data.blockAltitudeHigh ||
      data.cruise ||
      data.requestDescent
    );
  }
  ...
  static HandleClbDesStart(mcdu, value, data, climbRequest) {
    if (value === Keypad.clrValue || !value) {
      if (climbRequest) {
      ...
      const error = InputValidation.validateScratchpadAltitude(entries[0]);
      if (!error) {
        updateAlt = true;
        altitude = InputValidation.formatScratchpadAltitude(entries[0]);
        entries.shift();
      }
  ...
  static CreateRequests(mcdu, data) {
    ...
    if (data.climbStart) {
      retval.push(
        CDUAtcVertRequestFansA.CreateRequest(mcdu, /$[0-9]{4}Z^/.test(data.startAt) ? 'DM13' : 'DM11', [
          data.startAt,
          data.climbStart,
        ]),
      );
    ...
    if (data.descendStart) {
      retval.push(
        CDUAtcVertRequestFansA.CreateRequest(mcdu, /$[0-9]{4}Z^/.test(data.startAt) ? 'DM14' : 'DM12', [
          data.startAt,
          data.descendStart,
        ]),
      );
  ...
  static ShowPage1(mcdu: LegacyAtsuPageInterface, data = CDUAtcVertRequestFansA.CreateDataBlock()) {
    ...
    mcdu.onLeftInput[2] = (value) => {
      if (value === Keypad.clrValue) {
        ...
        if (entries.length !== 2) {
          ...
          const error = InputValidation.validateAltitudeRange(entries[0], entries[1]);
          if (error !== AtsuStatusCodes.Ok) {
            mcdu.addNewAtsuMessage(error);
          } else {
            data.blockAltitudeLow = InputValidation.formatScratchpadAltitude(entries[0]);
            data.blockAltitudeHigh = InputValidation.formatScratchpadAltitude(entries[1]);
            CDUAtcVertRequestFansA.ShowPage1(mcdu, data);
          }
        }
      }
    ...
    mcdu.onLeftInput[3] = (value) => {
      if (value === Keypad.clrValue) {
        ...
        if (error !== AtsuStatusCodes.Ok) {
          ...
          data.cruise = InputValidation.formatScratchpadAltitude(value);
        }
      }
      CDUAtcVertRequestFansA.ShowPage1(mcdu, data);
    };
    ...
    mcdu.onRightInput[3] = (value) => {
      if (value === Keypad.clrValue) {
        ...
        const range = InputValidation.validateScratchpadSpeedRanges(value);
        if (range[0] !== AtsuStatusCodes.Ok) {
