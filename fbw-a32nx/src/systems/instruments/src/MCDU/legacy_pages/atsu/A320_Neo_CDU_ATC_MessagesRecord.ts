  ...
  static ShowPage(mcdu: LegacyAtsuPageInterface, messages = null, offset = 0, confirmErase = false) {
    ...
    for (let i = 0; i < 5; ++i) {
      ...
      if (messages.length > offset + i && messages[offset + i]) {
        ...
        const lines = messages[offset + i].serialize(AtsuMessageSerializationFormat.Printer).split('\n');
        let firstLine = 'CPDLC';
        if (lines.length >= 2) {
          firstLine = messages[offset + i].serialize(AtsuMessageSerializationFormat.Printer).split('\n')[1];
        }
