...
export class CDUAtcAtisMenu {
  ...
  static InterpretLSK(mcdu, value, airports, idx, updateAtisPrintInProgress = false) {
    if (!value) {
      ...
      if (reports.length !== 0) {
        CDUAtcReportAtis.ShowPage(
          mcdu,
          `${airports[idx].icao}/${airports[idx].type === AtisType.Departure ? 'DEP' : 'ARR'}`,
          reports,
          0,
        );
      }
  ...
  static CreateLineData(mcdu: LegacyAtsuPageInterface, airport) {
    if (airport.icao !== '') {
      ...
      const left = `{cyan}${prefix}${airport.icao}/${airport.type === AtisType.Arrival ? 'ARR' : 'DEP'}{end}`;
