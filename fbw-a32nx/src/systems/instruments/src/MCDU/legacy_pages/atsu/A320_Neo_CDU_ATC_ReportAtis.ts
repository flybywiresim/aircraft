...
export class CDUAtcReportAtis {
  ...
  static ShowPage(mcdu: LegacyAtsuPageInterface, title, messages, messageIndex, offset = 0) {
    mcdu.clearDisplay();

    const message = messages[messageIndex];
    let serialized = message.serialize(AtsuMessageSerializationFormat.FmsDisplay);
    ...
    mcdu.setTemplate([
      ...
      [
        `[b-text]${title.replace('/', ' ')}`,
        `[b-text]${CDUAtcReportAtis.ConvertAtisInformation(messages[messageIndex].Information)} ${messages[messageIndex].Timestamp.mailboxTimestamp()}`,
      ],
