  ...
  static ShowPage1(mcdu: LegacyAtsuPageInterface, data = CDUAtcEmergencyFansA.CreateDataBlock()) {
    ...
    mcdu.onRightInput[1] = (value) => {
      if (value === Keypad.clrValue) {
        ...
        if (error === AtsuStatusCodes.Ok) {
          data.descendingTo = InputValidation.formatScratchpadAltitude(value);
        } else {
          mcdu.addNewAtsuMessage(error);
        }
      }
  ...
  static ShowPage2(mcdu: LegacyAtsuPageInterface, data = CDUAtcEmergencyFansA.CreateDataBlock()) {
    ...
    mcdu.onLeftInput[1] = (value) => {
      if (value === Keypad.clrValue) {
        ...
        if (split.length === 2) {
          if (split[0].length !== 0) {
            const error = InputValidation.validateScratchpadPosition(split[0]);
            if (error === AtsuStatusCodes.Ok) {
          ...
          const error = InputValidation.validateScratchpadPosition(split[1]);
          if (error === AtsuStatusCodes.Ok) {
    ...
    mcdu.onRightInput[0] = (value) => {
      if (value === Keypad.clrValue) {
        ...
        const error = InputValidation.validateScratchpadAltitude(value);
        if (error === AtsuStatusCodes.Ok) {
          data.climbingTo = InputValidation.formatScratchpadAltitude(value);
        } else {
          mcdu.addNewAtsuMessage(error);
        }
      }
