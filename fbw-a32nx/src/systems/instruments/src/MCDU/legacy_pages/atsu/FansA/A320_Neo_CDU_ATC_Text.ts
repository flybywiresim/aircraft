...
export class CDUAtcTextFansA {
  ...
  static CreateMessages(mcdu, messages, data) {
    ...
    if (freetextLines.length !== 0) {
      if (mcdu.atsu.fansMode() === FansMode.FansB) {
        freetextElement = CpdlcMessagesDownlink['DM98'][1].deepCopy();
      } else {
        freetextElement = CpdlcMessagesDownlink['DM67'][1].deepCopy();
      }
    ...
    if (data.performance) {
      ...
      extension = CpdlcMessagesDownlink['DM75'][1].deepCopy();
    } else if (data.icing) {
      extension = CpdlcMessagesDownlink['DM67'][1].deepCopy();
    ...
    if (messages.length === 0) {
      ...
      messages.push(new CpdlcMessage());
