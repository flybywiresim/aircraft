...
export class A32NX_TipsManager {
  ...
  private wasAnyAssistanceActive = false;
    ...
    if (!throttleConfig) {
      const checkThrottle = setInterval(() => {
        ...
        if (input === Math.round(SimVar.GetSimVarValue('L:A32NX_THROTTLE_MAPPING_INPUT:1', 'Number') * 100) / 100) {
          ...
          const clbLow =
            Math.round(SimVar.GetSimVarValue('L:A32NX_THROTTLE_MAPPING_CLIMB_LOW:1', 'Number') * 100) / 100;
          const clbHigh =
            Math.round(SimVar.GetSimVarValue('L:A32NX_THROTTLE_MAPPING_CLIMB_HIGH:1', 'Number') * 100) / 100;
          ...
          if (input >= clbLow - 0.03 && input <= clbHigh + 0.03 && fwcFlightPhase === 6 && atPhase === 3) {
            !once
              ...
              : this.notif.showNotification({
                  message:
                    'Please calibrate your throttles in the flyPad tablet (EFB). Potentially inaccurate throttle calibration has been detected.',
                  timeout: 60000,
                });
          } else {
            once = false;
  ...
  checkAssistenceConfiguration() {
    ...
    const isAnyAssistanceActive =
      assistanceAiControls || assistanceTakeOffEnabled || assistanceLandingEnabled || assistanceAutotrimActive;
    ...
    if (!this.wasAnyAssistanceActive && isAnyAssistanceActive) {
      this.notif.showNotification({
        message:
          'Ensure you have turned off all assistance functions:\n\n• AUTO-RUDDER\n• ASSISTED YOKE\n• ASSISTED LANDING\n• ASSISTED TAKEOFF\n• AI ANTI-STALL PROTECTION\n• AI AUTO-TRIM\n• ASSISTED CONTROLLER SENSITIVITY\n\nThey cause serious incompatibility!',
        timeout: 15000,
      });
    }
