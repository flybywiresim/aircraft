...
impl A320FwcRuntime {
    ...
    pub(super) fn new_running() -> Self {
        Self {
            ...
            // systems.
            elapsed: Duration::from_secs_f64(random_from_range(
    ...
    pub(super) fn update(&mut self, delta: Duration, inputs: &A320FwcInputs) {
        ...
        if self.elapsed >= self.computation_cycle_interval {
            ...
            // Instead of setting the elapsed time to Duration::ZERO, subtract to ensure that we
            // average out the correct information rate over multiple refreshes. This means the next
            // update might come a little sooner. We also use a loop so that we don't fall behind if
            // the frame rate spikes very high and we get an enormous delta.
            while self.elapsed >= self.computation_cycle_interval {
                self.elapsed -= self.computation_cycle_interval;
            }
        } else {
            self.elapsed += delta;
        }
