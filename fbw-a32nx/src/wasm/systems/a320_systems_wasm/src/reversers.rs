...
pub(super) fn reversers(builder: &mut MsfsAspectBuilder) -> Result<(), Box<dyn Error>> {
    builder.variables_to_object(Box::new(ReverserThrust { velz: 0. }));
...
impl VariablesToObject for ReverserThrust {
    ...
    fn write(&mut self, values: Vec<f64>) -> ObjectWrite {
        ...
        println!(
            "WRITEBACK CURR SPEED= {:.4} REVERSER_DELTA= {:.4}  FINAL WRITE {:.4}",
            values[0], values[1], self.velz
        );

        ObjectWrite::on(values[1].abs() > 0.)
    }
