...
pub(super) fn flaps(builder: &mut MsfsAspectBuilder) -> Result<(), Box<dyn Error>> {
    ...
    */
    ...
    builder.map_many(
        ExecuteOn::PostTick,
        vec![
            Variable::named("LEFT_FLAPS_1_POSITION_PERCENT"),
            Variable::named("LEFT_FLAPS_2_POSITION_PERCENT"),
        ],
        |values| values.iter().sum::<f64>() / (values.len() as f64),
    ...
    builder.map_many(
        ExecuteOn::PostTick,
        vec![
            Variable::named("RIGHT_FLAPS_1_POSITION_PERCENT"),
            Variable::named("RIGHT_FLAPS_2_POSITION_PERCENT"),
        ],
        |values| values.iter().sum::<f64>() / (values.len() as f64),
    ...
    builder.map_many(
        ExecuteOn::PostTick,
        vec![
            Variable::named("LEFT_SLATS_1_ANGLE"),
            Variable::named("LEFT_SLATS_2_ANGLE"),
            Variable::named("LEFT_SLATS_3_ANGLE"),
            Variable::named("LEFT_SLATS_4_ANGLE"),
            Variable::named("LEFT_SLATS_5_ANGLE"),
        ],
        |values| values.iter().sum::<f64>() / (values.len() as f64),
        Variable::named("LEFT_SLATS_ANGLE"),
    );
    builder.map_many(
        ExecuteOn::PostTick,
        vec![
            Variable::named("RIGHT_SLATS_1_ANGLE"),
            Variable::named("RIGHT_SLATS_2_ANGLE"),
            Variable::named("RIGHT_SLATS_3_ANGLE"),
            Variable::named("RIGHT_SLATS_4_ANGLE"),
            Variable::named("RIGHT_SLATS_5_ANGLE"),
        ],
        |values| values.iter().sum::<f64>() / (values.len() as f64),
        Variable::named("RIGHT_SLATS_ANGLE"),
    );

    builder.map_many(
        ExecuteOn::PostTick,
        vec![
            Variable::named("LEFT_FLAPS_1_ANGLE"),
            Variable::named("LEFT_FLAPS_2_ANGLE"),
        ],
        |values| values.iter().sum::<f64>() / (values.len() as f64),
        Variable::named("LEFT_FLAPS_ANGLE"),
    );
    builder.map_many(
        ExecuteOn::PostTick,
        vec![
            Variable::named("RIGHT_FLAPS_1_ANGLE"),
            Variable::named("RIGHT_FLAPS_2_ANGLE"),
        ],
        |values| values.iter().sum::<f64>() / (values.len() as f64),
        Variable::named("RIGHT_FLAPS_ANGLE"),
    );
