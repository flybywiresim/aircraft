...
import { FSComponent, DisplayComponent, Subject, VNode, EventBus } from 'msfssdk';
import { MathUtils } from '@shared/MathUtils';
import { rangeSettings } from '@shared/NavigationDisplay';
import { Arinc429Word } from '@shared/arinc429';
import { NDSimvars } from '../NDSimvarPublisher';

export class ArcModeOverlay extends DisplayComponent<{ bus: EventBus }> {
    ...
    private readonly trackUpMode = Subject.create<boolean>(false);
    ...
    private readonly headingFlagRef = FSComponent.createRef<SVGTextElement>();
    ...
    private handleRingRotation() {
        const isUsingTrackUpMode = this.trackUpMode.get();
    ...
    private handleHeadingFlagVisibility() {
        const flag = this.headingFlagRef.instance;

        const headingValid = !this.headingWord.get().isNormalOperation();

        flag.style.visibility = headingValid ? 'visible' : 'hidden';
    }

    private handleTrackFlagVisibility() {
        const isUsingTrackUpMode = this.trackUpMode.get();

        const flag = this.trackFlagRef.instance;

        if (isUsingTrackUpMode) {
            const trackValid = !this.trackWord.get().isNormalOperation();

            flag.style.visibility = trackValid ? 'visible' : 'hidden';
        } else {
...
class ArcModeOverlayHeadingRing extends DisplayComponent<{}> {
    render(): VNode | null {
        return (
            <>
                {/* R = 492 */}
