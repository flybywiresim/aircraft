...
export class FixInfoLayer implements MapLayer<NdSymbol> {
    ...
    private drawFixInfoRadial(context: CanvasRenderingContext2D, cx: number, cy: number, bearing: number, mapParameters: MapParameters, color: string, lineWidth: number) {
    ...
    private drawFixInfoRadius(context: CanvasRenderingContext2D, cx: number, cy: number, rPx: number, color: string, lineWidth: number) {
        context.strokeStyle = color;
        context.lineWidth = lineWidth;
