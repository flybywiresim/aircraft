...
export type RadioNavInfoProps = { index: 1 | 2, side: 'L' | 'R' }
...
const VorInfo: React.FC<{index: 1 | 2}> = ({ index }) => {
    ...
    const bigLittle = (value: number, digits: number) => {
        ...
        return <>{intPart}<tspan fontSize={20}>.{decimalPart}</tspan></>;
    };
    ...
    return (
        <g className="GtLayer">
            <path
                ...
                strokeLinecap="round"
            />
            <text x={x} y={692} fontSize={24} className="White">VOR{index}</text>
    ...
    return (
        <g className="GtLayer">
            <path
                ...
                strokeLinecap="round"
            />
            <text x={x} y={692} fontSize={24} className="Green">ADF{index}</text>
