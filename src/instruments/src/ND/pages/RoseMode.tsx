...
const Overlay: React.FC<OverlayProps> = ({ heading, track, rangeSetting, side, tcasMode, displayMode, selectedHeading, ilsCourse, lsDisplayed }) => {
    return (
        <>
...
const VorInfo: FC<{side: 'L' | 'R'}> = memo(({ side }) => {
    ...
    return (
        <Layer x={748} y={28}>
            <text x={-102} y={0} fontSize={25} className="White" textAnchor="end">VOR{index}</text>
            { vorAvailable && <text x={0} y={0} fontSize={25} className="White" textAnchor="end">{freqInt}<tspan fontSize={20}>.{freqDecimal}</tspan></text> }
            <text x={-56} y={30} fontSize={25} className="White" textAnchor="end">CRS</text>
            <text x={20} y={30} fontSize={25} className="Cyan" textAnchor="end">{vorCourse >= 0 ? ("" + vorCourse).padStart(3, "0") : "---"}&deg;</text>
...
const IlsInfo: FC = memo(() => {
    ...
    return (
        <Layer x={748} y={28}>
            ...
            { locAvailable && <text x={0} y={0} fontSize={25} className="Magenta" textAnchor="end">{freqInt}<tspan fontSize={20}>.{freqDecimal}</tspan></text> }
            <text x={-56} y={30} fontSize={25} className="White" textAnchor="end">CRS</text>
            <text x={20} y={30} fontSize={25} className="Magenta" textAnchor="end">{locAvailable ? ("" + ilsCourse).padStart(3, "0") : "---"}&deg;</text>
