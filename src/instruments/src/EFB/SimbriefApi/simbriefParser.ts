...
export const getSimbriefDataNew = (requestId: number): Promise<ISimbriefData> => (
    fetch(`${simbriefApiUrlV2.toString()}/flightplans/${requestId}`, getRequestData)
        .then((res) => {
            if (!res.ok) {
                return res.json().then((json) => Promise.reject(new Error(json.message)));
            }
            return res.json().then((json) => simbriefDataParserNew(json));
        })
);

const simbriefDataParserNew = (simbriefJson: any): ISimbriefData => {
    const {
        params,
        general,
        origin,
    ...
    return {
        ...
        units: params.units,
        alternate: {
            icao: alternate[0].icao_code,
            iata: alternate[0].iata_code,
            burn: alternate[0].burn,
        },
        times: {
