...
export default class RunwayVisualizationWidget extends React.Component<RunwayVisualizationProps, RunwayVisualizationState> {
	...
	private runwayLengthLabel(): JSX.Element {
		return (
			<div className="text-white absolute top-1/2 -right-3 transform -rotate-90 translate-x-1/2">{ this.props.mainLength ?? 0 }m</div>
		)
	}
		...
		for (let label of this.props.labels ?? []) {
			...
			elements.push(
				(<div className={"w-32 h-1 bg-white text-white absolute left-1/2 transform -translate-x-1/2 "
					...
					{showText && (
						<div className="w-full text-center absolute -top-0.5 transform -translate-y-full">{label.label} { Math.round(label.distance) }m</div>
					)}
		...
		return (
			<div className="h-full flex px-10">
				<div className="h-full flex flex-col relative">
					...
					<div className="bg-green-200 opacity-50" style={{height: `${this.todaHeightPercentage()}%`}}></div>
					<div className="bg-gray-700 opacity-50" style={{height: `${this.asdaHeightPercentage()}%`}}></div>
