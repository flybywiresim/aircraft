...
export default class LandingCalculator {
	...
	private calculateRequiredLandingDistance(weight: number, flaps: LandingFlapsConfig, runwayCondition: LandingRunwayConditions, autobrakeMode: AutobrakeMode,
		...
		let landingData = runwayConditionLandingData[runwayCondition][autobrakeMode][flaps];
		...
		let weightDifference = (weight / 1000) - 68;
		...
		let slopeCorrection = slope < 0
			...
			: 0;
		let temperatureCorrection = temperature > 15
			? ((temperature - 15) / 10) * landingData.tempCorrection
			: 0;
		let overweightProcCorrection = overweightProcedure
