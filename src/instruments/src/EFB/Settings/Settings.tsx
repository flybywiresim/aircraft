...
const SoundSettings: React.FC = () => {
    const [ptuAudible, setPtuAudible] = useState(() => getSimVar("L:A32NX_SOUND_PTU_AUDIBLE_COCKPIT", "Bool"));
    const [exteriorVolume, setExteriorVolume] = useState(() => getSimVar("L:A32NX_SOUND_EXTERIOR_MASTER", "number"));
    const [engineVolume, setEngineVolume] = useState(() => getSimVar("L:A32NX_SOUND_INTERIOR_ENGINE", "number"));
    const [windVolume, setWindVolume] = useState(() => getSimVar("L:A32NX_SOUND_INTERIOR_WIND", "number"));

    useEffect(() => setSimVar("L:A32NX_SOUND_PTU_AUDIBLE_COCKPIT", ptuAudible, "Bool"), [ptuAudible]);
    useEffect(() => setSimVar("L:A32NX_SOUND_EXTERIOR_MASTER", exteriorVolume - 50, "number"), [exteriorVolume]);
    useEffect(() => setSimVar("L:A32NX_SOUND_INTERIOR_ENGINE", engineVolume - 50, "number"), [engineVolume]);
    useEffect(() => setSimVar("L:A32NX_SOUND_INTERIOR_WIND", windVolume - 50, "number"), [windVolume]);
        ...
        <div className="bg-gray-800 rounded-xl px-6 py-4 shadow-lg">
            <div className="divide-y divide-gray-700 flex flex-col">
                ...
                <div className="mb-4 pt-3 flex flex-row justify-between items-center">
                    ...
                    <Slider value={exteriorVolume} onInput={value => setExteriorVolume(value)} className="w-60"/>
                </div>
                <div className="mb-4 pt-3 flex flex-row justify-between items-center">
                    <span className="text-lg text-gray-300">Engine Interior Volume</span>
                    <Slider value={engineVolume} onInput={value => setEngineVolume(value)} className="w-60"/>
                </div>
                <div className="pt-3 flex flex-row justify-between items-center">
                    <span className="text-lg text-gray-300">Wind Interior Volume</span>
                    <Slider value={windVolume} onInput={value => setWindVolume(value)} className="w-60"/>
                </div>
            </div>
        </div>
    );
