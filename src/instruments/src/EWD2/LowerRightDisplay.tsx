...
export class LowerRightDisplay extends DisplayComponent<LowerRightDisplayProps> {
    ...
    onAfterRender(node: VNode): void {
        ...
        sub.on('ewdLowerRight2').whenChanged().handle((m) => {
            this.line2 = padEWDCode(m);
        });

        sub.on('ewdLowerRight3').whenChanged().handle((m) => {
            this.line3 = padEWDCode(m);
        });

        sub.on('ewdLowerRight4').whenChanged().handle((m) => {
            this.line4 = padEWDCode(m);
        });

        sub.on('ewdLowerRight5').whenChanged().handle((m) => {
            this.line5 = padEWDCode(m);
        });

        sub.on('ewdLowerRight6').whenChanged().handle((m) => {
            this.line6 = padEWDCode(m);
        });

        sub.on('ewdLowerRight7').whenChanged().handle((m) => {
    ...
    render(): VNode {
        return (
            <FormattedFwcText message={this.message} x={520} y={554} />
        );
    }
}
