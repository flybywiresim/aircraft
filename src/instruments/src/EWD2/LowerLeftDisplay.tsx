...
export class LowerLeftDisplay extends DisplayComponent<LowerLeftDisplayProps> {
    ...
    onAfterRender(node: VNode): void {
        ...
        sub.on('ewdLowerLeft1').whenChanged().handle((m) => {
            this.line1 = padEWDCode(m);
        });

        sub.on('ewdLowerLeft2').whenChanged().handle((m) => {
            this.line2 = padEWDCode(m);
        });

        sub.on('ewdLowerLeft3').whenChanged().handle((m) => {
            this.line3 = padEWDCode(m);
        });

        sub.on('ewdLowerLeft4').whenChanged().handle((m) => {
            this.line4 = padEWDCode(m);
        });

        sub.on('ewdLowerLeft5').whenChanged().handle((m) => {
            this.line5 = padEWDCode(m);
        });

        sub.on('ewdLowerLeft6').whenChanged().handle((m) => {
            this.line6 = padEWDCode(m);
        });

        sub.on('ewdLowerLeft7').whenChanged().handle((m) => {
    ...
    render(): VNode {
        return (
            <FormattedFwcText message={this.message} x={14} y={554} />
        );
    }
}
