...
loopY.maxVolumeHighPressureSide=7;
loopY.res=3.3;    %
loopY.press=14.7;
loopY.delta_vol=0;
...
PTU.isActiveLeft=0;
PTU.isEnabled=0;
PTU.last_left_press = 0;
PTU.last_right_press = 0;
PTU.last_press_rate = 0;
% Yellow to Green
% /// ---------------
...
epump.flowRate=0;
epump.delta_vol=0;
epump.pressBreakpoints= [0 1000   2250   2500  2750   2900   2925   3000 3050 3500];
epump.displacementCarac=[0 0.263 0.263  0.224 0.195   0.195 0.125   0.120   0   0 ];
epump.minVol=0;
epump.maxVol=0;
epump.rpmMax=7600;
epump.spooltime=4;
...
for t=0:dt:maxTime
    ...
    [epump,loopY,actuator]= updateL(dt,epump,PTU,loopY,actuator);
...
linkaxes([ax_Press,ax_deltaVol,ax_Volume,ax_deltaFlow,ax_ResVol],'x');
...
function PTU=updatePTU(PTU,dt, loopLeft, loopRight)
    ...
    if PTU.isEnabled
        ...
        if PTU.isActiveLeft || (~PTU.isActiveRight && deltaP>500) %Left sends flow to right         
            vr = min(16,loopLeft.press * 0.0058) / 60.0 ;
            vr=min(vr,loopLeft.lastMaxFlow*0.6);

            vr=0.05*vr+0.95*PTU.last_flow;

            PTU.flowToLeftLoop= -vr;
            PTU.flowToRightLoop= vr * 0.9;
            PTU.last_flow=vr;


            PTU.isActiveLeft=1;
        elseif PTU.isActiveRight || (~PTU.isActiveLeft && deltaP<-500) %Right sends flow to left
            vr = min(34,loopRight.press * 0.0125) / 60.0 ;
            vr=min(vr,loopRight.lastMaxFlow*0.6);

            vr=0.05*vr+0.95*PTU.last_flow;

            PTU.flowToLeftLoop = vr * 0.9;
        ...
        if PTU.isActiveRight && loopLeft.press >= 3000 || PTU.isActiveLeft && loopRight.press >= 3000 ...
            ...
            PTU.isActiveLeft=0;
            PTU.last_flow=0;
            PTU.last_flow_rate=0;
        end
    end
    
        PTU.last_right_press=loopRight.press;
        PTU.last_left_press=loopLeft.press;
...
function [pump,loop,aileron]= updateL(dt,pump,PTU,loop,aileron)
    ...
    %END FOREACH PUMP
    loop.lastMaxFlow=deltaMaxVol/dt;
...
%    loop.press = max(14.7,loop.press + deltaPress_from_deltaVolume(deltavol,loop));
     tempPress = max(14.7,loop.press + deltaPress_from_deltaVolume(deltavol,loop));
     loop.press=0.3*loop.press+0.7*tempPress;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %Update Volumes
    loop.delta_vol=0.6*loop.delta_vol + 0.4*deltavol;
