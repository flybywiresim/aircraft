...
pub(super) struct A320FwcInputs {
    ...
    has_gs: VariableIdentifier,
    gs_deviation: VariableIdentifier,
}
...
impl A320FwcInputs {
    pub fn new(context: &mut InitContext, number: usize) -> Self {
        ...
        Self {
            ...
            has_gs: context.get_identifier("RADIO_RECEIVER_GS_IS_VALID".to_owned()),
            gs_deviation: context.get_identifier("RADIO_RECEIVER_GS_DEVIATION".to_owned()),
        }
    }
...
impl SimulationElement for A320FwcInputs {
    ...
    fn read(&mut self, reader: &mut SimulatorReader) {
        ...
        // MMR
        let has_glide_slope: bool = reader.read(&self.has_gs);
        let glide_slope_ssm = if has_glide_slope {
            SignStatus::NormalOperation
        } else {
            SignStatus::NoComputedData
        };
        let glide_slope_error = Ratio::new::<ratio>(reader.read(&self.gs_deviation));
        ...
        self.dmc_mda = reader.read(&self.minimum_descent_altitude_id);
    }
}
