...
mod a320_electrical_circuit_tests {
    ...
    use systems::{
        electrical::{
            ...
            ExternalPowerSource, NewPotential,
    ...
    impl SimulationElement for TestApu {
        fn receive_power(&mut self, buses: &impl ElectricalBuses) {
            self.start_motor_is_powered = buses.bus_is_powered(ElectricalBusType::Sub("49-42-00"));
        }
    }
    ...
    impl A320ElectricalTestAircraft {
        ...
        fn empty_battery_1(&mut self, electricity: &mut Electricity) {
            self.elec.empty_battery_1(electricity);
        }

        fn empty_battery_2(&mut self, electricity: &mut Electricity) {
            self.elec.empty_battery_2(electricity);
        }
        ...
        fn static_inverter_input<'a>(&self, electricity: &'a Electricity) -> &'a NewPotential {
            electricity.input_of(self.elec.direct_current.static_inverter())
        }

        fn tr_1_input<'a>(&self, electricity: &'a Electricity) -> &'a NewPotential {
            electricity.input_of(self.elec.tr_1())
        }

        fn tr_2_input<'a>(&self, electricity: &'a Electricity) -> &'a NewPotential {
            electricity.input_of(self.elec.tr_2())
        }

        fn tr_ess_input<'a>(&self, electricity: &'a Electricity) -> &'a NewPotential {
            electricity.input_of(self.elec.tr_ess())
        }

        fn battery_1_input<'a>(&self, electricity: &'a Electricity) -> &'a NewPotential {
            electricity.input_of(self.elec.battery_1())
        }

        fn battery_2_input<'a>(&self, electricity: &'a Electricity) -> &'a NewPotential {
    ...
    impl A320ElectricalTestBed {
        fn new() -> Self {
            let mut test_bed = SimulationTestBed::new();
            let mut aircraft = A320ElectricalTestAircraft::new(test_bed.electricity_mut());
        ...
        fn empty_battery_1(mut self) -> Self {
            self.aircraft
                .empty_battery_1(self.test_bed.electricity_mut());
            self
        }

        fn empty_battery_2(mut self) -> Self {
            self.aircraft
                .empty_battery_2(self.test_bed.electricity_mut());
            self
        }
