...
void FlyByWireModelClass::step()
{
  ...
  if (FlyByWire_DWork.Delay_DSTATE_i5 >= 25.0) {
    rtb_Y_h5 = rtb_BusAssignment_a_sim_data_pk_deg_s;
  ...
  if (rtb_Y_h5 > rtb_Sum1_c) {
    rtb_Sum1_c = rtb_Y_h5;
  }

  if (rtb_LimiteriH < rtb_Sum1_c) {
    rtb_Sum1_c = rtb_LimiteriH;
  }
  ...
  if ((FlyByWire_U.in.data.V_ias_kn <= 400.0) && (FlyByWire_U.in.data.V_ias_kn >= 0.0)) {
    ...
    rtb_Y_dd = (FlyByWire_U.in.data.V_ias_kn - static_cast<real_T>(b[low_i])) / static_cast<real_T>(b[low_i + 1] -
      b[low_i]);
    if (rtb_Y_dd == 0.0) {
      rtb_Y_dd = c[low_i];
      ...
      rtb_Y_dd = (1.0 - rtb_Y_dd) * static_cast<real_T>(c[low_i]) + static_cast<real_T>(c[low_i + 1]) * rtb_Y_dd;
    }
  }
