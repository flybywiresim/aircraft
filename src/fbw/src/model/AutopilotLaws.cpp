...
void AutopilotLawsModelClass::step()
{
  ...
  rtb_error_d = 0.017453292519943295 * rtb_GainTheta1;
  Phi2 = std::tan(b_R);
  a = std::sin(rtb_error_d);
  rtb_error_d = std::cos(rtb_error_d);
  result_tmp[0] = 1.0;
  result_tmp[3] = a * Phi2;
  result_tmp[6] = rtb_error_d * Phi2;
  result_tmp[1] = 0.0;
  result_tmp[4] = rtb_error_d;
  result_tmp[7] = -a;
  result_tmp[2] = 0.0;
  distance_m = std::cos(b_R);
  rtb_Divide = 1.0 / distance_m;
  result_tmp[5] = rtb_Divide * a;
  result_tmp[8] = rtb_Divide * rtb_error_d;
  ...
  for (rtb_on_ground = 0; rtb_on_ground < 3; rtb_on_ground++) {
    result[rtb_on_ground] = (result_tmp[rtb_on_ground + 3] * rtb_Saturation + result_tmp[rtb_on_ground] * rtb_dme) +
  ...
  result_tmp[1] = a * rtb_dme;
  result_tmp[4] = rtb_error_d;
  result_tmp[7] = distance_m * a;
  result_tmp[2] = rtb_error_d * rtb_dme;
  result_tmp[5] = 0.0 - a;
  result_tmp[8] = rtb_error_d * distance_m;
  for (rtb_on_ground = 0; rtb_on_ground < 3; rtb_on_ground++) {
  ...
  rtb_Divide = std::cos(rtb_error_d);
  rtb_error_d = std::sin(rtb_error_d);
  L = mod_mvZvttxs(mod_mvZvttxs(mod_mvZvttxs(std::atan2(std::sin(R) * L, rtb_Divide * std::sin(Phi2) - rtb_error_d * L *
  ...
  rtb_error_d = std::atan2(std::sin(rtb_Saturation) * L, rtb_Divide * std::sin(Phi2) - rtb_error_d * L * std::cos
    (rtb_Saturation)) * 57.295779513082323;
  if (rtb_error_d + 360.0 == 0.0) {
    rtb_Saturation = 0.0;
  } else {
    rtb_Saturation = std::fmod(rtb_error_d + 360.0, 360.0);
    if (rtb_Saturation == 0.0) {
      rtb_Saturation = 0.0;
    } else if (rtb_error_d + 360.0 < 0.0) {
  ...
  if (std::abs(distance_m / 1852.0) < 30.0) {
    if (AutopilotLaws_U.in.data.nav_loc_deg == 0.0) {
      ...
      rtb_error_d = std::fmod(AutopilotLaws_U.in.data.nav_loc_deg, 360.0);
      if (rtb_error_d == 0.0) {
        rtb_error_d = 0.0;
    ...
    if (rtb_error_d + 360.0 == 0.0) {
      ...
      rtb_error_d = std::fmod(rtb_error_d + 360.0, 360.0);
    }
    ...
    rtb_error_d = (rtb_error_d - (Phi2 + 360.0)) + 360.0;
    if (rtb_error_d == 0.0) {
      L = 0.0;
    } else {
  ...
  AutopilotLaws_DWork.DelayInput1_DSTATE = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE,
    AutopilotLaws_P.Constant3_Value_b);
  Phi2 = AutopilotLaws_U.in.data.nav_loc_deg - AutopilotLaws_U.in.data.nav_loc_magvar_deg;
  R = rt_modd(rt_modd(Phi2, AutopilotLaws_P.Constant3_Value_n) + AutopilotLaws_P.Constant3_Value_n,
  ...
  rtb_error_d = rt_modd(AutopilotLaws_P.Constant3_Value_i - b_R, AutopilotLaws_P.Constant3_Value_i);
  ...
  if (b_R < rtb_error_d) {
    ...
    b_R = AutopilotLaws_P.Gain_Gain * rtb_error_d;
  }
  ...
  AutopilotLaws_MATLABFunction(AutopilotLaws_P.tau_Value, AutopilotLaws_P.zeta_Value, &distance_m, &rtb_lo_k);
  ...
  rtb_Divide = std::sin(AutopilotLaws_P.Gain1_Gain_f * AutopilotLaws_U.in.data.nav_loc_error_deg) * b_R *
  ...
  rtb_error_d = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE, AutopilotLaws_P.Constant3_Value_p);
  if (b_R < rtb_error_d) {
    AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain1_Gain_p * b_R;
  } else {
    AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain_Gain_a * rtb_error_d;
  }

  AutopilotLaws_DWork.DelayInput1_DSTATE = (AutopilotLaws_P.Gain2_Gain_i * AutopilotLaws_DWork.DelayInput1_DSTATE +
    rtb_Divide) * distance_m;
  ...
  AutopilotLaws_LagFilter(AutopilotLaws_U.in.data.nav_loc_error_deg, AutopilotLaws_P.LagFilter2_C1,
    AutopilotLaws_U.in.time.dt, &distance_m, &AutopilotLaws_DWork.sf_LagFilter_h);
  b_R = AutopilotLaws_P.DiscreteDerivativeVariableTs_Gain * distance_m;
  ...
  AutopilotLaws_LagFilter(distance_m + AutopilotLaws_P.Gain3_Gain_i * AutopilotLaws_DWork.DelayInput1_DSTATE,
    AutopilotLaws_P.LagFilter_C1, AutopilotLaws_U.in.time.dt, &rtb_error_d, &AutopilotLaws_DWork.sf_LagFilter_m);
  ...
  rtb_Compare_l = (rtb_Saturation == AutopilotLaws_P.CompareToConstant_const_k);
  if (!rtb_Compare_l) {
  ...
  distance_m = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE, AutopilotLaws_P.Constant3_Value_n1);
  if (Phi2 < distance_m) {
    AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain1_Gain_j * Phi2;
  } else {
    AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain_Gain_i * distance_m;
  }
  ...
  Phi2 = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE, AutopilotLaws_P.Constant3_Value_nr);
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Constant3_Value_nr - Phi2;
  AutopilotLaws_DWork.DelayInput1_DSTATE = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE,
    AutopilotLaws_P.Constant3_Value_nr);
  AutopilotLaws_Chart_h(Phi2, AutopilotLaws_P.Gain_Gain_o * AutopilotLaws_DWork.DelayInput1_DSTATE,
                        AutopilotLaws_P.Constant1_Value_e, &rtb_dme, &AutopilotLaws_DWork.sf_Chart_b);
  ...
  Phi2 = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE, AutopilotLaws_P.Constant3_Value_m);
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Constant3_Value_m - Phi2;
  AutopilotLaws_DWork.DelayInput1_DSTATE = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE,
    AutopilotLaws_P.Constant3_Value_m);
  AutopilotLaws_Chart_h(Phi2, AutopilotLaws_P.Gain_Gain_fn * AutopilotLaws_DWork.DelayInput1_DSTATE,
  ...
  AutopilotLaws_Chart(R, AutopilotLaws_P.Gain_Gain_cy * AutopilotLaws_DWork.DelayInput1_DSTATE, rtb_valid != rtb_valid_d,
                      &Phi2, &AutopilotLaws_DWork.sf_Chart);
  ...
  AutopilotLaws_MATLABFunction_m(AutopilotLaws_U.in.input.Psi_c_deg, Phi2, rtb_Sum_gq, &b_L, &rtb_lo_b,
  ...
  AutopilotLaws_MATLABFunction_m(AutopilotLaws_U.in.input.Psi_c_deg, Phi2, rtb_Sum_i, &rtb_Y_f, &rtb_lo_k,
  ...
  AutopilotLaws_RateLimiter(AutopilotLaws_U.in.data.flight_guidance_phi_deg, AutopilotLaws_P.RateLimiterVariableTs_up_h,
    ...
    AutopilotLaws_P.RateLimiterVariableTs_InitialCondition_l, &Phi2, &AutopilotLaws_DWork.sf_RateLimiter);
  AutopilotLaws_LagFilter(Phi2, AutopilotLaws_P.LagFilter_C1_g, AutopilotLaws_U.in.time.dt,
  ...
  switch (static_cast<int32_T>(rtb_Saturation)) {
   ...
   case 1:
    if (rtb_Sum_gq > b_L) {
      rtb_Sum_gq = b_L;
    } else if (rtb_Sum_gq < rtb_lo_b) {
      rtb_Sum_gq = rtb_lo_b;
    }
    break;

   case 2:
    if (rtb_Sum_i > rtb_Y_f) {
      rtb_Sum_gq = rtb_Y_f;
   ...
   case 3:
    rtb_Cos1_fn = AutopilotLaws_P.Gain_Gain_c * AutopilotLaws_U.in.data.flight_guidance_xtk_nmi * rtb_Divide /
      AutopilotLaws_U.in.data.V_gnd_kn;
    if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat) {
      rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat;
    } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat) {
      rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat;
    }

    rtb_Sum_gq = AutopilotLaws_DWork.DelayInput1_DSTATE - (AutopilotLaws_P.Gain2_Gain *
      AutopilotLaws_U.in.data.flight_guidance_tae_deg + rtb_Cos1_fn) * R * AutopilotLaws_U.in.data.V_gnd_kn;
    break;
   ...
   case 5:
    Phi2 = rt_modd((AutopilotLaws_U.in.data.Psi_magnetic_deg - (AutopilotLaws_U.in.data.Psi_true_deg +
      AutopilotLaws_P.Constant3_Value)) + AutopilotLaws_P.Constant3_Value, AutopilotLaws_P.Constant3_Value);
    R = rt_modd(AutopilotLaws_P.Constant3_Value - Phi2, AutopilotLaws_P.Constant3_Value);
    if (Phi2 < R) {
      Phi2 *= AutopilotLaws_P.Gain1_Gain_l;
    } else {
      Phi2 = AutopilotLaws_P.Gain_Gain_g * R;
    }

    L = rt_modd((rt_modd(rt_modd(AutopilotLaws_U.in.data.Psi_magnetic_track_deg + Phi2,
    ...
    Phi2 = rt_modd(AutopilotLaws_P.Constant3_Value_c - L, AutopilotLaws_P.Constant3_Value_c);
    ...
    if (L < Phi2) {
      L *= AutopilotLaws_P.Gain1_Gain_g;
    } else {
      L = AutopilotLaws_P.Gain_Gain_f * Phi2;
    }

    if (rtb_Compare_jy) {
      Phi2 = AutopilotLaws_P.k_beta_Phi_Gain * AutopilotLaws_U.in.data.beta_deg;
    } else {
      Phi2 = AutopilotLaws_P.Constant1_Value_fk;
    }

    rtb_Cos1_fn = (rtb_error_d * look1_binlxpw(AutopilotLaws_U.in.data.V_tas_kn,
      ...
      AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1, AutopilotLaws_P.ScheduledGain_Table, 5U) + std::sin
                   (AutopilotLaws_P.Gain1_Gain_b * L) * AutopilotLaws_U.in.data.V_gnd_kn * AutopilotLaws_P.Gain2_Gain_g)
      + Phi2;
    if (rtb_Cos1_fn > AutopilotLaws_P.Saturation1_UpperSat) {
      rtb_Cos1_fn = AutopilotLaws_P.Saturation1_UpperSat;
    } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation1_LowerSat) {
      rtb_Cos1_fn = AutopilotLaws_P.Saturation1_LowerSat;
    }

    rtb_Sum_gq = (AutopilotLaws_P.Constant_Value - rtb_Y_pf) * a + rtb_Cos1_fn * rtb_Y_pf;
    break;
  ...
  rtb_error_d = std::abs(AutopilotLaws_U.in.data.V_tas_kn);
  if (rtb_error_d > 600.0) {
    rtb_error_d = 19.0;
  } else {
    int32_T low_i;
    int32_T low_ip1;
    ...
    while (i > low_ip1) {
      int32_T mid_i;
      mid_i = (low_i + i) >> 1;
      if (rtb_error_d >= (static_cast<real_T>(mid_i) - 1.0) * 150.0) {
        low_i = mid_i;
        low_ip1 = mid_i + 1;
      } else {
        i = mid_i;
    ...
    rtb_error_d = (rtb_error_d - (static_cast<real_T>(low_i) - 1.0) * 150.0) / static_cast<real_T>(150 * low_i - (low_i
      - 1) * 150);
    if (rtb_error_d == 0.0) {
      rtb_error_d = b[low_i - 1];
    } else if (rtb_error_d == 1.0) {
      rtb_error_d = b[low_i];
    } else if (b[low_i - 1] == b[low_i]) {
      rtb_error_d = b[low_i - 1];
    } else {
      rtb_error_d = (1.0 - rtb_error_d) * static_cast<real_T>(b[low_i - 1]) + rtb_error_d * static_cast<real_T>(b[low_i]);
    }
  }
  ...
  if ((AutopilotLaws_U.in.input.lateral_mode != 30.0) && (AutopilotLaws_U.in.input.lateral_mode != 31.0) &&
    ...
    rtb_error_d = std::fmin(25.0, rtb_error_d);
  ...
  AutopilotLaws_DWork.pY_b += std::fmax(std::fmin(rtb_error_d - AutopilotLaws_DWork.pY_b, 5.0 *
    AutopilotLaws_U.in.time.dt), -5.0 * AutopilotLaws_U.in.time.dt);
  if (rtb_Sum_gq > AutopilotLaws_DWork.pY_b) {
  ...
  AutopilotLaws_LagFilter(AutopilotLaws_P.Gain_Gain_lu * (AutopilotLaws_DWork.DelayInput1_DSTATE - rtb_GainTheta1),
    AutopilotLaws_P.LagFilter_C1_a, AutopilotLaws_U.in.time.dt, &rtb_error_d, &AutopilotLaws_DWork.sf_LagFilter_mp);
  AutopilotLaws_RateLimiter_n(rtb_Compare_l, AutopilotLaws_P.RateLimiterVariableTs_up_n,
    ...
    AutopilotLaws_P.RateLimiterVariableTs_InitialCondition_i, &rtb_Y_f, &AutopilotLaws_DWork.sf_RateLimiter_e);
  if (rtb_Y_f > AutopilotLaws_P.Saturation_UpperSat_k) {
    rtb_Divide = AutopilotLaws_P.Saturation_UpperSat_k;
  } else if (rtb_Y_f < AutopilotLaws_P.Saturation_LowerSat_f3) {
    rtb_Divide = AutopilotLaws_P.Saturation_LowerSat_f3;
  } else {
    rtb_Divide = rtb_Y_f;
  }

  rtb_dme = (AutopilotLaws_P.Gain_Gain_b * result[2] * rtb_Divide + (AutopilotLaws_P.Constant_Value_a - rtb_Divide) *
             (AutopilotLaws_P.Gain4_Gain_o * AutopilotLaws_U.in.data.beta_deg)) + AutopilotLaws_P.Gain5_Gain_o * rtb_dme;
  if (rtb_fpmtoms > AutopilotLaws_P.Switch_Threshold_n) {
    switch (static_cast<int32_T>(rtb_Saturation)) {
     ...
     default:
      R = AutopilotLaws_P.Gain3_Gain * rtb_dme;
      break;
    }
  } else {
  ...
  if (rtb_Compare_jy) {
    Phi2 = AutopilotLaws_P.Gain_Gain_ae * distance_m + AutopilotLaws_P.Gain1_Gain_k * AutopilotLaws_U.in.data.beta_deg;
  } else {
    Phi2 = AutopilotLaws_P.Constant1_Value_fk;
  }

  AutopilotLaws_LagFilter(Phi2, AutopilotLaws_P.LagFilter1_C1, AutopilotLaws_U.in.time.dt, &rtb_Y_f,
  ...
  switch (static_cast<int32_T>(rtb_Saturation)) {
   ...
   case 5:
    if (rtb_Y_f > AutopilotLaws_P.Saturation_UpperSat_e) {
      rtb_Saturation = AutopilotLaws_P.Saturation_UpperSat_e;
    } else if (rtb_Y_f < AutopilotLaws_P.Saturation_LowerSat_f) {
   ...
   default:
    rtb_Saturation = AutopilotLaws_P.Gain7_Gain * rtb_dme;
    break;
  }

  AutopilotLaws_LagFilter(rtb_Saturation, AutopilotLaws_P.LagFilter_C1_k, AutopilotLaws_U.in.time.dt, &b_L,
  ...
  AutopilotLaws_DWork.pY += std::fmax(std::fmin(rtb_Divide - AutopilotLaws_DWork.pY, 2.5 * AutopilotLaws_U.in.time.dt),
  ...
  AutopilotLaws_RateLimiter(static_cast<real_T>(rtb_fpmtoms), AutopilotLaws_P.RateLimiterVariableTs_up_b,
    ...
    AutopilotLaws_P.RateLimiterVariableTs_InitialCondition_il, &rtb_Y_f, &AutopilotLaws_DWork.sf_RateLimiter_d);
  if (rtb_Y_f > AutopilotLaws_P.Saturation_UpperSat_m) {
    AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Saturation_UpperSat_m;
  } else if (rtb_Y_f < AutopilotLaws_P.Saturation_LowerSat_fw) {
    AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Saturation_LowerSat_fw;
  } else {
    AutopilotLaws_DWork.DelayInput1_DSTATE = rtb_Y_f;
  }

  rtb_dme = rtb_Y_pf * AutopilotLaws_DWork.DelayInput1_DSTATE;
  ...
  rtb_Cos1_fn = AutopilotLaws_P.Gain_Gain_m3 * R;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_c) {
    AutopilotLaws_Y.out.output.Nosewheel_c = AutopilotLaws_P.Saturation_UpperSat_c;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_d) {
    AutopilotLaws_Y.out.output.Nosewheel_c = AutopilotLaws_P.Saturation_LowerSat_d;
  } else {
    AutopilotLaws_Y.out.output.Nosewheel_c = rtb_Cos1_fn;
  }

  AutopilotLaws_Y.out.output.flight_director.Beta_c_deg = b_L;
  AutopilotLaws_Y.out.output.autopilot.Beta_c_deg = rtb_Saturation;
  AutopilotLaws_Y.out.output.flight_director.Phi_c_deg = rtb_error_d;
  AutopilotLaws_Y.out.output.autopilot.Phi_c_deg = AutopilotLaws_DWork.DelayInput1_DSTATE;
  AutopilotLaws_WashoutFilter(rtb_GainTheta, AutopilotLaws_P.WashoutFilter_C1, AutopilotLaws_U.in.time.dt, &rtb_error_d,
  ...
  AutopilotLaws_LagFilter(AutopilotLaws_B.u - AutopilotLaws_U.in.data.H_ft, AutopilotLaws_P.LagFilter_C1_ai,
    AutopilotLaws_U.in.time.dt, &rtb_Y_f, &AutopilotLaws_DWork.sf_LagFilter_g);
  AutopilotLaws_DWork.DelayInput1_DSTATE += AutopilotLaws_P.Gain_Gain_ft * rtb_Y_f;
  ...
  rtb_Cos1_fn = Phi2 / AutopilotLaws_DWork.DelayInput1_DSTATE;
  if (rtb_Cos1_fn > 1.0) {
    rtb_Cos1_fn = 1.0;
  } else if (rtb_Cos1_fn < -1.0) {
    rtb_Cos1_fn = -1.0;
  }

  a = AutopilotLaws_P.Gain_Gain_k * std::asin(rtb_Cos1_fn);
  ...
  Phi2 = AutopilotLaws_P.Gain_Gain_es * std::asin(rtb_Cos1_fn);
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.fpmtoms_Gain * AutopilotLaws_U.in.data.H_dot_ft_min;
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_m * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_j) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_j;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_i) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_i;
  }

  AutopilotLaws_DWork.DelayInput1_DSTATE = std::atan(AutopilotLaws_DWork.DelayInput1_DSTATE / rtb_Cos1_fn) *
  ...
  AutopilotLaws_WashoutFilter(AutopilotLaws_P._Gain * (AutopilotLaws_P.GStoGS_CAS_Gain * (AutopilotLaws_P.ktstomps_Gain *
    AutopilotLaws_U.in.data.V_gnd_kn)), AutopilotLaws_P.WashoutFilter_C1_e, AutopilotLaws_U.in.time.dt, &rtb_Y_f,
    &AutopilotLaws_DWork.sf_WashoutFilter);
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_b * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_ei) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_ei;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_dz) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_dz;
  }

  AutopilotLaws_LeadLagFilter(rtb_Y_f - AutopilotLaws_P.g_Gain * (AutopilotLaws_P.Gain1_Gain_lp *
    ...
    rtb_Cos1_fn))) * (AutopilotLaws_P.Constant_Value_dy - std::cos(R)) + std::sin(R) * std::sin
  ...
  AutopilotLaws_LeadLagFilter(AutopilotLaws_P.ktstomps_Gain_b * AutopilotLaws_U.in.data.V_ias_kn,
    ...
    AutopilotLaws_P.LowPassFilter_C4, AutopilotLaws_U.in.time.dt, &rtb_Y_f, &AutopilotLaws_DWork.sf_LeadLagFilter_o);
  R = (rtb_Y_pf + rtb_Y_f) * AutopilotLaws_P.ug_Gain;
  ...
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_p * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_h) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_h;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_e) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_e;
  }
  ...
  AutopilotLaws_WashoutFilter(AutopilotLaws_P._Gain_h * (AutopilotLaws_P.GStoGS_CAS_Gain_m *
    ...
    AutopilotLaws_U.in.time.dt, &rtb_Y_f, &AutopilotLaws_DWork.sf_WashoutFilter_d);
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_l * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_i) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_i;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_h) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_h;
  }

  AutopilotLaws_LeadLagFilter(rtb_Y_f - AutopilotLaws_P.g_Gain_h * (AutopilotLaws_P.Gain1_Gain_dv *
    ...
    rtb_Cos1_fn))) * (AutopilotLaws_P.Constant_Value_c - std::cos(R)) + std::sin(R) * std::sin
  ...
  AutopilotLaws_LeadLagFilter(AutopilotLaws_P.ktstomps_Gain_i * AutopilotLaws_U.in.data.V_ias_kn,
    ...
    AutopilotLaws_P.LowPassFilter_C4_o, AutopilotLaws_U.in.time.dt, &rtb_Y_f, &AutopilotLaws_DWork.sf_LeadLagFilter_m);
  R = (rtb_Y_pf + rtb_Y_f) * AutopilotLaws_P.ug_Gain_a;
  rtb_Divide = AutopilotLaws_P.Gain1_Gain_hm * AutopilotLaws_DWork.DelayInput1_DSTATE;
  L = R + rtb_Divide;
  rtb_Sum_gq = AutopilotLaws_P.Constant1_Value_b4 - AutopilotLaws_P.Constant2_Value_c;
  rtb_lo_k = (AutopilotLaws_P.Gain1_Gain_mz * R + rtb_Divide) * AutopilotLaws_P.Gain_Gain_ie;
  if (rtb_Sum_gq > AutopilotLaws_P.Switch_Threshold_b) {
    R = AutopilotLaws_P.Constant1_Value_a;
  } else {
    R = AutopilotLaws_P.Gain5_Gain_l * rtb_lo_k;
  }

  rtb_Y_f = AutopilotLaws_U.in.data.V_ias_kn - AutopilotLaws_U.in.data.VMAX_kn;
  rtb_lo_b = rtb_Y_f * AutopilotLaws_P.Gain1_Gain_f1;
  if (rtb_lo_b <= R) {
    if (rtb_Sum_gq > AutopilotLaws_P.Switch1_Threshold_f) {
      R = AutopilotLaws_P.Constant_Value_p;
    } else {
      R = AutopilotLaws_P.Gain6_Gain_j * rtb_lo_k;
    }
  ...
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_f * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_eik) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_eik;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_ad) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_ad;
  }
  ...
  AutopilotLaws_WashoutFilter(AutopilotLaws_P._Gain_m * (AutopilotLaws_P.GStoGS_CAS_Gain_l * Phi2),
    AutopilotLaws_P.WashoutFilter_C1_k, AutopilotLaws_U.in.time.dt, &b_L, &AutopilotLaws_DWork.sf_WashoutFilter_n);
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_a * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_f) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_f;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_c) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_c;
  }

  AutopilotLaws_LeadLagFilter(b_L - AutopilotLaws_P.g_Gain_j * (AutopilotLaws_P.Gain1_Gain_ca *
    ...
    rtb_Cos1_fn))) * (AutopilotLaws_P.Constant_Value_l - rtb_lo_k) + rtb_lo_b * std::sin(AutopilotLaws_P.Gain1_Gain_id *
  ...
  AutopilotLaws_LeadLagFilter(AutopilotLaws_P.ktstomps_Gain_j * AutopilotLaws_U.in.data.V_ias_kn,
    ...
    AutopilotLaws_P.LowPassFilter_C4_b, AutopilotLaws_U.in.time.dt, &b_L, &AutopilotLaws_DWork.sf_LeadLagFilter_ja);
  Phi2 = (Phi2 + b_L) * AutopilotLaws_P.ug_Gain_o;
  b_L = (AutopilotLaws_P.Gain1_Gain_hu * Phi2 + R) * AutopilotLaws_P.Gain_Gain_bn;
  ...
  rtb_Compare_jy = ((distance_m > AutopilotLaws_P.CompareToConstant6_const) && (b_L <
    AutopilotLaws_P.CompareToConstant5_const_a) && (rtb_lo_k < AutopilotLaws_P.CompareToConstant2_const_d) && (rtb_dme ==
  ...
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_n * AutopilotLaws_U.in.data.V_tas_kn;
  if (distance_m < 0.0) {
    Phi2 = -1.0;
  } else if (distance_m > 0.0) {
    Phi2 = 1.0;
  ...
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_ju) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_ju;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_gw) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_gw;
  }

  rtb_Cos1_fn = (Phi2 * AutopilotLaws_P.Constant3_Value_ix - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_d / rtb_Cos1_fn;
  if (rtb_Cos1_fn > 1.0) {
    rtb_Cos1_fn = 1.0;
  } else if (rtb_Cos1_fn < -1.0) {
    rtb_Cos1_fn = -1.0;
  ...
  rtb_Sum_i = AutopilotLaws_P.Gain_Gain_nz * std::asin(rtb_Cos1_fn);
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_au * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_l) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_l;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_hm) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_hm;
  }

  rtb_Cos1_fn = (AutopilotLaws_U.in.input.H_dot_c_fpm - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_l / rtb_Cos1_fn;
  if (rtb_Cos1_fn > 1.0) {
    rtb_Cos1_fn = 1.0;
  } else if (rtb_Cos1_fn < -1.0) {
    rtb_Cos1_fn = -1.0;
  ...
  rtb_Gain_dn = AutopilotLaws_P.Gain_Gain_ey * std::asin(rtb_Cos1_fn);
  ...
  rtb_Y_pf = rtb_lo_k * rtb_Divide * 57.295779513082323;
  AutopilotLaws_DWork.prevVerticalLaw = AutopilotLaws_U.in.input.vertical_law;
  AutopilotLaws_DWork.prevTarget = AutopilotLaws_U.in.input.H_dot_c_fpm;
  ...
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_d * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_hb) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_hb;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_k) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_k;
  }

  R = AutopilotLaws_DWork.DelayInput1_DSTATE - std::atan(AutopilotLaws_P.fpmtoms_Gain_e *
    AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Cos1_fn) * AutopilotLaws_P.Gain_Gain_in * AutopilotLaws_P.Gain1_Gain_ps;
  ...
  AutopilotLaws_WashoutFilter(AutopilotLaws_P._Gain_k * (AutopilotLaws_P.GStoGS_CAS_Gain_k *
    AutopilotLaws_DWork.DelayInput1_DSTATE), AutopilotLaws_P.WashoutFilter_C1_o, AutopilotLaws_U.in.time.dt, &b_L,
    &AutopilotLaws_DWork.sf_WashoutFilter_fs);
  AutopilotLaws_LeadLagFilter(b_L - AutopilotLaws_P.g_Gain_m * (AutopilotLaws_P.Gain1_Gain_kdq *
  ...
  AutopilotLaws_LeadLagFilter(AutopilotLaws_P.ktstomps_Gain_c * AutopilotLaws_U.in.data.V_ias_kn,
    ...
    AutopilotLaws_P.LowPassFilter_C4_k, AutopilotLaws_U.in.time.dt, &b_L, &AutopilotLaws_DWork.sf_LeadLagFilter_kq);
  AutopilotLaws_DWork.DelayInput1_DSTATE = (AutopilotLaws_DWork.DelayInput1_DSTATE + b_L) * AutopilotLaws_P.ug_Gain_aa;
  ...
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_l5 * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_b3) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_b3;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_es) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_es;
  }

  R = AutopilotLaws_DWork.DelayInput1_DSTATE - std::atan(AutopilotLaws_P.fpmtoms_Gain_j *
    AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Cos1_fn) * AutopilotLaws_P.Gain_Gain_e5 * AutopilotLaws_P.Gain1_Gain_ja;
  ...
  AutopilotLaws_WashoutFilter(AutopilotLaws_P._Gain_i * (AutopilotLaws_P.GStoGS_CAS_Gain_n *
    AutopilotLaws_DWork.DelayInput1_DSTATE), AutopilotLaws_P.WashoutFilter_C1_p, AutopilotLaws_U.in.time.dt, &b_L,
    &AutopilotLaws_DWork.sf_WashoutFilter_j);
  AutopilotLaws_LeadLagFilter(b_L - AutopilotLaws_P.g_Gain_g * (AutopilotLaws_P.Gain1_Gain_hv *
    (AutopilotLaws_P.Gain_Gain_mx * (R * (AutopilotLaws_P.Constant_Value_ia - rtb_Cos_i) + rtb_Cos1_pk * std::sin
    (rtb_Divide)))), AutopilotLaws_P.HighPassFilter_C1_n, AutopilotLaws_P.HighPassFilter_C2_m,
  ...
  AutopilotLaws_LeadLagFilter(AutopilotLaws_P.ktstomps_Gain_o * AutopilotLaws_U.in.data.V_ias_kn,
    ...
    AutopilotLaws_P.LowPassFilter_C4_d, AutopilotLaws_U.in.time.dt, &b_L, &AutopilotLaws_DWork.sf_LeadLagFilter_p);
  AutopilotLaws_DWork.DelayInput1_DSTATE = (AutopilotLaws_DWork.DelayInput1_DSTATE + b_L) * AutopilotLaws_P.ug_Gain_f;
  R = AutopilotLaws_P.Gain1_Gain_ot * Phi2;
  b_L = AutopilotLaws_DWork.DelayInput1_DSTATE + R;
  ...
  rtb_Cos1_pk = rtb_Y_f * AutopilotLaws_P.Gain1_Gain_gy;
  ...
  Phi2 = (AutopilotLaws_P.Gain_Gain_dm * b_L - Phi2) + AutopilotLaws_DWork.DelayInput1_DSTATE;
  AutopilotLaws_SpeedProtectionSignalSelection(&AutopilotLaws_Y.out, rtb_Gain_dn, std::fmax(-rtb_Y_pf, std::fmin
    (rtb_Y_pf, AutopilotLaws_P.VS_Gain_h * rtb_Gain_dn)), rtb_Add3_g, AutopilotLaws_P.Gain_Gain_h4 * rtb_Add3_g, Phi2,
  ...
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_k * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_pj) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_pj;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_py) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_py;
  }

  R = AutopilotLaws_DWork.DelayInput1_DSTATE - std::atan(AutopilotLaws_P.fpmtoms_Gain_f *
    AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Cos1_fn) * AutopilotLaws_P.Gain_Gain_bf * AutopilotLaws_P.Gain1_Gain_jv;
  ...
  rtb_Add3_hx = AutopilotLaws_P.Gain1_Gain_j4 * AutopilotLaws_U.in.data.Psi_magnetic_track_deg -
    AutopilotLaws_DWork.DelayInput1_DSTATE;
  ...
  AutopilotLaws_WashoutFilter(AutopilotLaws_P._Gain_kb * (AutopilotLaws_P.GStoGS_CAS_Gain_o *
    AutopilotLaws_DWork.DelayInput1_DSTATE), AutopilotLaws_P.WashoutFilter_C1_j, AutopilotLaws_U.in.time.dt, &b_L,
  ...
  AutopilotLaws_LeadLagFilter(b_L - AutopilotLaws_P.g_Gain_l * (AutopilotLaws_P.Gain1_Gain_n4 *
    (AutopilotLaws_P.Gain_Gain_bc * (R * (AutopilotLaws_P.Constant_Value_lf - rtb_Add3_g) + rtb_Divide * std::sin
    (rtb_Add3_hx)))), AutopilotLaws_P.HighPassFilter_C1_i, AutopilotLaws_P.HighPassFilter_C2_h,
  ...
  AutopilotLaws_LeadLagFilter(AutopilotLaws_P.ktstomps_Gain_k * AutopilotLaws_U.in.data.V_ias_kn,
    ...
    AutopilotLaws_P.LowPassFilter_C4_dt, AutopilotLaws_U.in.time.dt, &b_L, &AutopilotLaws_DWork.sf_LeadLagFilter_kp);
  AutopilotLaws_DWork.DelayInput1_DSTATE = (AutopilotLaws_DWork.DelayInput1_DSTATE + b_L) * AutopilotLaws_P.ug_Gain_n;
  R = AutopilotLaws_P.Gain1_Gain_b1 * Phi2;
  rtb_Add3_g = AutopilotLaws_DWork.DelayInput1_DSTATE + R;
  rtb_Divide = AutopilotLaws_P.Constant3_Value_nk - AutopilotLaws_P.Constant4_Value_o;
  R = (AutopilotLaws_P.Gain1_Gain_on * AutopilotLaws_DWork.DelayInput1_DSTATE + R) * AutopilotLaws_P.Gain_Gain_hy;
  if (rtb_Divide > AutopilotLaws_P.Switch_Threshold_d) {
  ...
  if (b_L <= AutopilotLaws_DWork.DelayInput1_DSTATE) {
    if (rtb_Divide > AutopilotLaws_P.Switch1_Threshold_d) {
  ...
  rtb_Add3_hx = (AutopilotLaws_P.Gain_Gain_d0 * rtb_Add3_g - Phi2) + AutopilotLaws_DWork.DelayInput1_DSTATE;
  ...
  rtb_Add3_e5 = AutopilotLaws_P.Gain1_Gain_ip * AutopilotLaws_U.in.data.Psi_magnetic_track_deg -
    AutopilotLaws_DWork.DelayInput1_DSTATE;
  ...
  AutopilotLaws_WashoutFilter(AutopilotLaws_P._Gain_ip * (AutopilotLaws_P.GStoGS_CAS_Gain_e *
    AutopilotLaws_DWork.DelayInput1_DSTATE), AutopilotLaws_P.WashoutFilter_C1_c, AutopilotLaws_U.in.time.dt, &b_L,
  ...
  AutopilotLaws_LeadLagFilter(b_L - AutopilotLaws_P.g_Gain_hq * (AutopilotLaws_P.Gain1_Gain_mx *
    (AutopilotLaws_P.Gain_Gain_d3 * (R * (AutopilotLaws_P.Constant_Value_fo - rtb_Add3_g) + rtb_Divide * std::sin
    (rtb_Add3_e5)))), AutopilotLaws_P.HighPassFilter_C1_d, AutopilotLaws_P.HighPassFilter_C2_i,
  ...
  AutopilotLaws_DWork.DelayInput1_DSTATE = (AutopilotLaws_DWork.DelayInput1_DSTATE + b_L) * AutopilotLaws_P.ug_Gain_e;
  R = AutopilotLaws_P.Gain1_Gain_be * Phi2;
  b_L = AutopilotLaws_DWork.DelayInput1_DSTATE + R;
  rtb_Add3_g = AutopilotLaws_P.Constant1_Value_o - AutopilotLaws_P.Constant2_Value_h;
  ...
  rtb_Y_f *= AutopilotLaws_P.Gain1_Gain_fle;
  if (rtb_Y_f <= AutopilotLaws_DWork.DelayInput1_DSTATE) {
  ...
  Phi2 = (AutopilotLaws_P.Gain_Gain_gx * b_L - Phi2) + AutopilotLaws_DWork.DelayInput1_DSTATE;
  AutopilotLaws_SpeedProtectionSignalSelection(&AutopilotLaws_Y.out, rtb_Gain_dn, std::fmax(-rtb_Y_pf, std::fmin
    (rtb_Y_pf, AutopilotLaws_P.Gain_Gain_c3 * rtb_Gain_dn)), rtb_Add3_hx, AutopilotLaws_P.Gain_Gain_fnw * rtb_Add3_hx,
    Phi2, AutopilotLaws_P.Gain_Gain_ko * Phi2, AutopilotLaws_P.Constant_Value_fov, &rtb_FD_h, &rtb_Add3_g);
  rtb_Gain_dn = AutopilotLaws_P.Gain2_Gain_o * AutopilotLaws_U.in.data.H_dot_ft_min *
    AutopilotLaws_P.DiscreteDerivativeVariableTs1_Gain;
  AutopilotLaws_LagFilter((rtb_Gain_dn - AutopilotLaws_DWork.Delay_DSTATE_m) / AutopilotLaws_U.in.time.dt,
  ...
  AutopilotLaws_LagFilter(AutopilotLaws_P.Gain_Gain_gn * (std::tan(AutopilotLaws_P.Gain1_Gain_j2 * result[1]) * Phi2),
    AutopilotLaws_P.LagFilter3_C1, AutopilotLaws_U.in.time.dt, &b_L, &AutopilotLaws_DWork.sf_LagFilter_an);
  AutopilotLaws_LagFilter(AutopilotLaws_DWork.DelayInput1_DSTATE - b_L, AutopilotLaws_P.LagFilter4_C1,
  ...
  Phi2 = AutopilotLaws_P.Gain4_Gain_j * b_L;
  ...
  if (Phi2 > AutopilotLaws_P.Saturation_UpperSat_e5) {
    Phi2 = AutopilotLaws_P.Saturation_UpperSat_e5;
  } else if (Phi2 < AutopilotLaws_P.Saturation_LowerSat_kp) {
    Phi2 = AutopilotLaws_P.Saturation_LowerSat_kp;
  }

  AutopilotLaws_SignalEnablerGSTrack(Phi2, rtb_Compare_l, &b_L);
  AutopilotLaws_LagFilter(AutopilotLaws_U.in.data.nav_gs_error_deg, AutopilotLaws_P.LagFilter1_C1_p,
    AutopilotLaws_U.in.time.dt, &AutopilotLaws_DWork.DelayInput1_DSTATE, &AutopilotLaws_DWork.sf_LagFilter_cu);
  AutopilotLaws_SignalEnablerGSTrack(AutopilotLaws_P.Gain3_Gain_c * (AutopilotLaws_P.Gain2_Gain_m * b_L +
    AutopilotLaws_P.Gain_Gain_oq * (AutopilotLaws_DWork.DelayInput1_DSTATE * look1_binlxpw
    (AutopilotLaws_U.in.data.H_radio_ft, AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1_e,
     AutopilotLaws_P.ScheduledGain_Table_l, 7U))), (AutopilotLaws_U.in.data.H_radio_ft >
    AutopilotLaws_P.CompareToConstant_const_kt) && AutopilotLaws_U.in.data.nav_gs_valid, &rtb_Divide);
  AutopilotLaws_storevalue(rtb_dme == AutopilotLaws_P.CompareToConstant6_const_e, AutopilotLaws_Y.out.data.nav_gs_deg,
    &b_L, &AutopilotLaws_DWork.sf_storevalue_g);
  ...
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_k4 * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_eb) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_eb;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_gk) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_gk;
  }
  ...
  AutopilotLaws_SignalEnablerGSTrack(AutopilotLaws_P.Gain2_Gain_l * (Phi2 - AutopilotLaws_DWork.DelayInput1_DSTATE),
    rtb_Compare_l, &b_L);
  AutopilotLaws_Voter1(rtb_Divide + b_L, AutopilotLaws_P.Gain1_Gain_d4 * ((Phi2 + AutopilotLaws_P.Bias_Bias) -
  ...
  rtb_Add3_hx = AutopilotLaws_P.Gain5_Gain_c * AutopilotLaws_U.in.data.bz_m_s2;
  ...
  rtb_Compare_l = (rtb_dme == AutopilotLaws_P.CompareToConstant7_const);
  AutopilotLaws_LagFilter(AutopilotLaws_U.in.data.H_dot_ft_min, AutopilotLaws_P.LagFilterH_C1,
    AutopilotLaws_U.in.time.dt, &rtb_Y_f, &AutopilotLaws_DWork.sf_LagFilter_a);
  Phi2 = rtb_Y_f - AutopilotLaws_P.kntofpm_Gain * AutopilotLaws_U.in.data.V_gnd_kn * AutopilotLaws_P.maxslope_Gain;
  AutopilotLaws_LeadLagFilter(AutopilotLaws_U.in.data.H_radio_ft, AutopilotLaws_P.LeadLagFilter_C1,
    ...
    AutopilotLaws_U.in.time.dt, &rtb_Y_f, &AutopilotLaws_DWork.sf_LeadLagFilter_k);
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain1_Gain_oa * rtb_Y_f;
  rtb_Add3_e5 = std::fmax(Phi2, AutopilotLaws_DWork.DelayInput1_DSTATE);
  ...
  AutopilotLaws_LeadLagFilter(rtb_Vz, AutopilotLaws_P.LeadLagFilter_C1_a, AutopilotLaws_P.LeadLagFilter_C2_p,
    AutopilotLaws_P.LeadLagFilter_C3_m, AutopilotLaws_P.LeadLagFilter_C4_k, AutopilotLaws_U.in.time.dt, &b_L,
  ...
  if (AutopilotLaws_DWork.DelayInput1_DSTATE > AutopilotLaws_P.Saturation_UpperSat_i0) {
    rtb_Divide = AutopilotLaws_P.Saturation_UpperSat_i0;
  } else if (AutopilotLaws_DWork.DelayInput1_DSTATE < AutopilotLaws_P.Saturation_LowerSat_nd) {
    rtb_Divide = AutopilotLaws_P.Saturation_LowerSat_nd;
  } else {
    rtb_Divide = AutopilotLaws_DWork.DelayInput1_DSTATE;
  }

  R = AutopilotLaws_P.ftmintoms_Gain_k * b_L / rtb_Divide;
  ...
  rtb_Cos1_fn = (rtb_Vz - rtb_Add3_e5) * AutopilotLaws_P.ftmintoms_Gain_j / AutopilotLaws_DWork.DelayInput1_DSTATE;
  if (R > 1.0) {
    R = 1.0;
  } else if (R < -1.0) {
    R = -1.0;
  ...
  rtb_Sum1_g = AutopilotLaws_P.Gain_Gain_gr * std::asin(R) * AutopilotLaws_P.Gain1_Gain_ml +
    AutopilotLaws_P.Gain_Gain_by * std::asin(rtb_Cos1_fn) * AutopilotLaws_P.Gain2_Gain_mj;
  ...
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_bh * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_pd) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_pd;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_l) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_l;
  }

  AutopilotLaws_DWork.DelayInput1_DSTATE = std::atan(AutopilotLaws_DWork.DelayInput1_DSTATE / rtb_Cos1_fn) *
    AutopilotLaws_P.Gain_Gain_cr;
  R = AutopilotLaws_P.Gain1_Gain_ga * AutopilotLaws_DWork.DelayInput1_DSTATE;
  b_L = AutopilotLaws_P.Gain1_Gain_hm2 * rtb_GainTheta1;
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_py * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_ec) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_ec;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_m) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_m;
  }

  rtb_GainTheta1 = (AutopilotLaws_P.Gain1_Gain_ol * rtb_GainTheta - std::atan(AutopilotLaws_P.fpmtoms_Gain_k *
    AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Cos1_fn) * AutopilotLaws_P.Gain_Gain_hc * AutopilotLaws_P.Gain1_Gain_ln) *
    (AutopilotLaws_P.Constant_Value_h - std::cos(b_L));
  rtb_Cos1_fn = std::sin(b_L);
  rtb_Divide = AutopilotLaws_P.Gain1_Gain_it * AutopilotLaws_U.in.data.Psi_magnetic_track_deg;
  rtb_Add3_b = rtb_Divide - AutopilotLaws_P.Gain1_Gain_a * AutopilotLaws_U.in.data.Psi_magnetic_deg;
  b_L = AutopilotLaws_P.ktstomps_Gain_k5 * AutopilotLaws_U.in.data.V_gnd_kn;
  AutopilotLaws_WashoutFilter(AutopilotLaws_P._Gain_f * (AutopilotLaws_P.GStoGS_CAS_Gain_j * b_L),
    AutopilotLaws_P.WashoutFilter_C1_cn, AutopilotLaws_U.in.time.dt, &b_L, &AutopilotLaws_DWork.sf_WashoutFilter_i);
  AutopilotLaws_LeadLagFilter(b_L - AutopilotLaws_P.g_Gain_p * (AutopilotLaws_P.Gain1_Gain_mxw *
    (AutopilotLaws_P.Gain_Gain_er * (rtb_GainTheta1 + rtb_Cos1_fn * std::sin(rtb_Add3_b)))),
  ...
  AutopilotLaws_LeadLagFilter(AutopilotLaws_P.ktstomps_Gain_mf * AutopilotLaws_U.in.data.V_ias_kn,
    ...
    AutopilotLaws_P.LowPassFilter_C4_a, AutopilotLaws_U.in.time.dt, &b_L, &AutopilotLaws_DWork.sf_LeadLagFilter_n);
  b_L = (rtb_Divide + b_L) * AutopilotLaws_P.ug_Gain_c;
  rtb_GainTheta1 = (AutopilotLaws_P.Gain1_Gain_nc * b_L + R) * AutopilotLaws_P.Gain_Gain_bg;
  ...
  rtb_Compare_l = ((rtb_Sum3_m3 > AutopilotLaws_P.CompareToConstant6_const_d) && (rtb_GainTheta1 <
    AutopilotLaws_P.CompareToConstant5_const_h) && (rtb_Divide < AutopilotLaws_P.CompareToConstant2_const_j) && (rtb_dme
    == AutopilotLaws_P.CompareToConstant8_const));
  R += b_L;
  if (rtb_Compare_l) {
    b_L = AutopilotLaws_P.Constant_Value_o;
  } else {
    if (rtb_Sum3_m3 > AutopilotLaws_P.CompareToConstant_const_h) {
      b_L = AutopilotLaws_P.Constant1_Value_g5;
    } else {
      b_L = AutopilotLaws_P.Gain5_Gain_n * rtb_GainTheta1;
    }

    if (rtb_Divide <= b_L) {
      if (rtb_Sum3_m3 > AutopilotLaws_P.CompareToConstant4_const_e) {
        b_L = std::fmax(AutopilotLaws_P.Constant2_Value_m, AutopilotLaws_P.Gain1_Gain_m * rtb_GainTheta1);
      } else {
        b_L = AutopilotLaws_P.Gain6_Gain_fa * rtb_GainTheta1;
      }
  ...
  rtb_GainTheta1 = (AutopilotLaws_P.Gain_Gain_c2 * R - AutopilotLaws_DWork.DelayInput1_DSTATE) + b_L;
  rtb_Cos1_fn = AutopilotLaws_P.kntoms_Gain_om * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Sum3_m3 < 0.0) {
    R = -1.0;
  ...
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_ed) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_ed;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_ee) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_ee;
  }

  rtb_Cos1_fn = (R * AutopilotLaws_P.Constant3_Value_ew - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_m / rtb_Cos1_fn;
  if (rtb_Cos1_fn > 1.0) {
    rtb_Cos1_fn = 1.0;
  } else if (rtb_Cos1_fn < -1.0) {
    rtb_Cos1_fn = -1.0;
  ...
  rtb_Gain_pp = AutopilotLaws_P.Gain_Gain_kon * std::asin(rtb_Cos1_fn);
  b_L = AutopilotLaws_P.kntoms_Gain_iw * AutopilotLaws_U.in.data.V_tas_kn;
  if (b_L > AutopilotLaws_P.Saturation_UpperSat_jt) {
    b_L = AutopilotLaws_P.Saturation_UpperSat_jt;
  } else if (b_L < AutopilotLaws_P.Saturation_LowerSat_ih) {
    b_L = AutopilotLaws_P.Saturation_LowerSat_ih;
  }

  rtb_Cos1_fn = (AutopilotLaws_P.Constant_Value_iaf - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_lv / b_L;
  if (rtb_Cos1_fn > 1.0) {
    rtb_Cos1_fn = 1.0;
  } else if (rtb_Cos1_fn < -1.0) {
    rtb_Cos1_fn = -1.0;
  ...
  rtb_Divide = AutopilotLaws_P.Gain_Gain_o1 * std::asin(rtb_Cos1_fn);
  if (rtb_Compare_l) {
    b_L = rtb_GainTheta1;
  } else if (rtb_Sum3_m3 > AutopilotLaws_P.Switch_Threshold_k) {
    b_L = std::fmax(rtb_GainTheta1, rtb_Gain_pp);
  } else {
    b_L = std::fmin(rtb_GainTheta1, rtb_Gain_pp);
  }

  AutopilotLaws_Voter1(rtb_Sum_es, b_L, rtb_Divide, &R);
  AutopilotLaws_LagFilter(AutopilotLaws_U.in.input.H_c_ft - AutopilotLaws_U.in.data.H_ft, AutopilotLaws_P.LagFilter_C1_b,
    AutopilotLaws_U.in.time.dt, &b_L, &AutopilotLaws_DWork.sf_LagFilter_d);
  rtb_Cos1_fn = AutopilotLaws_P.Gain2_Gain_hq * b_L;
  b_L = AutopilotLaws_P.kntoms_Gain_j * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_f3) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_f3;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_b) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_b;
  }

  if (b_L > AutopilotLaws_P.Saturation_UpperSat_nuy) {
    b_L = AutopilotLaws_P.Saturation_UpperSat_nuy;
  } else if (b_L < AutopilotLaws_P.Saturation_LowerSat_dj) {
    b_L = AutopilotLaws_P.Saturation_LowerSat_dj;
  }

  rtb_Cos1_fn = ((AutopilotLaws_U.in.input.H_dot_c_fpm + rtb_Cos1_fn) - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_kr / b_L;
  if (rtb_Cos1_fn > 1.0) {
    rtb_Cos1_fn = 1.0;
  } else if (rtb_Cos1_fn < -1.0) {
    rtb_Cos1_fn = -1.0;
  ...
  rtb_Add3_b = AutopilotLaws_P.Gain_Gain_fs * std::asin(rtb_Cos1_fn);
  switch (static_cast<int32_T>(rtb_dme)) {
   case 0:
    rtb_Sum_gq = AutopilotLaws_P.Constant_Value_dh;
    break;
   ...
   case 3:
    if (rtb_Compare_jy) {
      ...
      rtb_Sum_gq = std::fmax(rtb_lo_b, rtb_Sum_i);
    } else {
      rtb_Sum_gq = std::fmin(rtb_lo_b, rtb_Sum_i);
    }
    break;

   case 4:
    rtb_Sum_gq = rtb_Cos1_pk;
    break;
   ...
   case 6:
    rtb_Sum_gq = AutopilotLaws_P.Gain1_Gain_d * rtb_Product_dh;
    break;

   case 7:
    if (rtb_on_ground > AutopilotLaws_P.Switch1_Threshold_j) {
      rtb_Sum_gq = AutopilotLaws_P.Gain2_Gain_h * rtb_Gain4;
    } else {
      rtb_Sum_gq = ((AutopilotLaws_P.Gain1_Gain_i * rtb_Y_pf + rtb_Add3_hx) + rtb_Sum1_g * rtb_uDLookupTable_m) *
   ...
   default:
    rtb_Sum_gq = rtb_Add3_b;
    break;
  }

  if (rtb_Sum_gq > AutopilotLaws_P.Constant1_Value_i) {
    b_L = AutopilotLaws_P.Constant1_Value_i;
  } else {
    b_L = AutopilotLaws_P.Gain1_Gain_n * AutopilotLaws_P.Constant1_Value_i;
    if (rtb_Sum_gq >= b_L) {
      b_L = rtb_Sum_gq;
    }
  }

  AutopilotLaws_RateLimiter(b_L - rtb_error_d, AutopilotLaws_P.RateLimiterVariableTs1_up,
    AutopilotLaws_P.RateLimiterVariableTs1_lo, AutopilotLaws_U.in.time.dt,
    AutopilotLaws_P.RateLimiterVariableTs1_InitialCondition, &b_L, &AutopilotLaws_DWork.sf_RateLimiter_h);
  AutopilotLaws_LagFilter(b_L, AutopilotLaws_P.LagFilter_C1_gh, AutopilotLaws_U.in.time.dt, &R,
  ...
  AutopilotLaws_VSLimiter(AutopilotLaws_P.VS_Gain_n * a, &AutopilotLaws_Y.out, &b_L);
  if (!rtb_Compare_jy) {
  ...
  AutopilotLaws_WashoutFilter(rtb_Saturation, AutopilotLaws_P.WashoutFilter1_C1_h, AutopilotLaws_U.in.time.dt,
    &rtb_error_d, &AutopilotLaws_DWork.sf_WashoutFilter_k);
  rtb_Cos1_fn = std::abs(rtb_error_d);
  if (rtb_Cos1_fn > AutopilotLaws_P.Saturation_UpperSat_ig) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_UpperSat_ig;
  } else if (rtb_Cos1_fn < AutopilotLaws_P.Saturation_LowerSat_ous) {
    rtb_Cos1_fn = AutopilotLaws_P.Saturation_LowerSat_ous;
  }

  rtb_Saturation = AutopilotLaws_P.Gain_Gain_j0 * rtb_Cos1_fn;
  if (rtb_on_ground <= AutopilotLaws_P.Switch_Threshold_c) {
    rtb_Gain4 = ((rtb_Add3_hx + a) + rtb_uDLookupTable_m * rtb_Sum_gq) + rtb_Saturation;
  }

  AutopilotLaws_VSLimiter_f(rtb_Gain4, &AutopilotLaws_Y.out, &rtb_error_d);
  if (!rtb_Compare_l) {
    if (rtb_Sum3_m3 > AutopilotLaws_P.Switch_Threshold_hz) {
      rtb_GainTheta1 = std::fmax(rtb_GainTheta1, AutopilotLaws_P.VS_Gain_n5 * rtb_Gain_pp);
    } else {
      rtb_GainTheta1 = std::fmin(rtb_GainTheta1, AutopilotLaws_P.VS_Gain_n5 * rtb_Gain_pp);
    }
  }

  AutopilotLaws_Voter1(rtb_Sum_es, AutopilotLaws_P.Gain_Gain_o2 * rtb_GainTheta1, AutopilotLaws_P.VS_Gain_nx *
                       rtb_Divide, &rtb_Sum_i);
  ...
  switch (static_cast<int32_T>(rtb_dme)) {
   ...
   default:
    b_L = std::fmax(-rtb_Y_pf, std::fmin(rtb_Y_pf, AutopilotLaws_P.VS_Gain_ne * rtb_Add3_b));
    break;
  }
  ...
  b_L = std::fmin(b_L - AutopilotLaws_DWork.Delay_DSTATE_h2, AutopilotLaws_DWork.DelayInput1_DSTATE);
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain1_Gain_i0 * rtb_GainTheta1;
  AutopilotLaws_DWork.DelayInput1_DSTATE *= AutopilotLaws_U.in.time.dt;
  AutopilotLaws_DWork.Delay_DSTATE_h2 += std::fmax(b_L, AutopilotLaws_DWork.DelayInput1_DSTATE);
  AutopilotLaws_LagFilter(AutopilotLaws_DWork.Delay_DSTATE_h2, AutopilotLaws_P.LagFilter_C1_i,
    AutopilotLaws_U.in.time.dt, &rtb_error_d, &AutopilotLaws_DWork.sf_LagFilter_gn);
  ...
  if (b_L > AutopilotLaws_P.Saturation_UpperSat_ix) {
    b_L = AutopilotLaws_P.Saturation_UpperSat_ix;
  } else if (b_L < AutopilotLaws_P.Saturation_LowerSat_eq) {
    b_L = AutopilotLaws_P.Saturation_LowerSat_eq;
  }

  rtb_GainTheta1 = rtb_error_d * b_L;
  AutopilotLaws_LagFilter(rtb_Y_f, AutopilotLaws_P.LagFilter1_C1_d, AutopilotLaws_U.in.time.dt, &rtb_error_d,
    &AutopilotLaws_DWork.sf_LagFilter_cs);
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain7_Gain_l * rtb_error_d;
  AutopilotLaws_Y.out.output.flight_director.Theta_c_deg = R;
  AutopilotLaws_Y.out.output.autopilot.Theta_c_deg = (AutopilotLaws_P.Constant_Value_i4 - b_L) * rtb_GainTheta +
    rtb_GainTheta1;
  ...
  AutopilotLaws_Y.out.output.flare_law.H_dot_radio_fpm = rtb_Add3_e5;
  ...
  AutopilotLaws_Y.out.output.flare_law.delta_Theta_bz_deg = rtb_Add3_hx;
  ...
  AutopilotLaws_DWork.Delay_DSTATE_m = rtb_Gain_dn;
