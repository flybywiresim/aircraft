...
void AutopilotLawsModelClass::step()
{
  ...
  rtb_ManualSwitch = std::sin(rtb_Saturation1);
  rtb_Mod2_k = std::cos(rtb_Saturation1);
  result_tmp[0] = 1.0;
  result_tmp[3] = rtb_ManualSwitch * rtb_out_c;
  result_tmp[6] = rtb_Mod2_k * rtb_out_c;
  result_tmp[1] = 0.0;
  result_tmp[4] = rtb_Mod2_k;
  result_tmp[7] = -rtb_ManualSwitch;
  result_tmp[2] = 0.0;
  rtb_Y_m = 1.0 / std::cos(rtb_Saturation);
  result_tmp[5] = rtb_Y_m * rtb_ManualSwitch;
  result_tmp[8] = rtb_Y_m * rtb_Mod2_k;
  ...
  result_tmp[1] = rtb_Mod2_k * rtb_ManualSwitch;
  result_tmp[4] = rtb_Saturation;
  result_tmp[7] = rtb_out_c * rtb_Mod2_k;
  result_tmp[2] = rtb_Saturation * rtb_ManualSwitch;
  ...
  if (AutopilotLaws_U.in.data.nav_dme_nmi > AutopilotLaws_P.Saturation_UpperSat_c) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_c;
  } else if (AutopilotLaws_U.in.data.nav_dme_nmi < AutopilotLaws_P.Saturation_LowerSat_d) {
    rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_d;
  } else {
    rtb_Y_m = AutopilotLaws_U.in.data.nav_dme_nmi;
  }

  rtb_Saturation1 = std::sin(AutopilotLaws_P.Gain1_Gain_g * AutopilotLaws_U.in.data.nav_loc_error_deg) * rtb_Y_m;
  ...
  if (AutopilotLaws_U.in.data.nav_dme_nmi > AutopilotLaws_P.Saturation_UpperSat_o) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_o;
  } else if (AutopilotLaws_U.in.data.nav_dme_nmi < AutopilotLaws_P.Saturation_LowerSat_o) {
    rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_o;
  } else {
    rtb_Y_m = AutopilotLaws_U.in.data.nav_dme_nmi;
  }

  rtb_Saturation1 = std::sin(AutopilotLaws_P.Gain1_Gain_h5 * AutopilotLaws_U.in.data.nav_loc_error_deg) * rtb_Y_m *
  ...
  if (rtb_Saturation1 > AutopilotLaws_P.Saturation1_UpperSat_g) {
    ...
    if (rtb_Saturation1 < AutopilotLaws_P.Saturation1_LowerSat_k) {
  ...
  if (rtb_Saturation < rtb_Mod2_f) {
    rtb_Saturation *= AutopilotLaws_P.Gain1_Gain_ni;
  } else {
  ...
  switch (static_cast<int32_T>(rtb_ManualSwitch)) {
   ...
   case 3:
    rtb_Y_m = AutopilotLaws_P.Gain_Gain_n * AutopilotLaws_U.in.data.flight_guidance_xtk_nmi;
    if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat) {
      rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat;
    } else {
      if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat) {
        rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat;
      }
    }

    rtb_Saturation1 = (AutopilotLaws_P.Gain2_Gain * AutopilotLaws_U.in.data.flight_guidance_tae_deg + rtb_Y_m) *
  ...
  rtb_Y_m = rtb_Saturation1 - AutopilotLaws_DWork.Delay_DSTATE_h;
  rtb_Divide_h = AutopilotLaws_P.Constant2_Value_h * AutopilotLaws_U.in.time.dt;
  if (rtb_Y_m < rtb_Divide_h) {
    rtb_Divide_h = rtb_Y_m;
  }

  rtb_Y_p = AutopilotLaws_P.Gain1_Gain_k * AutopilotLaws_P.Constant2_Value_h * AutopilotLaws_U.in.time.dt;
  if (rtb_Divide_h > rtb_Y_p) {
    rtb_Y_p = rtb_Divide_h;
  }
  ...
  if (rtb_Y_p > AutopilotLaws_P.Saturation_UpperSat_j) {
    rtb_Gain_ny = AutopilotLaws_P.Saturation_UpperSat_j;
  } else if (rtb_Y_p < AutopilotLaws_P.Saturation_LowerSat_p) {
    rtb_Gain_ny = AutopilotLaws_P.Saturation_LowerSat_p;
  } else {
    rtb_Gain_ny = rtb_Y_p;
  }

  rtb_Divide_h *= rtb_Gain_ny;
  rtb_Gain_ny = AutopilotLaws_P.Constant_Value_n - rtb_Gain_ny;
  ...
  if (AutopilotLaws_U.in.data.H_radio_ft <= AutopilotLaws_P.CompareToConstant_const_d) {
    rtb_Y_m = AutopilotLaws_P.Gain_Gain_a * rtb_out_c;
  } else {
    rtb_Y_m = AutopilotLaws_P.Constant1_Value;
  }

  AutopilotLaws_LagFilter(rtb_Y_m, AutopilotLaws_P.LagFilter1_C1, AutopilotLaws_U.in.time.dt, &rtb_Y_p,
  ...
  rtb_Saturation1 += AutopilotLaws_P.Gain_Gain_f * rtb_ManualSwitch;
  if (rtb_Saturation1 > AutopilotLaws_P.Saturation_UpperSat_n) {
    rtb_Saturation1 = AutopilotLaws_P.Saturation_UpperSat_n;
  } else {
    if (rtb_Saturation1 < AutopilotLaws_P.Saturation_LowerSat_d4) {
      rtb_Saturation1 = AutopilotLaws_P.Saturation_LowerSat_d4;
    }
  }

  rtb_Saturation1 -= AutopilotLaws_U.in.data.H_dot_ft_min;
  rtb_Y_m = AutopilotLaws_P.kntoms_Gain * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_n3) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_n3;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_m) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_m;
    }
  }

  rtb_Y_m = AutopilotLaws_P.ftmintoms_Gain * rtb_Saturation1 / rtb_Y_m;
  if (rtb_Y_m > 1.0) {
    rtb_Y_m = 1.0;
  } else {
    if (rtb_Y_m < -1.0) {
  ...
  rtb_ManualSwitch = AutopilotLaws_P.Gain_Gain_kr * std::asin(rtb_Y_m);
  rtb_Y_m = AutopilotLaws_P.kntoms_Gain_a * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_d) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_d;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_b) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_b;
    }
  }

  rtb_Y_m = (AutopilotLaws_U.in.input.H_dot_c_fpm - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_h / rtb_Y_m;
  if (rtb_Y_m > 1.0) {
    rtb_Y_m = 1.0;
  } else {
    if (rtb_Y_m < -1.0) {
  ...
  rtb_out_c = AutopilotLaws_P.Gain_Gain_df * std::asin(rtb_Y_m);
  ...
  rtb_Mod2_k = (rtb_Gain_ny + rtb_Saturation1) * AutopilotLaws_P.Gain_Gain_pb;
  ...
  if (rtb_Saturation1 > AutopilotLaws_P.Saturation_UpperSat_e) {
    rtb_Saturation1 = AutopilotLaws_P.Saturation_UpperSat_e;
  } else {
    if (rtb_Saturation1 < AutopilotLaws_P.Saturation_LowerSat_mk) {
      rtb_Saturation1 = AutopilotLaws_P.Saturation_LowerSat_mk;
    }
  }

  rtb_Mod2_f = AutopilotLaws_P.Gain_Gain_o * rtb_Mod2_k + rtb_Saturation1;
  rtb_Saturation1 = AutopilotLaws_P.Gain1_Gain_nv * AutopilotLaws_U.in.data.alpha_deg;
  rtb_Gain_ny = result_0[2] * std::sin(rtb_Saturation1);
  rtb_Saturation1 = std::cos(rtb_Saturation1);
  rtb_Saturation1 *= result_0[0];
  rtb_Sum2_j = AutopilotLaws_U.in.data.V_ias_kn - AutopilotLaws_U.in.data.VMAX_kn;
  rtb_Y_m = rtb_Sum2_j * AutopilotLaws_P.Gain1_Gain_ji;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_jm) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_jm;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_on) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_on;
    }
  }

  rtb_Saturation1 = (rtb_Gain_ny + rtb_Saturation1) * AutopilotLaws_P.Gain_Gain_e1 * AutopilotLaws_P.Gain_Gain_eq +
    rtb_Y_m;
  AutopilotLaws_SpeedProtectionMode(&AutopilotLaws_Y.out, rtb_out_c, AutopilotLaws_P.VS_Gain_h * rtb_out_c, rtb_Mod2_f,
    AutopilotLaws_P.Gain_Gain_of * rtb_Mod2_f, rtb_Saturation1, AutopilotLaws_P.Gain_Gain_mw * rtb_Saturation1,
    &rtb_Mod2_k, &rtb_Divide_h);
  rtb_Y_m = AutopilotLaws_P.kntoms_Gain_p * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_k) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_k;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_l) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_l;
    }
  }

  rtb_Mod2_f = AutopilotLaws_U.in.input.FPA_c_deg - std::atan(AutopilotLaws_P.fpmtoms_Gain *
    AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Y_m) * AutopilotLaws_P.Gain_Gain_l;
  rtb_Saturation1 = AutopilotLaws_P.Gain1_Gain_b2 * AutopilotLaws_U.in.data.alpha_deg;
  rtb_Gain_ny = result_0[2] * std::sin(rtb_Saturation1);
  rtb_Saturation1 = std::cos(rtb_Saturation1);
  rtb_Saturation1 *= result_0[0];
  AutopilotLaws_V_LSSpeedSelection(&AutopilotLaws_Y.out, &rtb_Y_o);
  rtb_Y_m = (AutopilotLaws_U.in.data.V_ias_kn - rtb_Y_o) * AutopilotLaws_P.Gain1_Gain_fq;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_h) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_h;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_a) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_a;
    }
  }

  rtb_Y_p = (rtb_Gain_ny + rtb_Saturation1) * AutopilotLaws_P.Gain_Gain_p2 * AutopilotLaws_P.Gain_Gain_n1 + rtb_Y_m;
  rtb_Saturation1 = AutopilotLaws_P.Gain1_Gain_ib * AutopilotLaws_U.in.data.alpha_deg;
  rtb_Gain_ny = result_0[2] * std::sin(rtb_Saturation1);
  rtb_Saturation1 = std::cos(rtb_Saturation1);
  rtb_Saturation1 *= result_0[0];
  rtb_Y_m = rtb_Sum2_j * AutopilotLaws_P.Gain1_Gain_gs;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_l) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_l;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_i) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_i;
    }
  }

  rtb_Saturation1 = (rtb_Gain_ny + rtb_Saturation1) * AutopilotLaws_P.Gain_Gain_he * AutopilotLaws_P.Gain_Gain_p4 +
    rtb_Y_m;
  AutopilotLaws_SpeedProtectionMode(&AutopilotLaws_Y.out, rtb_Mod2_f, AutopilotLaws_P.Gain_Gain_c * rtb_Mod2_f, rtb_Y_p,
    AutopilotLaws_P.Gain_Gain_i * rtb_Y_p, rtb_Saturation1, AutopilotLaws_P.Gain_Gain_e5 * rtb_Saturation1, &rtb_out_c,
    &rtb_AP_b);
  AutopilotLaws_WashoutFilter(result_0[2], AutopilotLaws_P.WashoutFilter_C1, AutopilotLaws_U.in.time.dt, &rtb_Y_o,
  ...
  rtb_Mod2_f = AutopilotLaws_P.DiscreteDerivativeVariableTs_Gain_l * rtb_Y_p;
  AutopilotLaws_LagFilter(rtb_Y_p + AutopilotLaws_P.Gain3_Gain_o * ((rtb_Mod2_f - AutopilotLaws_DWork.Delay_DSTATE_g) /
  ...
  rtb_Y_m = AutopilotLaws_P.kntoms_Gain_i * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_g) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_g;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_c) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_c;
    }
  }

  rtb_Gain_ny = std::atan(AutopilotLaws_P.fpmtoms_Gain_e * AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Y_m) *
  ...
  if ((AutopilotLaws_U.in.data.H_radio_ft > AutopilotLaws_P.CompareToConstant_const_n) &&
    ...
    rtb_Y_m = AutopilotLaws_P.Gain_Gain_h * rtb_Y_o + rtb_Saturation1 * look1_binlxpw(AutopilotLaws_U.in.data.H_radio_ft,
      AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1_h, AutopilotLaws_P.ScheduledGain_Table_ir, 5U);
  } else {
    rtb_Y_m = (rtb_Y_p - rtb_Gain_ny) * AutopilotLaws_P.Gain2_Gain_j;
  }

  AutopilotLaws_Voter1(rtb_Y_m, AutopilotLaws_P.Gain1_Gain_nq * ((rtb_Y_p + AutopilotLaws_P.Bias_Bias) - rtb_Gain_ny),
                       AutopilotLaws_P.Gain_Gain_p2b * ((rtb_Y_p + AutopilotLaws_P.Bias1_Bias) - rtb_Gain_ny), &rtb_Y_o);
  rtb_Sum_ib_tmp = AutopilotLaws_U.in.input.H_c_ft - AutopilotLaws_U.in.data.H_ind_ft;
  rtb_Y_m = AutopilotLaws_P.kntoms_Gain_e * AutopilotLaws_U.in.data.V_tas_kn;
  ...
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_dn) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_dn;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_bx) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_bx;
    }
  }

  rtb_Y_m = ((AutopilotLaws_P.Constant_Value_b * rtb_Saturation1 + rtb_Sum_ib_tmp) * AutopilotLaws_P.Gain_Gain_el -
             AutopilotLaws_U.in.data.H_dot_ft_min) * AutopilotLaws_P.ftmintoms_Gain_g / rtb_Y_m;
  if (rtb_Y_m > 1.0) {
    rtb_Y_m = 1.0;
  } else {
    if (rtb_Y_m < -1.0) {
  ...
  rtb_Gain_hl = AutopilotLaws_P.Gain_Gain_da * std::asin(rtb_Y_m);
  ...
  rtb_Y_m = rtb_Sum2_j * AutopilotLaws_P.Gain1_Gain_hn;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_j4) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_j4;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_bb) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_bb;
    }
  }

  rtb_Sum2_d = (rtb_Gain_ny + rtb_Saturation1) * AutopilotLaws_P.Gain_Gain_bi * AutopilotLaws_P.Gain_Gain_ei + rtb_Y_m;
  rtb_Y_m = AutopilotLaws_P.kntoms_Gain_iw * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_kg) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_kg;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_ce) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_ce;
    }
  }

  rtb_Y_m = (AutopilotLaws_P.Constant_Value_k - AutopilotLaws_U.in.data.H_dot_ft_min) * AutopilotLaws_P.ftmintoms_Gain_p
    / rtb_Y_m;
  if (rtb_Y_m > 1.0) {
    rtb_Y_m = 1.0;
  } else {
  ...
  rtb_Gain_gd = AutopilotLaws_P.Gain_Gain_md * std::asin(rtb_Y_m);
  ...
  if (AutopilotLaws_U.in.data.H_radio_ft > AutopilotLaws_P.Saturation_UpperSat_e0) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_e0;
  } else if (AutopilotLaws_U.in.data.H_radio_ft < AutopilotLaws_P.Saturation_LowerSat_mg) {
    rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_mg;
  } else {
    rtb_Y_m = AutopilotLaws_U.in.data.H_radio_ft;
  }

  AutopilotLaws_LagFilter(rtb_Y_m, AutopilotLaws_P.LagFilter_C1_h, AutopilotLaws_U.in.time.dt, &rtb_Gain_ny,
  ...
  AutopilotLaws_LagFilter(AutopilotLaws_P.Gain2_Gain_c * rtb_Saturation1, AutopilotLaws_P.LagFilter3_C1,
    AutopilotLaws_U.in.time.dt, &rtb_Y_m, &AutopilotLaws_DWork.sf_LagFilter_e);
  ...
  if ((!AutopilotLaws_DWork.wasActive) && rtb_OR) {
    rtb_Y_m = std::abs(rtb_Saturation1) / 60.0;
    AutopilotLaws_DWork.Tau = AutopilotLaws_U.in.data.H_radio_ft / (rtb_Y_m - 1.6666666666666667);
    AutopilotLaws_DWork.H_bias = AutopilotLaws_DWork.Tau * rtb_Y_m - AutopilotLaws_U.in.data.H_radio_ft;
  }

  if (rtb_OR) {
    rtb_Gain_ny = -1.0 / AutopilotLaws_DWork.Tau * (AutopilotLaws_U.in.data.H_radio_ft + AutopilotLaws_DWork.H_bias) *
      60.0;
  } else {
    rtb_Gain_ny = rtb_Saturation1;
  }

  AutopilotLaws_DWork.wasActive = rtb_OR;
  rtb_Y_m = AutopilotLaws_P.kntoms_Gain_av * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_m) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_m;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_d1) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_d1;
    }
  }

  rtb_Y_m = (rtb_Gain_ny - rtb_Saturation1) * AutopilotLaws_P.ftmintoms_Gain_i / rtb_Y_m;
  if (rtb_Y_m > 1.0) {
    rtb_Y_m = 1.0;
  } else {
    if (rtb_Y_m < -1.0) {
  ...
  rtb_Gain_ar = AutopilotLaws_P.Gain_Gain_fz * std::asin(rtb_Y_m);
  ...
  rtb_Saturation1 = std::cos(rtb_Saturation1);
  rtb_Saturation1 *= result_0[0];
  rtb_Y_m = rtb_Sum2_j * AutopilotLaws_P.Gain1_Gain_fr;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_e3) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_e3;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_py) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_py;
    }
  }

  rtb_Sum2_j = (rtb_Gain_ny + rtb_Saturation1) * AutopilotLaws_P.Gain_Gain_nub * AutopilotLaws_P.Gain_Gain_ao + rtb_Y_m;
  rtb_Y_m = AutopilotLaws_P.kntoms_Gain_f * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Y_m > AutopilotLaws_P.Saturation_UpperSat_b) {
    rtb_Y_m = AutopilotLaws_P.Saturation_UpperSat_b;
  } else {
    if (rtb_Y_m < AutopilotLaws_P.Saturation_LowerSat_ow) {
      rtb_Y_m = AutopilotLaws_P.Saturation_LowerSat_ow;
    }
  }

  rtb_Y_m = (AutopilotLaws_P.Constant_Value_i - AutopilotLaws_U.in.data.H_dot_ft_min) * AutopilotLaws_P.ftmintoms_Gain_j
    / rtb_Y_m;
  if (rtb_Y_m > 1.0) {
    rtb_Y_m = 1.0;
  } else {
  ...
  rtb_Gain_et = AutopilotLaws_P.Gain_Gain_bd * std::asin(rtb_Y_m);
  ...
  switch (static_cast<int32_T>(rtb_GainTheta1)) {
   ...
   case 2:
    rtb_Saturation1 = AutopilotLaws_P.VS_Gain_a * rtb_Gain_hl;
    break;

   case 3:
    if (rtb_Sum_ib_tmp > AutopilotLaws_P.Switch_Threshold_n) {
      rtb_Y_m = AutopilotLaws_P.Gain_Gain_kg * rtb_Sum2_d;
      rtb_Saturation1 = AutopilotLaws_P.VS_Gain_j * rtb_Gain_gd;
      if (rtb_Y_m > rtb_Saturation1) {
        rtb_Saturation1 = rtb_Y_m;
      }
    } else {
      rtb_Y_m = AutopilotLaws_P.Gain_Gain_kg * rtb_Sum2_d;
  ...
  if (rtb_Saturation1 > AutopilotLaws_P.Constant1_Value_i) {
    ...
    rtb_Divide_h = AutopilotLaws_P.Gain1_Gain_m * AutopilotLaws_P.Constant1_Value_i;
    if (rtb_Saturation1 < rtb_Divide_h) {
      rtb_Saturation1 = rtb_Divide_h;
    }
  }

  if (AutopilotLaws_Y.out.output.ap_on == 0.0) {
  ...
  rtb_Divide_h = AutopilotLaws_P.Constant2_Value_h1 * AutopilotLaws_U.in.time.dt;
  if (rtb_Saturation1 >= rtb_Divide_h) {
    rtb_Saturation1 = rtb_Divide_h;
  }

  rtb_Divide_h = AutopilotLaws_P.Gain1_Gain_i0 * AutopilotLaws_P.Constant2_Value_h1 * AutopilotLaws_U.in.time.dt;
  if (rtb_Saturation1 > rtb_Divide_h) {
    rtb_Divide_h = rtb_Saturation1;
  }

  AutopilotLaws_DWork.Delay_DSTATE_h2 += rtb_Divide_h;
  AutopilotLaws_LagFilter(AutopilotLaws_DWork.Delay_DSTATE_h2, AutopilotLaws_P.LagFilter_C1_i,
    AutopilotLaws_U.in.time.dt, &rtb_Divide_h, &AutopilotLaws_DWork.sf_LagFilter_n);
  ...
  if (rtb_Saturation1 > AutopilotLaws_P.Saturation_UpperSat_ju) {
    ...
    if (rtb_Saturation1 < AutopilotLaws_P.Saturation_LowerSat_n) {
  ...
  rtb_Saturation1 *= rtb_GainTheta;
  AutopilotLaws_Y.out.output.autopilot.Theta_c_deg = rtb_Divide_h + rtb_Saturation1;
  ...
  switch (static_cast<int32_T>(rtb_GainTheta1)) {
   ...
   case 1:
    rtb_Gain_ny = rtb_ManualSwitch;
    break;

   case 2:
    rtb_Gain_ny = rtb_Gain_hl;
    break;
   ...
   case 5:
    rtb_Gain_ny = rtb_out_c;
    break;

   case 6:
    rtb_Gain_ny = AutopilotLaws_P.Gain1_Gain_h * rtb_Y_o;
    break;
  ...
  rtb_GainTheta += rtb_Gain_ny;
  if (rtb_GainTheta > AutopilotLaws_P.Constant1_Value_i) {
    ...
    if (rtb_GainTheta < rtb_Saturation1) {
      AutopilotLaws_Y.out.output.flight_director.Theta_c_deg = rtb_Saturation1;
    } else {
      AutopilotLaws_Y.out.output.flight_director.Theta_c_deg = rtb_GainTheta;
    }
  }
  ...
  AutopilotLaws_DWork.Delay_DSTATE_g = rtb_Mod2_f;
