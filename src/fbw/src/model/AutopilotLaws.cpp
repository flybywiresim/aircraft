...
void AutopilotLawsModelClass::step()
{
  ...
  switch (static_cast<int32_T>(rtb_Saturation)) {
   ...
   case 3:
    rtb_Add1_a = AutopilotLaws_P.Gain_Gain_c * AutopilotLaws_U.in.data.flight_guidance_xtk_nmi * b_L /
    ...
    b_R = rtb_Y_o - (AutopilotLaws_P.Gain2_Gain * AutopilotLaws_U.in.data.flight_guidance_tae_deg + rtb_Add1_a) * a *
      AutopilotLaws_U.in.data.V_gnd_kn;
    break;
  ...
  rtb_Gain_nrh = AutopilotLaws_U.in.input.H_c_ft - AutopilotLaws_U.in.data.H_ind_ft;
  if (rtb_Gain_nrh < 0.0) {
    b_R = -1.0;
  } else if (rtb_Gain_nrh > 0.0) {
    b_R = 1.0;
  } else {
    b_R = rtb_Gain_nrh;
  }

  b_R = b_R * AutopilotLaws_DWork.dH_offset + rtb_Gain_nrh;
  ...
  if (rtb_Gain_nrh > AutopilotLaws_P.Switch_Threshold_l) {
  ...
  b_L = (AutopilotLaws_U.in.data.V_ias_kn - rtb_Y_j5) * AutopilotLaws_P.Gain1_Gain_oz;
  if (b_L <= b_R) {
    if (rtb_Gain_nrh > AutopilotLaws_P.Switch1_Threshold) {
  ...
  b_L = (AutopilotLaws_P.Gain_Gain_b0 * rtb_out_f - Phi2) + b_R;
  ...
  if (rtb_Gain_nrh > AutopilotLaws_P.Switch_Threshold_b) {
  ...
  if (rtb_Tsxlo <= b_R) {
    if (rtb_Gain_nrh > AutopilotLaws_P.Switch1_Threshold_f) {
  ...
  AutopilotLaws_SpeedProtectionSignalSelection(&AutopilotLaws_Y.out, R, AutopilotLaws_P.VS_Gain * R, b_L,
    AutopilotLaws_P.Gain_Gain_m0 * b_L, b_R, AutopilotLaws_P.Gain_Gain_lr * b_R, look1_binlxpw
  ...
  R = b_R + rtb_Tsxlo;
  b_L = (AutopilotLaws_P.Gain1_Gain_ov * b_R + rtb_Tsxlo) * AutopilotLaws_P.Gain_Gain_a2;
  ...
  if ((rtb_Gain_nrh > AutopilotLaws_P.CompareToConstant_const_a) && (b_L < AutopilotLaws_P.CompareToConstant1_const_n) &&
      (b_R < AutopilotLaws_P.CompareToConstant2_const_b)) {
    b_R = AutopilotLaws_P.Constant_Value_c;
  } else {
    if (rtb_Gain_nrh > AutopilotLaws_P.Switch2_Threshold) {
      rtb_Tsxlo = AutopilotLaws_P.Constant1_Value_mf;
    } else {
      rtb_Tsxlo = AutopilotLaws_P.Gain5_Gain_f * b_L;
    }

    if (b_R > rtb_Tsxlo) {
      b_R = rtb_Tsxlo;
    } else {
      if (rtb_Gain_nrh > AutopilotLaws_P.Switch1_Threshold_o) {
  ...
  AutopilotLaws_LeadLagFilter(b_R - AutopilotLaws_P.g_Gain_m * (AutopilotLaws_P.Gain1_Gain_kdq *
    ...
    rtb_Add1_a))) * (AutopilotLaws_P.Constant_Value_od - rtb_Cos_f2) + rtb_Cos1_pk * std::sin(rtb_Add3_gy)))),
  ...
  if (rtb_Gain_nrh > AutopilotLaws_P.Switch_Threshold_o) {
  ...
  if (rtb_Tsxlo <= b_R) {
    if (rtb_Gain_nrh > AutopilotLaws_P.Switch1_Threshold_c) {
  ...
  rtb_Add3_gy = (AutopilotLaws_P.Gain_Gain_j * rtb_Cos_f2 - Phi2) + b_R;
  ...
  AutopilotLaws_LeadLagFilter(b_R - AutopilotLaws_P.g_Gain_gr * (AutopilotLaws_P.Gain1_Gain_hv *
    ...
    rtb_Add1_a))) * (AutopilotLaws_P.Constant_Value_ia - rtb_Cos_f2) + rtb_Cos1_pk * std::sin(rtb_Add3_n2)))),
  ...
  if (rtb_Gain_nrh > AutopilotLaws_P.Switch_Threshold_a) {
  ...
  if (rtb_Cos1_pk <= b_R) {
    if (rtb_Gain_nrh > AutopilotLaws_P.Switch1_Threshold_b) {
  ...
  b_R += AutopilotLaws_P.Gain_Gain_dm * rtb_Cos_f2 - Phi2;
  ...
  if (rtb_Gain_nrh > AutopilotLaws_P.Switch_Threshold_d) {
  ...
  if (b_L <= b_R) {
    if (rtb_Gain_nrh > AutopilotLaws_P.Switch1_Threshold_d) {
  ...
  rtb_Sum2_p = (AutopilotLaws_P.Gain_Gain_d0 * rtb_Add1_a - Phi2) + b_R;
  ...
  if (rtb_Gain_nrh > AutopilotLaws_P.Switch_Threshold_g) {
  ...
  if (rtb_Y_j5 <= b_R) {
    if (rtb_Gain_nrh > AutopilotLaws_P.Switch1_Threshold_h) {
  ...
  AutopilotLaws_SpeedProtectionSignalSelection(&AutopilotLaws_Y.out, rtb_Add3_gy, rtb_Y_o, rtb_Sum2_p,
    AutopilotLaws_P.Gain_Gain_fnw * rtb_Sum2_p, b_R, AutopilotLaws_P.Gain_Gain_ko * b_R, look1_binlxpw
  ...
  Phi2 = rtb_Gain_nrh - AutopilotLaws_DWork.Delay_DSTATE_hi;
  ...
  AutopilotLaws_Voter1(rtb_Tsxlo + b_L, AutopilotLaws_P.Gain1_Gain_d4 * ((Phi2 + AutopilotLaws_P.Bias_Bias) - b_R),
                       AutopilotLaws_P.Gain_Gain_eyl * ((Phi2 + AutopilotLaws_P.Bias1_Bias) - b_R), &rtb_Sum2_p);
  ...
  rtb_Gain_kjv = AutopilotLaws_P.Gain_Gain_by * std::asin(rtb_Add1_a);
  ...
  b_L = AutopilotLaws_P.Constant2_Value_k - AutopilotLaws_U.in.data.H_ind_ft;
  ...
  if ((b_L > AutopilotLaws_P.CompareToConstant_const_h) && (rtb_Tsxlo < AutopilotLaws_P.CompareToConstant1_const_g) &&
    ...
    if (b_L > AutopilotLaws_P.Switch2_Threshold_b) {
      rtb_Add1_a = AutopilotLaws_P.Constant1_Value_mq;
    } else {
      rtb_Add1_a = AutopilotLaws_P.Gain5_Gain_k * rtb_Tsxlo;
    }
  ...
  b_L = AutopilotLaws_P.Gain_Gain_o1 * std::asin(rtb_Add1_a);
  ...
  switch (static_cast<int32_T>(rtb_Saturation)) {
   ...
   case 6:
    Phi2 = AutopilotLaws_P.Gain1_Gain_d * rtb_Sum2_p;
    break;
   ...
   case 7:
    if (rtb_on_ground > AutopilotLaws_P.Switch1_Threshold_j) {
      ...
      Phi2 = (AutopilotLaws_P.Gain1_Gain_ix * rtb_Y_j5 + rtb_Gain5) + rtb_Gain_kjv;
    }
    break;
  }
  ...
  switch (static_cast<int32_T>(rtb_Saturation)) {
   ...
   case 7:
    if (rtb_on_ground > AutopilotLaws_P.Switch_Threshold) {
      ...
      b_R = (AutopilotLaws_P.Gain3_Gain * rtb_Y_j5 + rtb_Gain5) + AutopilotLaws_P.VS_Gain_e * rtb_Gain_kjv;
    }
    break;
  }
