...
test('when empty, writes a value without update', async () => {
    const w = writer();
    w.write(1);
...
test('writes the next value after consumption of the first value', async () => {
    const w = writer();
    w.write(1);
    w.write(2);
...
test('resolves as soon as the write and consumption are observed', async () => {
    const w = writer();
