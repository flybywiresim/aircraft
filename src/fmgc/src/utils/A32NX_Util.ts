...
/* global Avionics, LatLong, NotificationButton */

import * as math from 'mathjs';

export const A32NX_Util = {
    createDeltaTimeCalculator: (startTime = Date.now()) => {
     ...
     */
    magneticToTrue: (heading, magVar) => (360 + heading + (magVar || SimVar.GetSimVarValue('MAGVAR', 'degree'))) % 360,

    /**
     * Takes a LatLongAlt or LatLong and returns a vector of spherical co-ordinates
     * @param {(LatLong | LatLongAlt)} ll
     */
    latLonToSpherical: (ll) => [
     ...
     * @returns {LatLong}
     */
    sphericalToLatLon: (s) => new LatLong(Math.asin(s[2]) * Avionics.Utils.RAD2DEG, Math.atan2(s[1], s[0]) * Avionics.Utils.RAD2DEG),
     ...
     * @returns {LatLong}
     */
    greatCircleIntersection: (latlon1, brg1, latlon2, brg2) => {
     ...
     */
    getIsaTemp: (alt = Simplane.getAltitude()) => Math.min(alt, 36089) * -0.0019812 + 15,
     ...
     */
    getIsaTempDeviation: (alt = Simplane.getAltitude(), sat = Simplane.getAmbientTemperature()) => sat - A32NX_Util.getIsaTemp(alt),

    meterToFeet: (meterValue) => meterValue * 3.28084,
};
...
export class NXPopUp {
     ...
     */
    showPopUp(params: PopUpParams, callbackYes, callbackNo) {
        if (params.title) {
 ...
 */
export class NXNotif {
    time: number;

    params: { id: string; title: string; type: string; theme: string; image: string; description: string; timeout: number; time: number; };
    ...
    setData(params: { title?: any; type?: any; theme?: any; image?: any; message?: any; timeout?: any; }) {
        if (params.title) {
            this.params.title = params.title;
