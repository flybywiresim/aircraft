...
import { A32NX_Util } from '@fmgc/utils/A32NX_Util';
import { FmgcFlightPhases } from '@fmgc/utils/NXFMGCFlightPhases';
import { soundList } from './A32NX_SoundManager';

export class A32NX_GPWS {
    core: any;

    minimumsState: number;
    ...
    modes: ({
        ...
        current: number; previous: number; type:
        (
            {
                sound?: undefined;
                soundPeriod?: undefined;
                gpwsLight?: undefined;
                pullUp?: undefined;
            } |
            {
        ...
        current: number; previous: number; type: any; onChange: (current: any, _: any) => void;
    })[];

    PrevShouldPullUpPlay: any;
    ...
    differentiate_radioalt(radioAlt, deltaTime) {
        if (!Number.isNaN(this.prevRadioAlt2)) {
        ...
        } else if (!Number.isNaN(this.prevRadioAlt)) {
            this.prevRadioAlt2 = this.prevRadioAlt;
            this.prevRadioAlt = radioAlt;
        } else {
...
export const RetardStateMachine = {
...
export const AltCallStateMachine = {
    init: 'ground',
