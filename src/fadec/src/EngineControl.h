...
class EngineControl {
 private:
  ...
  void updateFuel(double deltaTime) {
    ...
    if (pumpStateLeft == 0) {
      if (fuelLeftPre > 0 && leftQuantity <= 0) {
        timerLeft->reset();
        simVars->setPumpStateLeft(1);
      } else if (fuelLeftPre == 0 && leftQuantity > 0) {
        timerLeft->reset();
    ...
    if (pumpStateRight == 0) {
      if (fuelRightPre > 0 && rightQuantity <= 0) {
        timerRight->reset();
        simVars->setPumpStateRight(1);
      } else if (fuelRightPre == 0 && rightQuantity > 0) {
        timerRight->reset();
    ...
    if (abs(fuelTotalActual - fuelTotalPre) > 1 && refuelStartedByUser == 0) {
      uiFuelTamper = true;
    }
 ...
 public:
  ...
  void initialize() {
    ...
    double fuelLeftInit = (rand() % 166) + 100;  // USG
