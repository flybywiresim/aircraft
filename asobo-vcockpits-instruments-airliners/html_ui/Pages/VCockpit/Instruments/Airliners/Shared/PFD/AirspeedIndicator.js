class Jet_PFD_AirspeedIndicator extends HTMLElement {
        ...
        this.centerSVG.setAttribute("viewBox", "0 0 " + width + " " + height);
        {
            ...
            graduationGroup.setAttribute("id", "Graduations");
            {
                ...
                for (var i = 0; i < this.totalGraduations; i++) {
                    ...
                    const lineWidth = line.IsPrimary ? 20 : 8;
                    const lineHeight = line.IsPrimary ? 2 : 2;
            ...
            {
                const _cursorPosX = -2;
                const _cursorPosY = cursorHeight * 0.5 + 7;
            ...
            const stripViewPosX = _left + gradWidth + 2;
            ...
            const targetSpeedPointerPosX = _left + gradWidth;
            const targetSpeedPointerPosY = _top + _height * 0.5;
            const targetSpeedPointerWidth = width;
            ...
            const speedMarkersPosX = _left + gradWidth;
    ...
    construct_B747_8() {
        ...
        this.centerSVG.setAttribute("viewBox", "0 0 " + (width + arcWidth) + " " + height);
        {
            ...
            const cursorPosX = _left - 7;
            const cursorPosY = _top + _height * 0.5 + 3;
            const cursorWidth = width;
            const cursorHeight = 76;
            if (!this.cursorSVG) {
                this.cursorSVG = document.createElementNS(Avionics.SVG.NS, "svg");
                this.cursorSVG.setAttribute("id", "CursorGroup");
            } else {
            ...
            const stripViewPosX = _left + _width;
            const stripViewPosY = this.stripBorderSize;
            const stripViewWidth = width;
            const stripViewHeight = _height - this.stripBorderSize * 2;
            if (!this.stripsSVG) {
            ...
            this.stripsSVG.setAttribute("viewBox", "0 0 " + stripViewWidth + " " + stripViewHeight);
            {
                ...
                this.vMaxStripSVG.setAttribute("id", "VMax");
                {
                    ...
                    const dashSpacing = dashHeight * 1.15;
            ...
            {
                const textPosX = _left + _width * 1.25;
            ...
            const targetSpeedPointerPosX = _left + _width * 0.77;
            const targetSpeedPointerPosY = _top + _height * 0.5;
            const targetSpeedPointerWidth = width;
            ...
            const speedMarkersPosX = _left + _width - 5;
            const speedMarkersPosY = 0;
            this.speedMarkersWidth = width;
            this.speedMarkersHeight = 70;
            const nbHandles = Simplane.getFlapsNbHandles();
    ...
    construct_AS01B() {
        ...
        this.centerSVG.setAttribute("viewBox", "0 0 " + (width + arcWidth) + " " + height);
        {
            ...
            const cursorPosX = _left - 7;
            const cursorPosY = _top + _height * 0.5 + 7;
            const cursorWidth = width;
            const cursorHeight = 76;
            if (!this.cursorSVG) {
                this.cursorSVG = document.createElementNS(Avionics.SVG.NS, "svg");
    ...
    update(dTime) {
        ...
        const speedTrend = this.computeIASAcceleration(indicatedSpeed) * 10;
        ...
        const altitudeAboveGround = Simplane.getAltitudeAboveGround();
    ...
    computeIASAcceleration(_currentAirspeed) {
        ...
        const dTime = time - this._lastIASTime.t;
        if (dTime > 0) {
            const frameIASAcceleration = (speed - this._lastIASTime.ias) / dTime;
    ...
    updateCursorScrolling(_speed) {
        if (_speed < this.graduationMinValue && this.aircraft != Aircraft.B747_8 && this.aircraft != Aircraft.AS01B) {
            ...
            const speed = Math.max(_speed, this.graduationMinValue);
            if (this.cursorIntegrals) {
        ...
        if (this.aircraft == Aircraft.A320_NEO) {
            ...
            let hideBlueText = true;
            {
                ...
                if (this.blueAirspeed > this.graduationMinValue) {
                    const blueSpeedPosY = this.valueToSvg(currentAirspeed, this.blueAirspeed);
                    const blueSpeedHeight = 44;
                    if (blueSpeedPosY > 0) {
                        if (!forceHidePointer) {
                            if (this.blueSpeedSVG) {
            ...
            let hideRedText = true;
            {
                ...
                if (this.redAirspeed > this.graduationMinValue) {
                    const redSpeedPosY = this.valueToSvg(currentAirspeed, this.redAirspeed);
                    const redSpeedHeight = 44;
                    if (redSpeedPosY > 0) {
                        if (this.redSpeedSVG) {
            ...
            if (this.targetSpeedSVG) {
                ...
                if (APMode != AutopilotMode.MANAGED) {
                    ...
                    if (Simplane.getAutoPilotMachModeActive()) {
                        ...
                        if (machAirspeed < 1.0) {
                            const fixedMach = machAirspeed.toFixed(3);
                    ...
                    if (this.selectedAirspeed >= this.graduationMinValue) {
                        const pointerPosY = this.valueToSvg(currentAirspeed, this.selectedAirspeed);
                        if (pointerPosY > 0) {
