 ...
 * @param {FMCMainDisplay} fmc FMCMainDisplay
 * @param {A320_Neo_CDU_Display} mcdu The MCDU making the request
 * @param {() => void} updateView
 */
const getSimBriefOfp = (fmc, mcdu, updateView) => {
    ...
    return SimBriefApi.getSimBriefOfp(simBriefUserId)
        .then(data => {
            ...
            fmc.simbrief["blockFuel"] = fmc.simbrief["units"] === 'kgs' ? data.fuel.plan_ramp : lbsToKg(data.fuel.plan_ramp);
            fmc.simbrief["payload"] = fmc.simbrief["units"] === 'kgs' ? data.weights.payload : lbsToKg(data.weights.payload);
            fmc.simbrief["estZfw"] = fmc.simbrief["units"] === 'kgs' ? data.weights.est_zfw : lbsToKg(data.weights.est_zfw);
            fmc.simbrief["paxCount"] = data.weights.pax_count;
            fmc.simbrief["paxWeight"] = fmc.simbrief["units"] === 'kgs' ? data.weights.pax_weight : lbsToKg(data.weights.pax_weight);
            fmc.simbrief["cargo"] = fmc.simbrief["units"] === 'kgs' ? data.weights.cargo : lbsToKg(data.weights.cargo);
            ...
            fmc.simbrief["taxiFuel"] = fmc.simbrief["units"] === 'kgs' ? data.fuel.taxi : lbsToKg(data.fuel.taxi);
            fmc.simbrief["tripFuel"] = fmc.simbrief["units"] === 'kgs' ? data.fuel.enroute_burn : lbsToKg(data.fuel.enroute_burn);
...
const insertUplink = (fmc, mcdu) => {
    ...
    fmc.addNewMessage(NXSystemMessages.uplinkInsertInProg);

    /**
     * AOC ACT F-PLN UPLINK
     */
    fmc.tryUpdateFromTo(fromTo, async (result) => {
        if (result) {
            CDUPerformancePage.UpdateThrRedAccFromOrigin(fmc);
            CDUPerformancePage.UpdateEngOutAccFromOrigin(fmc);

            await fmc.tryUpdateAltDestination(alternateIcao);

            setTimeout(async () => {
                await uplinkRoute(fmc);
                fmc.addNewMessage(NXSystemMessages.aocActFplnUplink);
    ...
    */
    setTimeout(() => {
        fmc.setCruiseFlightLevelAndTemperature(cruiseAltitude);
        fmc.tryUpdateCostIndex(costIndex);
        fmc.tryUpdateTropo(avgTropopause);
        mcdu.requestUpdate();
    }, mcdu.getDelayHigh());
};
        ...
        return new Promise((res, rej) => {
            fmc.insertWaypointsAlongAirway(routeIdent, wpIndex, via, (result) => {
                if (result) {
                    ...
                    fmc.addNewMessage(NXSystemMessages.awyWptMismatch);
                    res(false);
                }
            });
        });
        ...
        return new Promise((res, rej) => {
            ...
            getWaypointByIdentAndCoords(fmc, routeIdent, coords, (waypoint) => {
                if (waypoint) {
                    ...
                    fmc.addNewMessage(NXSystemMessages.notInDatabase);
                    res(false);
                }
            });
        });
